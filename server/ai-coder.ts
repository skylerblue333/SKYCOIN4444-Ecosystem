...
class AICodeImprover {
    ...
    try {
      ...
      try {
        const { stdout: eslintOutput } = await execAsync(
          "pnpm eslint . --format=json 2>&1 || true"
        );
  ...
  async autoFixIssues(): Promise<number> {
    ...
    try {
      ...
      await execAsync(
        "pnpm prettier --write . --ignore-path .gitignore 2>&1 || true"
      );
      fixedCount++;
  ...
  async optimizePerformance(): Promise<ImprovalLog> {
    ...
    try {
      // Analyze bundle size
      const { stdout: bundleInfo } = await execAsync(
      ...
      log.changes =
        typeof content === "string" ? content : "No suggestions generated";
        ...
        messages: [
          {
            role: "system",
            content:
              "You are a testing expert. Analyze coverage and suggest which areas need more tests.",
          },
          {
            role: "user",
   ...
   */
  getStatistics() {
    const completed = this.improvementLogs.filter(
      l => l.status === "completed"
    ).length;
    const failed = this.improvementLogs.filter(
      l => l.status === "failed"
    ).length;
    const pending = this.improvementLogs.filter(
      l => l.status === "pending"
    ).length;
    ...
    return {
      ...
      pending,
      successRate:
        this.improvementLogs.length > 0
          ? (completed / this.improvementLogs.length) * 100
          : 0,
    };
  }
}
