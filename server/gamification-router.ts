...
export const gamificationRouter = router({
  ...
  missions: router({
    daily: protectedProcedure.query(async ({ ctx }) =>
      rewardSystem.getDailyMissions(String(ctx.user.id))
    ),
    weekly: protectedProcedure.query(async ({ ctx }) =>
      rewardSystem.getWeeklyChallenges(String(ctx.user.id))
    ),
    complete: protectedProcedure
      .input(z.object({ missionId: z.string() }))
      .mutation(async ({ ctx, input }) => {
        const reward = await rewardSystem.completeMission(
          String(ctx.user.id),
          input.missionId
        );
  ...
  shop: router({
    items: protectedProcedure.query(async () =>
      rewardSystem.getRewardShopItems()
    ),
    purchase: protectedProcedure
      .input(
        z.object({ itemId: z.string(), costType: z.enum(["xp", "tokens"]) })
      )
      .mutation(async ({ ctx, input }) => {
        const success = await rewardSystem.purchaseRewardItem(
          String(ctx.user.id),
          input.itemId
        );
        return { success };
  ...
  summary: protectedProcedure.query(async ({ ctx }) =>
    rewardSystem.getUserRewardsSummary(String(ctx.user.id))
  ),
