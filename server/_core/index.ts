...
async function startServer() {
  ...
  registerMiningHeartbeats().catch(err =>
    console.error("[Mining] Failed to register heartbeats:", err)
  );
  ...
  app.post("/api/scheduled/sprint", async (req: Request, res: Response) => {
    try {
      ...
      if (!user.isCron) {
        res.status(403).json({ error: "Forbidden: cron only" });
        return;
      }
      ...
      console.error("[Sprint Scheduled] Error:", err);
  ...
  (global as any).__notifBroadcast = (
    ...
    clients.forEach(c => {
      if (!c.writableEnded) c.write(payload);
    });
  };
  ...
  server.listen(port, () => {
    console.log(
      `[Server] Running on http://localhost:${port}/ (${process.env.NODE_ENV || "production"})`
    );
    ...
    void (async () => {
      try {
        ...
        console.warn("[FreeWill] Engine start failed:", e);
      }
      try {
