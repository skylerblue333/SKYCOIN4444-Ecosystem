...
export class CompetitiveRadarEngine {
  ...
  async generateWeeklyRadarReport(): Promise<WeeklyRadarReport> {
    ...
    const report: WeeklyRadarReport = {
      ...
      emergingGaps: [
        {
          gapDescription:
            "No competitor offers mobile-first experience with offline support",
        ...
        {
          gapDescription:
            "Limited community collaboration tools (forums, real-time collab)",
    ...
    const matrix: any = {
      ...
      competitors: competitors.map(c => ({
        ...
        features: Object.fromEntries(
          c.features.map(f => [
            f.featureName,
            { has: f.hasFeature, maturity: f.maturityLevel },
          ])
        ),
      })),
    };
  ...
  async getPricingComparison(): Promise<any> {
    ...
    return {
      ...
      marketAverage:
        competitors.reduce((sum, c) => sum + c.pricing.averagePrice, 0) /
        competitors.length,
