...
export interface SearchSuggestion {
  ...
  suggestion: string;
  suggestionType:
    "completion" | "correction" | "related" | "trending" | "personalized";
  score: number;
  entityType?: SearchEntityType;
  metadata?: Record<string, unknown>;
}
...
export const searchIndexEngine = {
  indexDocument(
    doc: Omit<SearchDocument, "embedding" | "isIndexed">
  ): SearchDocument {
  ...
  bulkIndex(
    docs: Array<Omit<SearchDocument, "embedding" | "isIndexed">>
  ): number {
    let count = 0;
    for (const doc of docs) {
...
export const fuzzySearchEngine = {
  ...
  search(
    ...
    const docs = Array.from(_searchIndex.values()).filter(
      d => d.isActive && (!entityTypes || entityTypes.includes(d.entityType))
    );
    const results: SearchResult[] = [];
    for (const doc of docs) {
      const titleScore = this._fuzzyScore(query, doc.title) * 2;
      const descScore = this._fuzzyScore(query, doc.description);
      const tagScore = doc.tags.some(t => this._fuzzyScore(query, t) > 0.8)
        ? 0.5
        : 0;
...
export const semanticSearchEngine = {
  search(
    ...
    const results: SearchResult[] = [];
    for (const doc of docs) {
      const similarity = searchIndexEngine._cosineSimilarity(
        queryEmbedding,
        doc.embedding
      );
      if (similarity > 0.5) {
...
export const aiIntentSearchEngine = {
  async detectIntent(query: string): Promise<{
    intent: string;
    entityType?: SearchEntityType;
    confidence: number;
    refinedQuery: string;
    ...
    if (q.includes("stream") || q.includes("live") || q.includes("watch")) {
      ...
      try {
        ...
        if (match) {
          ...
          entityType =
            parsed.entityType !== "null" ? parsed.entityType : undefined;
  ...
  async search(
    ...
    const fuzzyResults = fuzzySearchEngine.search(
      refinedQuery,
      entityTypes,
      limit
    );
    const semanticResults = semanticSearchEngine.search(
      refinedQuery,
      entityTypes,
      limit
    );
    ...
    if (userId) {
      ...
      if (personalization) {
        for (const r of merged) {
          if (
            personalization.preferredEntityTypes.includes(r.document.entityType)
          ) {
            r.score *= 1.2;
          }
          if (
            r.document.authorId &&
            personalization.topCreatorIds.includes(r.document.authorId)
          ) {
            r.score *= 1.3;
          }
        }
...
export const trendingSearchEngine = {
  recordSearch(
    ...
    if (existing) {
      ...
      existing.velocityScore =
        existing.searchCount /
        Math.max(1, (Date.now() - existing.firstSeenAt.getTime()) / 3600000);
...
export const predictiveSearchEngine = {
  ...
  getSuggestions(
    ...
    for (const t of trending) {
      if (
        t.query.startsWith(p) &&
        !suggestions.find(s => s.suggestion === t.query)
      ) {
        suggestions.push({
          id: `trend_${t.id}`,
          prefix: p,
    ...
    if (userId) {
      ...
      if (personalization) {
        for (const recent of personalization.recentSearches) {
          if (
            recent.toLowerCase().startsWith(p) &&
            !suggestions.find(s => s.suggestion === recent)
          ) {
            suggestions.push({
  ...
  getRelatedSearches(query: string, limit = 5): string[] {
    ...
    for (const t of _trendingSearches.values()) {
      if (
        t.query !== q &&
        (t.query.includes(q.split(" ")[0]) || q.includes(t.query.split(" ")[0]))
      ) {
...
export const searchPersonalizationEngine = {
  ...
  recordSearch(userId: number, query: string): SearchPersonalization {
    ...
    p.recentSearches = [
      query,
      ...p.recentSearches.filter(q => q !== query),
    ].slice(0, 20);
    p.searchHistory.unshift({ query, at: new Date() });
    if (p.searchHistory.length > 100)
      p.searchHistory = p.searchHistory.slice(0, 100);
...
export const universalSearch = {
  async search(params: {
    ...
    if (useAI) {
      ...
      const semantic = semanticSearchEngine.search(
        query,
        entityTypes,
        pageSize * 2
      );
    ...
    if (sortBy === "recency") {
      ...
      results.sort(
        (a, b) =>
          b.document.viewCount +
          b.document.likeCount -
          (a.document.viewCount + a.document.likeCount)
      );
    } else if (sortBy === "trending") {
      results.sort(
        (a, b) => b.document.trendingScore - a.document.trendingScore
      );
    }
    ...
    const suggestions = predictiveSearchEngine
      .getSuggestions(query.slice(0, 3), userId, 5)
      .map(s => s.suggestion);
    const relatedSearches = predictiveSearchEngine.getRelatedSearches(query, 5);
    const trending = trendingSearchEngine
      .getTrending(entityTypes?.[0], undefined, 5)
      .map(t => t.query);
  ...
  getSearchDashboard(): {
    ...
    return {
      ...
      topQueries,
      noResultsRate:
        _searchAnalytics.length > 0 ? noResults / _searchAnalytics.length : 0,
