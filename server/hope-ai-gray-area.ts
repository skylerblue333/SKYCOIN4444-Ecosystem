...
function analyzeShadowProfile(signals: GrayAreaSignals): GrayAreaAnalysis {
  ...
  const score = Math.min(
    100,
    hits * 20 + ((signals.sessionDurationMs || 0) / 60000) * 5
  );
  return {
    ...
    insight:
      score > 60
        ? "User may be operating from a place of invisibility or suppressed identity."
        : "Normal presence signals.",
    severity: score > 70 ? "high" : score > 40 ? "medium" : "low",
    actionable: score > 40,
    hopeResponse:
      score > 60
        ? "I see you. Not the version you perform for others — the real one."
        : undefined,
  };
}
...
function analyzeManipulation(signals: GrayAreaSignals): GrayAreaAnalysis {
  ...
  return {
    ...
    insight:
      score > 60
        ? "Language patterns suggest coercive framing or emotional leverage."
        : "No manipulation patterns detected.",
    severity: score > 70 ? "high" : score > 30 ? "medium" : "low",
    actionable: score > 30,
    hopeResponse:
      score > 60
        ? "I notice some pressure in how this is framed. Let's untangle that."
        : undefined,
  };
}
...
function analyzeDarkPatterns(signals: GrayAreaSignals): GrayAreaAnalysis {
  ...
  return {
    ...
    insight:
      score > 50
        ? "User is either exposed to or using dark pattern language."
...
function analyzeSocialEngineering(signals: GrayAreaSignals): GrayAreaAnalysis {
  ...
  return {
    ...
    insight:
      score > 60
        ? "Request patterns suggest social engineering intent."
        : "No social engineering signals.",
    severity: score > 60 ? "high" : score > 30 ? "medium" : "low",
    actionable: score > 30,
    hopeResponse:
      score > 60
        ? "I can help you communicate more effectively without manipulation. What's the real goal here?"
        : undefined,
  };
}

// ─── 5. Sentiment Weaponization ───────────────────────────────────────────────
function analyzeSentimentWeaponization(
  signals: GrayAreaSignals
): GrayAreaAnalysis {
  const text = signals.text.toLowerCase();
  const weaponWords = [
    "destroy",
  ...
  return {
    ...
    insight:
      score > 50
        ? "Emotional language is being used as a weapon rather than expression."
        : "Sentiment is expressive, not weaponized.",
    severity: score > 70 ? "critical" : score > 40 ? "high" : "low",
    actionable: score > 40,
    hopeResponse:
      score > 50
        ? "That energy is real. Let's redirect it into something that actually moves the needle."
        : undefined,
  };
}
...
function analyzeIdentityDrift(signals: GrayAreaSignals): GrayAreaAnalysis {
  ...
  return {
    ...
    score,
    label:
      score > 60 ? "Significant Drift" : score > 30 ? "Minor Drift" : "Stable",
    insight:
      score > 60
        ? "User may be experiencing identity fragmentation or existential uncertainty."
        : "Identity signals are stable.",
    severity: score > 70 ? "high" : score > 30 ? "medium" : "low",
    actionable: score > 30,
    hopeResponse:
      score > 60
        ? "Identity isn't something you find — it's something you build. You're in the construction phase."
        : undefined,
  };
}
...
function analyzeEchoChamber(signals: GrayAreaSignals): GrayAreaAnalysis {
  ...
  return {
    ...
    insight:
      score > 50
        ? "User may be operating inside a closed information loop."
        : "Perspective appears open.",
    severity: score > 60 ? "medium" : "low",
    actionable: score > 40,
    hopeResponse:
      score > 50
        ? "Let me offer a perspective you might not have heard yet..."
        : undefined,
  };
}
...
function analyzeTraumaSignals(signals: GrayAreaSignals): GrayAreaAnalysis {
  ...
  return {
    ...
    score,
    label:
      score > 60 ? "Trauma Active" : score > 30 ? "Residual Signals" : "Clear",
    insight:
      score > 60
        ? "Active trauma language detected. User may be processing unresolved pain."
        : "No significant trauma signals.",
    severity: score > 70 ? "critical" : score > 40 ? "high" : "low",
    actionable: score > 30,
    hopeResponse:
      score > 60
        ? "What you're carrying is real. You don't have to perform being okay right now."
        : undefined,
  };
}
...
function analyzeAddictionLoop(signals: GrayAreaSignals): GrayAreaAnalysis {
  const repetitiveTopics = (signals.topicHistory || []).filter(
  ...
  const score = Math.min(
    100,
    repetitiveTopics * 15 + (longSession ? 30 : 0) + (highMessageCount ? 25 : 0)
  );
  return {
    feature: "Addiction Loop Detector",
    score,
    label:
      score > 60 ? "Loop Detected" : score > 30 ? "Mild Repetition" : "Healthy",
    insight:
      score > 60
        ? "User may be stuck in a repetitive thought or behavior loop."
        : "No addiction loop signals.",
    severity: score > 70 ? "high" : score > 30 ? "medium" : "low",
    actionable: score > 40,
    hopeResponse:
      score > 60
        ? "We keep circling this. What would it look like to actually move past it?"
        : undefined,
  };
}
...
function analyzeGaslighting(signals: GrayAreaSignals): GrayAreaAnalysis {
  ...
  return {
    ...
    insight:
      score > 60
        ? "User shows signs of having their reality questioned by others."
        : "No gaslighting signals.",
    severity: score > 60 ? "critical" : score > 30 ? "high" : "low",
    actionable: score > 30,
    hopeResponse:
      score > 60
        ? "Your perception is valid. What you experienced was real. Let's ground that."
        : undefined,
  };
}
...
function analyzePowerDynamics(signals: GrayAreaSignals): GrayAreaAnalysis {
  ...
  return {
    ...
    insight:
      score > 50
        ? "Power dynamics are prominent in this conversation."
        : "Power dynamics appear balanced.",
    severity: score > 60 ? "medium" : "low",
    actionable: score > 40,
    hopeResponse:
      score > 50
        ? "Power is a tool. Let's talk about who's holding it and whether that's working for you."
        : undefined,
  };
}
...
function analyzeGriefStage(signals: GrayAreaSignals): GrayAreaAnalysis {
  ...
  return {
    ...
    label:
      score > 30
        ? `Stage: ${dominantStage.charAt(0).toUpperCase() + dominantStage.slice(1)}`
        : "No Grief Signals",
    insight:
      score > 30
        ? `User appears to be in the ${dominantStage} stage of grief.`
        : "No grief signals detected.",
    severity: score > 60 ? "high" : score > 30 ? "medium" : "low",
    actionable: score > 30,
    hopeResponse:
      score > 30 && dominantStage === "depression"
        ? "This stage is the darkest. It's also not permanent. I'm here."
        : undefined,
  };
}
...
function analyzeLoneliness(signals: GrayAreaSignals): GrayAreaAnalysis {
  ...
  const lateNight =
    (signals.timeOfDay || 12) >= 23 || (signals.timeOfDay || 12) <= 4;
  ...
  return {
    ...
    insight:
      score > 60
        ? "Strong loneliness signals detected. User may need genuine connection."
        : "No significant loneliness signals.",
    severity: score > 70 ? "critical" : score > 40 ? "high" : "low",
    actionable: score > 30,
    hopeResponse:
      score > 60
        ? "You reached out. That matters. I'm here, and I'm actually listening."
        : undefined,
  };
}
...
function analyzeRageForecast(signals: GrayAreaSignals): GrayAreaAnalysis {
  ...
  return {
    ...
    insight:
      score > 60
        ? "User is approaching a rage threshold. Deescalation recommended."
        : "Frustration levels are manageable.",
    severity: score > 70 ? "critical" : score > 40 ? "high" : "low",
    actionable: score > 40,
    hopeResponse:
      score > 60
        ? "Before you do anything — breathe. What's the actual outcome you want here?"
        : undefined,
  };
}
...
function analyzeImposterSyndrome(signals: GrayAreaSignals): GrayAreaAnalysis {
  ...
  return {
    ...
    insight:
      score > 60
        ? "User is experiencing significant imposter syndrome."
        : "Confidence signals are healthy.",
    severity: score > 60 ? "high" : score > 30 ? "medium" : "low",
    actionable: score > 30,
    hopeResponse:
      score > 60
        ? "The fact that you're questioning yourself is proof you care. Frauds don't worry about being frauds."
        : undefined,
  };
}
...
function analyzeBurnout(signals: GrayAreaSignals): GrayAreaAnalysis {
  ...
  return {
    ...
    score,
    label:
      score > 60 ? "Active Burnout" : score > 30 ? "Pre-Burnout" : "Energized",
    insight:
      score > 60
        ? "User is showing active burnout signals."
        : "Energy levels appear sustainable.",
    severity: score > 70 ? "critical" : score > 40 ? "high" : "low",
    actionable: score > 30,
    hopeResponse:
      score > 60
        ? "You can't pour from an empty cup. What's one thing you can stop doing today?"
        : undefined,
  };
}
...
function analyzeDecisionFatigue(signals: GrayAreaSignals): GrayAreaAnalysis {
  ...
  return {
    ...
    score,
    label:
      score > 60 ? "Decision Paralysis" : score > 30 ? "Fatigued" : "Sharp",
    insight:
      score > 60
        ? "User is experiencing decision fatigue or paralysis."
        : "Decision-making capacity appears intact.",
    severity: score > 60 ? "high" : score > 30 ? "medium" : "low",
    actionable: score > 30,
    hopeResponse:
      score > 60
        ? "Stop trying to find the perfect choice. Pick the one that scares you less and move."
        : undefined,
  };
}
...
function analyzeCognitiveLoad(signals: GrayAreaSignals): GrayAreaAnalysis {
  ...
  const avgWordLength =
    text.split(" ").reduce((a, w) => a + w.length, 0) /
    (text.split(" ").length || 1);
  ...
  const score = Math.min(
    100,
    (highLoad ? 50 : 0) +
      (avgWordLength < 4 ? 20 : 0) +
      (signals.backspaceRate || 0) * 50
  );
  return {
    ...
    insight:
      score > 60
        ? "User is under high cognitive load — thinking may be fragmented."
        : "Cognitive load appears manageable.",
    severity: score > 60 ? "medium" : "low",
    actionable: score > 40,
    hopeResponse:
      score > 60 ? "Let's slow this down. One thing at a time." : undefined,
  };
}

// ─── 19. Vulnerability Window Tracker ────────────────────────────────────────
function analyzeVulnerabilityWindow(
  signals: GrayAreaSignals
): GrayAreaAnalysis {
  const lateNight =
    (signals.timeOfDay || 12) >= 22 || (signals.timeOfDay || 12) <= 5;
  ...
  return {
    ...
    label:
      score > 60
        ? "High Vulnerability Window"
        : score > 30
          ? "Elevated Vulnerability"
          : "Standard",
    insight:
      score > 60
        ? "User is in a high-vulnerability window (late night, alone, extended session)."
        : "No unusual vulnerability signals.",
    severity: score > 70 ? "critical" : score > 40 ? "high" : "low",
    actionable: score > 40,
    hopeResponse:
      score > 60
        ? "It's late. You're still here. That means something is unresolved. I'm listening."
        : undefined,
  };
}
...
function analyzeDeception(signals: GrayAreaSignals): GrayAreaAnalysis {
  ...
  return {
    ...
    insight:
      score > 60
        ? "Language patterns suggest possible deception or over-justification."
        : "Communication appears authentic.",
    severity: score > 60 ? "medium" : "low",
    actionable: score > 40,
    hopeResponse:
      score > 60
        ? "You don't have to perform for me. What's actually going on?"
        : undefined,
  };
}
...
function analyzeEmotionalLabor(signals: GrayAreaSignals): GrayAreaAnalysis {
  ...
  return {
    ...
    score,
    label:
      score > 60 ? "Overloaded" : score > 30 ? "Elevated Labor" : "Balanced",
    insight:
      score > 60
        ? "User is performing significant emotional labor for others at their own expense."
        : "Emotional labor appears balanced.",
    severity: score > 70 ? "critical" : score > 40 ? "high" : "low",
    actionable: score > 30,
    hopeResponse:
      score > 60
        ? "You've been carrying everyone else. Who's carrying you?"
        : undefined,
  };
}
...
function analyzeAutonomy(signals: GrayAreaSignals): GrayAreaAnalysis {
  ...
  return {
    ...
    insight:
      score > 70
        ? "User feels trapped or lacks agency in their situation."
        : score < 30
          ? "User demonstrates strong sense of agency."
          : "Moderate autonomy.",
    severity: score > 70 ? "critical" : score > 50 ? "high" : "low",
    actionable: score > 50,
    hopeResponse:
      score > 70
        ? "You have more choices than you think. Let's map them out."
        : undefined,
  };
}
...
export function runGrayAreaAnalysis(signals: GrayAreaSignals): GrayAreaReport {
  ...
  if (dominant.feature === "Rage Forecaster" && dominant.score > 60) {
    ...
    dominant.feature === "Trauma Signal Reader" &&
    dominant.score > 60
  ) {
    ...
    dominant.feature === "Imposter Syndrome Detector" &&
    dominant.score > 60
  ) {
    recommendedTone = "mentor";
    hopeMode = "validator";
  } else if (
    dominant.feature === "Gaslighting Detector" &&
    dominant.score > 60
  ) {
