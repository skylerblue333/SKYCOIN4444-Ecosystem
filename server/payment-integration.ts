...
export const paymentRouter = router({
  ...
  createSession: protectedProcedure
    ...
    .mutation(async ({ ctx, input }: any) => {
      return paymentIntegration.createPaymentSession(
        ctx.user.id,
        input.amount,
        input.currency
      );
  ...
  processStripe: protectedProcedure
    ...
    .mutation(async ({ input }: any) => {
      return paymentIntegration.processStripePayment(
        input.sessionId,
        input.token
      );
    }),
  ...
  processCrypto: protectedProcedure
    ...
    .mutation(async ({ input }: any) => {
      return paymentIntegration.processCryptoPayment(
        input.sessionId,
        input.walletAddress,
        input.cryptoType
      );
  ...
  getHistory: protectedProcedure
    ...
    .query(async ({ ctx, input }: any) => {
      return paymentIntegration.getPaymentHistory(
        ctx.user.id,
        input?.limit || 20
      );
    }),
  ...
  generateInvoice: protectedProcedure
    .input(z.object({ transactionId: z.string() }))
    .mutation(async ({ ctx, input }: any) => {
      return paymentIntegration.generateInvoice(
        input.transactionId,
        ctx.user.id
      );
    }),
  ...
  refundPayment: protectedProcedure
    ...
    .mutation(async ({ input }: any) => {
      return paymentIntegration.refundPayment(
        input.transactionId,
        input.reason
      );
    }),
