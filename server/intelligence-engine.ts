...
export async function buildTwinContext(userId: number): Promise<string> {
  ...
  const lines: string[] = [];
  lines.push(
    "You are HOPE AI, the user's personal digital twin and orchestrator across the SKYCOIN4444 ecosystem."
  );
  lines.push(
    "You have persistent memory of this user. Use it naturally; do not recite it verbatim."
  );
  ...
  if (activeGoals.length) {
    lines.push("\nCurrent goals:");
    for (const g of activeGoals.slice(0, 8))
      lines.push(
        `- ${g.title}${g.target ? ` (target: ${g.target})` : ""} [${g.status}]`
      );
  }
  ...
  if (activeProjects.length) {
    ...
    for (const p of activeProjects.slice(0, 8))
      lines.push(`- ${p.name} [${p.status}]${p.note ? ` — ${p.note}` : ""}`);
  }

  if (twin.learning.length) {
    lines.push("\nLearning in progress:");
    for (const l of twin.learning.slice(0, 8))
      lines.push(`- ${l.topic} (${l.progress}% complete)`);
  }

  if (Object.keys(twin.preferences).length) {
    lines.push("\nPreferences:");
    for (const [k, v] of Object.entries(twin.preferences).slice(0, 10))
      lines.push(`- ${k}: ${v}`);
  }
...
export async function computeReputation(userId: number) {
  ...
  const builderScore = clamp(
    Math.min(50, s.blueprints * 15) +
      Math.min(30, s.listings * 8) +
      Math.min(20, (s.contributionScore ?? 0) / 5)
  );
  const teachingScore = clamp(
    Math.min(60, s.listingSales * 6) +
      Math.min(40, (s.isCreator ? 25 : 0) + (s.reputation ?? 0) / 50)
  );
  const communityScore = clamp(
    Math.min(50, (s.followerCount ?? 0) / 5) +
      Math.min(30, (s.postCount ?? 0) / 3) +
      Math.min(20, (s.level ?? 1) * 2)
  );
  // Trust starts from platform reliability/behaviour and is penalised by toxicity.
  const trustScore = clamp(
    (s.reliabilityScore ?? 50) * 0.5 +
      (s.behaviorScore ?? 50) * 0.5 -
      (s.toxicityScore ?? 0) +
      (s.verified ? 10 : 0)
  );
...
export async function scoreOpportunity(
  ...
  const parsed = parseJson<{ score: number; reasoning: string }>(textOf(res), {
    score: 50,
    reasoning: "Partial match based on available profile data.",
  });
  return {
    score: clamp(parsed.score ?? 50),
    reasoning: parsed.reasoning ?? "Match computed from profile fit.",
  };
}
...
// ════════════════════════════════════════════════════════════════════════
export async function generateMissionSteps(
  title: string,
  category: string,
  description?: string
  ...
  const parsed = parseJson<Array<{ title: string; detail: string }>>(
    textOf(res),
    []
  );
  return parsed
    ...
    .map(s => ({
      ...
      detail: String(s.detail ?? "").slice(0, 1000),
    }));
}
...
export async function generateDailySuggestions(
  ...
  const prompt = [
    ...
    snapshot.topOpportunity
      ? `Top opportunity: ${snapshot.topOpportunity}`
      : "",
  ]
    .filter(Boolean)
    .join("\n");
...
export async function generateStartup(idea: string): Promise<StartupOutput> {
  ...
  const fallback: StartupOutput = {
    ...
    mvpRoadmap: [
      {
        phase: "MVP",
        items: ["Define core flow", "Build prototype", "Get 10 users"],
      },
    ],
  ...
  return {
    ...
    mvpRoadmap:
      Array.isArray(parsed.mvpRoadmap) && parsed.mvpRoadmap.length
        ? parsed.mvpRoadmap
        : fallback.mvpRoadmap,
    teamPlan:
      Array.isArray(parsed.teamPlan) && parsed.teamPlan.length
        ? parsed.teamPlan
        : fallback.teamPlan,
  };
}
