...
export class GlobalExpansionEngine {
  ...
  private initializeRegions(): void {
    ...
    this.totalCountries = Array.from(this.regions.values()).reduce(
      (sum, r) => sum + r.countries,
      0
    );
  }
  ...
  getGlobalExpansionSummary(): any {
    const totalUsers = Array.from(this.regions.values()).reduce(
      (sum, r) => sum + r.users,
      0
    );
    const totalRevenue = Array.from(this.regions.values()).reduce(
      (sum, r) => sum + r.revenue,
      0
    );
