...
const mockInsights: Insight[] = [
  {
    ...
    description:
      "Analysis of feedback patterns shows 34% increase in mobile-related complaints",
    insight:
      "Mobile users are experiencing performance issues, particularly on older devices. Sentiment analysis shows frustration levels increasing over the past 2 weeks.",
    recommendation:
      "Prioritize mobile optimization in Q2 roadmap. Consider implementing lazy loading and reducing bundle size by 25%.",
    impact: "high",
    confidence: 0.92,
    actionable: true,
    relatedMetrics: {
  ...
  },
  {
    ...
    description:
      "Current roadmap has 3 high-effort items competing for same resources",
    insight:
      "The Mobile App Launch, AI Personalization v2, and Scalable SSO Integration are all scheduled for Q2 but require overlapping engineering resources. This creates a 40% probability of timeline slippage.",
    recommendation:
      "Shift Scalable SSO to Q3 to reduce resource contention. This maintains Mobile App Launch and AI v2 on schedule while improving team velocity.",
    impact: "high",
    confidence: 0.88,
    actionable: true,
    relatedMetrics: {
  ...
  },
  {
    ...
    description:
      "Users with <2 sessions/month show 65% churn probability within 30 days",
    insight:
      "Behavioral analysis identifies a high-risk segment: users with low engagement frequency are 3.2x more likely to churn. This segment represents 8% of active users but accounts for 34% of monthly churn.",
    recommendation:
      "Launch targeted re-engagement campaign with personalized onboarding for this segment. Expected impact: 25-30% reduction in churn for this group.",
    impact: "high",
    confidence: 0.85,
    actionable: true,
    relatedMetrics: {
  ...
  },
  {
    ...
    description:
      "Competitor B gaining market share 2.1x faster than historical average",
    insight:
      "Competitive intelligence shows Competitor B has increased marketing spend by 180% and launched 3 new features in the past month. Their market share growth rate is now 2.1x the historical average, indicating aggressive market positioning.",
    recommendation:
      "Accelerate feature roadmap for Q2. Focus on differentiation in AI personalization and mobile experience where we have 6-month lead time advantage.",
    impact: "critical",
    confidence: 0.91,
    actionable: true,
    relatedMetrics: {
  ...
  },
  {
    ...
    description:
      "Personalization algorithm change shows 45% conversion lift with 99.9% confidence",
    insight:
      "A/B test results show the new personalization algorithm increases conversion rate from 4.2% to 6.09% with p-value < 0.001. This represents a statistically significant and economically meaningful improvement.",
    recommendation:
      "Roll out new algorithm to 100% of users immediately. Estimated annual impact: $2.3M additional revenue. Monitor for any downstream effects on retention.",
    impact: "critical",
    confidence: 0.999,
    actionable: true,
    relatedMetrics: {
...
export const aiInsightsRouter = router({
   ...
   */
  getHighImpactInsights: protectedProcedure.query(async () => {
    return mockInsights
      .filter(
        i =>
  ...
  getSummary: protectedProcedure.query(async () => {
    const byEngine = mockInsights.reduce(
      (acc, i) => {
    ...
    const byImpact = mockInsights.reduce(
      (acc, i) => {
  ...
  getDashboard: protectedProcedure.query(async () => {
    const highImpact = mockInsights
      .filter(
        i =>
          i.impact === "critical" ||
          (i.impact === "high" && i.confidence > 0.85)
      )
      .slice(0, 3);
    ...
    const engineSummary = Object.entries(byEngine).map(
      ([engine, insights]) => ({
        engine,
  ...
  generateInsight: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      const insight: Insight = {
        ...
        insight:
          "Analysis of the provided data reveals significant patterns and opportunities.",
        recommendation:
          "Consider implementing changes based on the identified patterns.",
        impact: "medium",
        confidence: 0.82,
        actionable: true,
