...
export const skySchoolRouter = router({
    ...
    .query(async ({ input }) => {
      ...
      try {
        const [[course]] = await (db as any).execute(
          `SELECT * FROM sky_school_courses WHERE id = ?`,
          [input.id]
        );
        if (!course) return null;
        const [lessons] = await (db as any).execute(
          `SELECT * FROM sky_school_lessons WHERE course_id = ? ORDER BY order_index ASC`,
          [input.id]
        );
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      try {
        ...
        const newCompleted = Math.min(
          (enrollment.completed_lessons || 0) + (input.completed ? 1 : 0),
          course?.total_lessons || 1
        );
        const newProgress = Math.round(
          (newCompleted / (course?.total_lessons || 1)) * 100
        );
          ...
          messages: [
            {
              role: "system",
              content:
                "You are HOPE AI, an expert educator for the SkySchool platform. Generate concise, engaging lesson content in Markdown format. Include: overview, key concepts (3-5), practical example, and a 3-question quiz at the end.",
            },
            {
              role: "user",
  ...
  stats: publicProcedure.query(async () => {
    ...
    try {
      const [[courseCount]] = await (db as any).execute(
        `SELECT COUNT(*) as count FROM sky_school_courses WHERE status = 'published'`
      );
      const [[enrollCount]] = await (db as any).execute(
        `SELECT COUNT(*) as count FROM sky_school_enrollments`
      );
      const [[certCount]] = await (db as any).execute(
        `SELECT COUNT(*) as count FROM sky_school_certificates`
      );
      return {
  ...
  seedCourses: protectedProcedure.mutation(async ({ ctx }) => {
    ...
    const courses = [
      ...
      {
        title: "SKY444 Tokenomics Deep Dive",
        category: "crypto",
        level: "intermediate",
        total_lessons: 6,
        duration_minutes: 90,
        reward_sky: 75,
        tags: "sky444,tokenomics,defi",
      },
      {
        title: "DeFi Staking & Yield Strategies",
        category: "defi",
        level: "intermediate",
        total_lessons: 10,
      ...
      },
      {
        ...
        reward_sky: 200,
        tags: "solidity,contracts,development",
      },
      {
        ...
        duration_minutes: 60,
        reward_sky: 35,
        tags: "community,dao,governance",
      },
    ];
    let seeded = 0;
    for (const course of courses) {
      const [[existing]] = await (db as any).execute(
        `SELECT id FROM sky_school_courses WHERE title = ?`,
        [course.title]
      );
      if (!existing) {
        await (db as any).execute(
...
const GAMES = [
  {
    id: "snake",
    name: "Shadow Snake",
    description: "Classic snake with SKY444 rewards",
    maxReward: 5,
    rewardPer1000: 1,
  },
  {
    id: "memory",
    name: "Memory Matrix",
    description: "Match crypto pairs to earn SKY444",
    maxReward: 3,
    rewardPer1000: 0.5,
  },
  {
    id: "flappy",
    name: "Flappy SKY",
    description: "Fly through the blockchain",
    maxReward: 4,
    rewardPer1000: 0.8,
  },
  {
    id: "2048",
    name: "2048 Crypto",
    description: "Merge tokens to reach SKY444",
    maxReward: 6,
    rewardPer1000: 1.2,
  },
  {
...
const ACHIEVEMENTS = [
  ...
  },
  {
    id: "score_10000",
    name: "Five Digits",
    description: "Score 10,000 in any game",
  ...
  },
  {
    id: "score_100000",
    name: "Six Digits",
    description: "Score 100,000 in any game",
...
export const gamingRouter = router({
  ...
  submitScore: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (reward > 0) {
        ...
        await (db as any).execute(
          `UPDATE game_sessions SET reward_claimed = TRUE WHERE id = ?`,
          [sessionId]
        );
      }
      ...
      const [[distinctGames]] = await (db as any).execute(
        `SELECT COUNT(DISTINCT game_id) as count FROM game_sessions WHERE user_id = ?`,
        [ctx.user.id]
      );
      ...
      const achievementsToCheck = [
        ...
        {
          ...ACHIEVEMENTS[5],
          condition: Number(distinctGames?.count) >= GAMES.length,
        },
      ];
  ...
  platformStats: publicProcedure.query(async () => {
    const db = await getDb();
    if (!db)
      return { totalSessions: 0, totalPlayers: 0, totalRewardsIssued: 0 };
    try {
      const [[stats]] = await (db as any).execute(
