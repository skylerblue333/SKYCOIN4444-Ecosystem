...
export const referralEngine = {
  ...
  generateCode(
    userId: number,
    channel: ReferralRecord["channel"] = "link"
  ): ReferralRecord {
  ...
  trackClick(code: string, ipAddress?: string): boolean {
    ...
    record.metadata["clicks"] =
      ((record.metadata["clicks"] as number) ?? 0) + 1;
  ...
  convert(
    code: string,
    refereeId: number
  ): { success: boolean; rewardCents: number; reason?: string } {
    const record = _referrals.get(code);
    if (!record)
      return { success: false, rewardCents: 0, reason: "invalid_code" };
    if (record.status !== "pending")
      return { success: false, rewardCents: 0, reason: "already_converted" };
    if (record.referrerId === refereeId)
      return { success: false, rewardCents: 0, reason: "self_referral" };
  ...
  getUserStats(userId: number): {
    ...
    const allReferrals = Array.from(_referrals.values()).filter(
      r => r.referrerId === userId
    );
    return {
      code,
      totalReferrals: allReferrals.length,
      convertedReferrals: allReferrals.filter(
        r => r.status === "converted" || r.status === "rewarded"
      ).length,
      totalRewardsCents: allReferrals
        .filter(r => r.status === "rewarded")
        .reduce((s, r) => s + r.rewardAmountCents, 0),
    };
  },

  getTopReferrers(
    limit = 20
  ): Array<{ userId: number; conversions: number; rewardsCents: number }> {
    const byUser = new Map<
      number,
      { conversions: number; rewardsCents: number }
    >();
    for (const r of _referrals.values()) {
      if (r.status === "converted" || r.status === "rewarded") {
        const existing = byUser.get(r.referrerId) ?? {
          conversions: 0,
          rewardsCents: 0,
        };
        existing.conversions++;
        if (r.status === "rewarded")
          existing.rewardsCents += r.rewardAmountCents;
  ...
  getStats() {
    ...
    return {
      ...
      totalRewardsPaidCents: all
        .filter(r => r.status === "rewarded")
        .reduce((s, r) => s + r.rewardAmountCents, 0),
      conversionRate: all.length
        ? all.filter(r => r.status !== "pending").length / all.length
        : 0,
    };
  },
};
...
export const funnelTracker = {
  record(
    ...
    if (!_userFunnelStages.has(userId))
      _userFunnelStages.set(userId, new Set());
  ...
  getConversionRate(fromStage: FunnelStage, toStage: FunnelStage): number {
    const fromUsers = new Set(
      _funnelEntries.filter(e => e.stage === fromStage).map(e => e.userId)
    );
    const toUsers = new Set(
      _funnelEntries.filter(e => e.stage === toStage).map(e => e.userId)
    );
  ...
  getFunnelReport(): Array<{
    ...
    return stages.map((stage, i) => {
      const users = new Set(
        _funnelEntries.filter(e => e.stage === stage).map(e => e.userId)
      ).size;
      const prevStage = stages[i - 1];
      const conversionFromPrev = prevStage
        ? this.getConversionRate(prevStage, stage)
        : 1;
  ...
  getStats() {
    const signupCount = new Set(
      _funnelEntries.filter(e => e.stage === "signup").map(e => e.userId)
    ).size;
    const paymentCount = new Set(
      _funnelEntries.filter(e => e.stage === "first_payment").map(e => e.userId)
    ).size;
    return {
...
export const cohortAnalyzer = {
  assignUserToCohort(
    ...
    const key =
      type === "monthly" ? getMonthStr(joinedAt) : getWeekStr(joinedAt);
  ...
  getRetentionCurve(
    cohortKey: string,
    maxPeriods = 12
  ): Array<{ period: number; retainedUsers: number; retentionRate: number }> {
    const cohort = _cohorts.get(cohortKey);
    if (!cohort) return [];
    ...
    return Array.from({ length: maxPeriods }, (_, i) => {
      ...
      return {
        ...
        retentionRate: baseSize > 0 ? retained / baseSize : 0,
      };
    });
  },

  getAverageRetentionByPeriod(
    type: "monthly" | "weekly" = "monthly",
    maxPeriods = 12
  ): Array<{ period: number; averageRetentionRate: number }> {
    const relevantCohorts = Array.from(_cohorts.values()).filter(
      c => c.cohortType === type
    );
    ...
    return Array.from({ length: maxPeriods }, (_, i) => {
      ...
      return {
        period,
        averageRetentionRate: rates.reduce((s, r) => s + r, 0) / rates.length,
      };
    });
  },
...
export const growthDashboard = {
  ...
  getGrowthHealth(): { healthy: boolean; score: number; signals: string[] } {
    ...
    if (viral.kFactor >= 1.0) {
      score += 20;
      signals.push("viral_growth");
    } else if (viral.kFactor >= 0.5) {
      score += 10;
      signals.push("organic_growth");
    } else signals.push("needs_acquisition");

    if (activity.dauMauRatio >= 0.2) {
      score += 15;
      signals.push("strong_retention");
    } else if (activity.dauMauRatio >= 0.1) {
      score += 5;
      signals.push("moderate_retention");
    } else signals.push("weak_retention");

    if (funnel.signupToPaymentRate >= 0.05) {
      score += 15;
      signals.push("strong_monetization");
    } else if (funnel.signupToPaymentRate >= 0.01) {
      score += 5;
      signals.push("early_monetization");
...
(referralSystem as any).createReferralCode = async function (
  userId: number,
...
(referralSystem as any).convertReferral = async function (
  code: string,
  referredId: number
) {
  // Look up the referrer before converting
  const preRecord = (referralEngine as any)._getReferral
    ? (referralEngine as any)._getReferral(code)
    : null;
  const referrerId =
    preRecord?.referrerId ??
    (() => {
