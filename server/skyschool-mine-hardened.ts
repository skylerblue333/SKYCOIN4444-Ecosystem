...
const QUESTS = [
  ...
  {
    id: "q_mini_flip",
    title: "Coin Flip Master",
    desc: "Win 5 coin flips in a row",
    reward: 20,
    xp: 40,
  ...
  },
  {
    ...
    desc: "Complete the hash pattern",
    reward: 25,
    xp: 50,
    cooldownMs: 1_800_000,
    category: "game",
  ...
  },
  {
    ...
    desc: "Donate to a charity campaign",
    reward: 30,
    xp: 75,
    cooldownMs: 0,
    category: "social",
...
const ACTIVE_CHALLENGES = new Map<
  string,
  { answer: number; issuedAt: number; game: string }
>();

function issueChallenge(
  userId: number,
  game: string
): { token: string; puzzle: number[]; timeLimit: number } {
...
function verifyChallenge(token: string, answer: number): boolean {
  ...
  if (Date.now() - challenge.issuedAt > 60_000) {
...
async function getLastActionTime(
  userId: number,
  action: string
): Promise<number> {
...
async function getQuestCompletion(
  userId: number,
  questId: string
): Promise<{ completed: boolean; completedAt: number; count: number }> {
  const db = await getDb();
  ...
  try {
    ...
    return {
      ...
      count: Number(r.progress_count ?? 0),
    };
  } catch {
    return { completed: false, completedAt: 0, count: 0 };
  }
}
...
async function addXP(
  ...
  try {
    ...
    const rows = await db.execute(
      `SELECT xp, level FROM user_xp WHERE user_id = ${userId}`
    );
    const r = (rows as any).rows?.[0];
    ...
    if (newLevel > oldLevel) {
      await db.execute(
        `UPDATE user_xp SET level = ${newLevel} WHERE user_id = ${userId}`
      );
    }
  ...
  school: router({
    ...
    courses: publicProcedure.query(async () => {
      return [
        // ── Web3 & Blockchain ─────────────────────────────────────
        {
          ...
          desc: "How distributed ledgers work, consensus mechanisms, and why blockchain matters.",
          lessons: 8,
          duration: "2h 30m",
          reward: 75,
          xp: 150,
        ...
        },
        {
          ...
          desc: "Write, deploy, and audit Solidity smart contracts from scratch.",
          lessons: 12,
          duration: "4h",
          reward: 150,
          xp: 300,
        ...
        },
        {
          ...
          desc: "Uniswap, Aave, Compound — how AMMs and lending protocols work under the hood.",
          lessons: 10,
          duration: "3h 30m",
          reward: 125,
          xp: 250,
        ...
        },
        {
          id: 4,
          track: "web3",
          title: "NFT Engineering",
          desc: "ERC-721, ERC-1155, metadata standards, and building NFT marketplaces.",
          lessons: 9,
          duration: "3h",
          reward: 100,
          xp: 200,
        ...
        },
        {
          ...
          desc: "Optimistic rollups, ZK proofs, Polygon, Arbitrum — the scaling landscape.",
          lessons: 8,
          duration: "2h 45m",
          reward: 125,
          xp: 250,
        ...
        // ── Crypto Trading ────────────────────────────────────────
        {
          ...
          desc: "Spot trading, order types, reading charts, and managing risk.",
          lessons: 10,
          duration: "3h",
          reward: 100,
          xp: 200,
          level: "beginner",
          icon: "📈",
          tags: ["trading", "charts", "risk"],
        },
        {
          id: 7,
          track: "trading",
          title: "Technical Analysis",
          desc: "RSI, MACD, Bollinger Bands, support/resistance, and pattern recognition.",
          lessons: 14,
          duration: "5h",
          reward: 175,
          xp: 350,
          level: "intermediate",
          icon: "🔍",
          tags: ["ta", "indicators", "patterns"],
        },
        {
          id: 8,
          track: "trading",
          title: "DeFi Yield Strategies",
          desc: "Liquidity provision, yield farming, impermanent loss, and APY optimization.",
          lessons: 11,
          duration: "3h 45m",
          reward: 150,
          xp: 300,
        ...
        },
        {
          ...
          desc: "Perpetual contracts, funding rates, leverage, and hedging strategies.",
          lessons: 12,
          duration: "4h 30m",
          reward: 175,
          xp: 350,
          level: "advanced",
          icon: "📊",
          tags: ["futures", "leverage", "derivatives"],
        },
        // ── AI & Machine Learning ─────────────────────────────────
        {
          id: 10,
          ...
          desc: "Neural networks, training, inference, and the AI landscape in 2025.",
          lessons: 8,
          duration: "2h 30m",
          reward: 75,
          xp: 150,
        ...
        },
        {
          ...
          desc: "Master prompting techniques for GPT-4, Claude, and Gemini.",
          lessons: 7,
          duration: "2h",
          reward: 75,
          xp: 150,
          level: "beginner",
          icon: "💬",
          tags: ["prompting", "llm", "gpt"],
        },
        {
          id: 12,
          track: "ai",
          title: "AI Agents & Automation",
          desc: "Build autonomous agents with tool use, memory, and multi-step reasoning.",
          lessons: 10,
          duration: "3h 30m",
          reward: 125,
          xp: 250,
        ...
        },
        {
          ...
          desc: "Build ML-powered trading bots with backtesting and live execution.",
          lessons: 13,
          duration: "5h",
          reward: 200,
          xp: 400,
          level: "advanced",
          icon: "🤖📈",
          tags: ["trading-bot", "ml", "backtesting"],
        },
        // ── Security & Hacking ────────────────────────────────────
        {
          id: 14,
          ...
          desc: "Wallet security, phishing, social engineering, and OpSec basics.",
          lessons: 8,
          duration: "2h 30m",
          reward: 75,
          xp: 150,
          level: "beginner",
          icon: "🔒",
          tags: ["security", "opsec", "wallet"],
        },
        {
          id: 15,
          track: "security",
          title: "Smart Contract Auditing",
          desc: "Find reentrancy, overflow, and access control bugs in Solidity.",
          lessons: 12,
          duration: "4h",
          reward: 175,
          xp: 350,
          level: "advanced",
          icon: "🔍🔒",
          tags: ["audit", "solidity", "bugs"],
        },
        {
          id: 16,
          track: "security",
          title: "Ethical Hacking & Bug Bounty",
          desc: "Web app hacking, OWASP Top 10, and how to earn from bug bounties.",
          lessons: 15,
          duration: "6h",
          reward: 250,
          xp: 500,
          level: "advanced",
          icon: "🕵️",
          tags: ["hacking", "bounty", "owasp"],
        },
        {
          id: 17,
          track: "security",
          title: "Privacy & Anonymity",
          desc: "Tor, VPNs, zero-knowledge proofs, and staying private on-chain.",
          lessons: 9,
          duration: "3h",
          reward: 125,
          xp: 250,
        ...
        // ── Development ───────────────────────────────────────────
        {
          id: 18,
          track: "dev",
          title: "Web3 Full-Stack Dev",
          desc: "Build dApps with React, ethers.js, and Hardhat from zero to deployed.",
          lessons: 16,
          duration: "6h 30m",
          reward: 225,
          xp: 450,
          level: "intermediate",
          icon: "💻",
          tags: ["dapp", "react", "ethers"],
        },
        {
          id: 19,
          track: "dev",
          title: "Rust for Blockchain",
          desc: "Solana programs, Anchor framework, and high-performance blockchain dev.",
          lessons: 14,
          duration: "5h 30m",
          reward: 200,
          xp: 400,
        ...
        },
        {
          ...
          desc: "ZK circuits, Groth16, PLONK, and building privacy-preserving apps.",
          lessons: 12,
          duration: "5h",
          reward: 250,
          xp: 500,
          level: "advanced",
          icon: "🔐",
          tags: ["zk", "circuits", "privacy"],
        },
        // ── SKY444 Platform ───────────────────────────────────────
        {
          id: 21,
          track: "sky444",
          title: "SKY444 Onboarding",
          desc: "Everything you need to know about the SKYCOIN4444 ecosystem.",
          lessons: 5,
          duration: "1h",
          reward: 50,
          xp: 100,
          level: "beginner",
          icon: "🌟",
          tags: ["sky444", "onboarding", "ecosystem"],
        },
        {
          id: 22,
          track: "sky444",
          title: "Proof-of-Engagement Mining",
          desc: "How PoE works, earn caps, anti-manipulation rules, and maximizing rewards.",
          lessons: 6,
          duration: "1h 30m",
          reward: 50,
          xp: 100,
          level: "beginner",
          icon: "⛏️",
          tags: ["poe", "mining", "rewards"],
        },
        {
          id: 23,
          track: "sky444",
          title: "Staking & Yield",
          desc: "Pool mechanics, APY calculation, lock periods, and compound strategies.",
          lessons: 7,
          duration: "2h",
          reward: 75,
          xp: 150,
          level: "beginner",
          icon: "💎",
          tags: ["staking", "yield", "apy"],
        },
        {
          id: 24,
          track: "sky444",
          title: "Governance & Voting",
          desc: "How DAO governance works, proposal creation, and voting power.",
          lessons: 6,
          duration: "1h 30m",
          reward: 60,
          xp: 120,
        ...
        },
        {
          id: 25,
          track: "sky444",
          title: "Creator Economy on SKY444",
          desc: "Monetize content, set up subscriptions, and earn from your community.",
          lessons: 8,
          duration: "2h 30m",
          reward: 100,
          xp: 200,
          level: "intermediate",
          icon: "🎨",
          tags: ["creator", "monetization", "subscriptions"],
        },
      ];
    }),
    ...
    myProgress: protectedProcedure.query(async ({ ctx }) => {
      ...
      const [enrollRows, xpRows] = await Promise.all([
        db.execute(
          `SELECT * FROM sky_school_enrollments WHERE user_id = ${ctx.user.id}`
        ),
    ...
    submitQuiz: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const correct =
          input.answer.toLowerCase().trim() === correctAnswer.toLowerCase();
        if (!correct)
          return {
            success: true,
            correct: false,
        ...
        if (dailyEarned >= DAILY_EARN_CAP) {
          return {
            ...
            correct: true,
            reward: 0,
            message: "Daily earn cap reached (500 SKY444/day)",
          };
        }
        ...
        await dbInst.createTransaction({
          ...
          token: "SKY444",
          amount: reward,
          description: `Quiz reward: ${input.quizId}`,
        });
    ...
    completeLesson: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if ((alreadyDone as any).rows?.length > 0)
        ...
        if (reward > 0) {
          ...
          await dbInst.createTransaction({
            ...
            token: "SKY444",
            amount: reward,
            description: `Lesson completed: course ${input.courseId} lesson ${input.lessonId}`,
          });
        }
            ...
            messages: [
              {
                role: "system",
                content:
                  "You are a crypto/Web3/AI educator. Write concise, engaging lesson content in markdown. Include key concepts, examples, and a 3-question quiz at the end. Keep it under 600 words.",
              },
              {
                role: "user",
                content: `Write lesson ${input.lessonId} for course ${input.courseId}: "${input.topic}"`,
              },
            ],
          });
          return {
  ...
  mine: router({
    ...
    stats: protectedProcedure.query(async ({ ctx }) => {
      ...
      const [balance, xpRows, dailyEarned] = await Promise.all([
        ...
        db.execute(
          `SELECT xp, level, streak_days FROM user_xp WHERE user_id = ${ctx.user.id}`
        ),
    ...
    issueChallenge: protectedProcedure
      .input(
        z.object({ game: z.enum(["hash_rush", "coin_flip", "pattern_break"]) })
      )
      .mutation(async ({ ctx, input }) => {
    ...
    submitGame: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const baseReward = Math.min(
          30,
          Math.floor(input.score / 100) * 0.5 + 5
        );
        ...
        await dbInst.createTransaction({
          ...
          token: "SKY444",
          amount: reward,
          description: `Mini-game: ${input.game} (score: ${input.score})`,
        });
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (dailyEarned >= DAILY_EARN_CAP) {
          return {
            success: false,
            reward: 0,
            error: "Daily earn cap reached (500 SKY444/day)",
            dailyEarned,
            dailyCap: DAILY_EARN_CAP,
          };
        }
        ...
        if (cooldownMs > 0) {
          const lastTime = await getLastActionTime(
            ctx.user.id,
            `Engagement mining: ${input.action}`
          );
          ...
          if (elapsed < cooldownMs) {
            ...
            return {
              success: false,
              reward: 0,
              error: `Cooldown: ${remainingSec}s remaining`,
        ...
        const reward = Math.min(
          ENGAGEMENT_REWARDS[input.action] ?? 1,
          DAILY_EARN_CAP - dailyEarned
        );
        const dbInst = await import("./db");
        ...
        await dbInst.createTransaction({
          ...
          token: input.token,
          amount: reward,
          description: `Engagement mining: ${input.action}`,
        });
        ...
        return {
          ...
          token: input.token,
          action: input.action,
          xp: Math.floor(reward),
          newLevel: xpResult.newLevel,
          leveledUp: xpResult.leveledUp,
          dailyEarned: dailyEarned + reward,
          dailyCap: DAILY_EARN_CAP,
        };
      }),
    ...
    mine: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        const CONFIGS: Record<
          string,
          { difficulty: number; baseReward: number; blockTime: number }
        > = {
        ...
        const blocksFound = Math.floor(
          probability + Math.random() * probability * 0.5
        ); // reduced variance
        const rawReward = parseFloat(
          (blocksFound * cfg.baseReward * (1 + Math.random() * 0.05)).toFixed(8)
        );
        ...
        if (reward > 0) {
          ...
          await dbInst.createTransaction({
            ...
            token: input.token,
            amount: reward,
            description: `CPU Mining: ${hashesAttempted.toLocaleString()} hashes @ ${(hashPower / 1000).toFixed(2)} GH/s`,
          });
          const xpResult = await addXP(
            ctx.user.id,
            Math.max(1, Math.floor(reward * 10))
          );
          ...
          if (db) {
            await db.execute(
              `INSERT INTO mine_sessions (user_id, session_start, hash_power, blocks_found, reward, xp_earned, created_at) VALUES (${ctx.user.id}, ${Date.now()}, ${hashPower}, ${blocksFound}, ${reward}, ${Math.floor(reward * 10)}, ${Date.now()})`
            );
          }
          ...
          return {
            ...
            reward,
            token: input.token,
            hashesFound: hashesAttempted,
            hashRate: `${(hashPower / 1000).toFixed(2)} GH/s`,
            blocksFound,
    ...
    quests: protectedProcedure.query(async ({ ctx }) => {
      ...
      const completionMap = new Map<
        string,
        { completedAt: number; count: number }
      >();
      for (const row of (completions as any).rows || []) {
        completionMap.set(row.quest_id, {
          completedAt: Number(row.completed_at ?? 0),
          count: Number(row.progress_count ?? 0),
        });
      }
      ...
      return QUESTS.map(q => {
        ...
        const cooldownOk =
          !c || !q.cooldownMs || Date.now() - c.completedAt >= q.cooldownMs;
        return {
          ...q,
          ...
          cooldownRemaining:
            c && q.cooldownMs
              ? Math.max(
                  0,
                  Math.ceil(
                    (q.cooldownMs - (Date.now() - c.completedAt)) / 1000
                  )
                )
              : 0,
        };
    ...
    claimQuest: protectedProcedure
      ...
      .mutation(async ({ ctx, input }) => {
        ...
        if (
          ...
          Date.now() - completion.completedAt < quest.cooldownMs
        ) {
          const remaining = Math.ceil(
            (quest.cooldownMs - (Date.now() - completion.completedAt)) / 1000
          );
          return {
            success: false,
            error: `Quest on cooldown: ${remaining}s remaining`,
          };
        }
    ...
    dailyLogin: protectedProcedure.mutation(async ({ ctx }) => {
      ...
      const xpRows = await db.execute(
        `SELECT last_login_date, streak_days FROM user_xp WHERE user_id = ${ctx.user.id}`
      );
      const xpData = (xpRows as any).rows?.[0];
      ...
      if (lastLogin === today)
        return { success: true, alreadyClaimed: true, streak };
      ...
      const dailyEarned = await getDailyEarned(ctx.user.id);
      const reward =
        dailyEarned < DAILY_EARN_CAP ? ENGAGEMENT_REWARDS.login : 0;
      if (reward > 0) {
        const dbInst = await import("./db");
        await dbInst.upsertTokenBalance(ctx.user.id, "SKY444", reward);
        await dbInst.createTransaction({
          ...
          token: "SKY444",
          amount: reward,
          description: "Engagement mining: login",
        });
      }
