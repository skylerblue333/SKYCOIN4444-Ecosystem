...
export class BehaviorEngine {
  ...
  async recordSignal(
    ...
    eventBus.publish(
      "BEHAVIOR_SIGNAL_RECORDED",
      { userId, signalType, value },
      userId
    );
  ...
  async computeArchetype(userId: number): Promise<BehaviorProfile> {
    ...
    const normalizedScores = Object.fromEntries(
      Object.entries(scores).map(([k, v]) => [
        k,
        Math.round((v / maxScore) * 100) / 100,
      ])
    ) as Record<ArchetypeLabel, number>;

    // Primary archetype = highest score
    const sortedArchetypes = Object.entries(normalizedScores).sort(
      ([, a], [, b]) => b - a
    );
    ...
    if (existing) {
      ...
      if (existing.primary !== primary) {
        eventBus.publish(
          "USER_ARCHETYPE_CHANGED",
          { userId, from: existing.primary, to: primary },
          userId
        );
      }
    } else {
    ...
    eventBus.publish(
      "ARCHETYPE_COMPUTED",
      { userId, archetype: primary, confidence },
      userId
    );
  ...
  async getBehaviorProfile(userId: number): Promise<BehaviorProfile | null> {
    ...
    const scores =
      (archetype.scores as Record<ArchetypeLabel, number>) ??
      ({} as Record<ArchetypeLabel, number>);
    ...
    return {
      ...
      personalityTraits: this.getPersonalityTraits(
        archetype.primary as ArchetypeLabel
      ),
    ...
    if (!profile) {
      return {
        ...
        recommendations: [
          ...
          {
            action: "Explore HOPE AI Twin",
            reason: "Discover your AI companion",
            priority: "high",
          },
          {
            action: "Join a quest",
            reason: "Earn XP and tokens",
            priority: "medium",
          },
        ],
        explanation:
          "New user — start with the basics to unlock personalized features.",
      };
    }
...
Generate 3 specific, actionable recommendations for this user in JSON format:
{
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        eventBus.publish(
          ...
          { userId, archetype: profile.archetype },
          userId
        );
        return result;
      }
   ...
   */
  async checkRetentionHealth(): Promise<{
    healthy: boolean;
    ...
    const [{ total }] = await db
      .select({
        total: sql<number>`COUNT(DISTINCT ${userBehaviorSignals.userId})`,
      })
      .from(userBehaviorSignals)
  ...
  private computeEngagementStyle(
    signalCount: number
  ...
  private getDefaultRecommendations(
    ...
    const archetypeRecs: Record<
      ArchetypeLabel,
      AdaptiveRecommendation["recommendations"]
    > = {
      investor: [
        {
          action: "Stake SKY4444 tokens",
          reason: "Earn passive yield on your holdings",
          priority: "high",
          tokenIncentive: "5% APY in SKY4444",
        },
        {
          action: "View token market dashboard",
          reason: "Track your portfolio performance",
          priority: "high",
        },
        {
          action: "Vote on governance proposals",
          reason: "Influence platform economics",
          priority: "medium",
        },
      ],
      creator: [
        {
          action: "Publish content in Creator Hub",
          reason: "Earn CREATOR tokens for quality content",
          priority: "high",
          tokenIncentive: "50 CREATOR tokens",
        },
        {
          action: "Set up AI Twin for fans",
          reason: "Scale your creator presence with AI",
          priority: "high",
        },
        {
          action: "Join Creator Guild",
          reason: "Collaborate and earn bonus rewards",
          priority: "medium",
        },
      ],
      gamer: [
        {
          ...
          reason: "Earn XP and level up",
          priority: "high",
          tokenIncentive: "100 XP + 10 SKY4444",
        },
        {
          action: "Join tournament",
          reason: "Compete for GOLD token prizes",
          priority: "high",
          tokenIncentive: "500 GOLD",
        },
        {
          action: "Unlock achievement badges",
          reason: "Boost your reputation score",
          priority: "medium",
        },
      ],
      builder: [
        {
          action: "Explore Developer Portal",
          reason: "Build on SKYCOIN4444 ecosystem",
          priority: "high",
        },
        {
          action: "Submit governance proposal",
          reason: "Shape platform architecture",
          priority: "high",
        },
        {
          action: "Join AI Agent Network",
          reason: "Deploy your own AI agents",
          priority: "medium",
        },
      ],
      speculator: [
        {
          action: "Monitor token price curves",
          reason: "Identify trading opportunities",
          priority: "high",
        },
        {
          action: "Participate in token swaps",
          reason: "Capitalize on price movements",
          priority: "high",
        },
        {
          action: "Track emission cap status",
          reason: "Anticipate supply changes",
          priority: "medium",
        },
      ],
      learner: [
        {
          ...
          reason: "Unlock full platform features",
          priority: "high",
          tokenIncentive: "50 XP",
        },
        {
          action: "Explore HOPE AI Twin",
          reason: "Get personalized learning path",
          priority: "high",
        },
        {
          action: "Read ecosystem documentation",
          reason: "Understand token mechanics",
          priority: "medium",
        },
      ],
      ambassador: [
        {
          action: "Refer new users",
          reason: "Earn referral bonuses in CHARITY tokens",
          priority: "high",
          tokenIncentive: "25 CHARITY per referral",
        },
        {
          action: "Join Ambassador Program",
          reason: "Become a regional cultural director",
          priority: "high",
        },
        {
          action: "Share governance votes",
          reason: "Drive community participation",
          priority: "medium",
        },
      ],
    };
