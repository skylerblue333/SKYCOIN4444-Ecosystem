...
export const social44Router = router({
    ...
    .mutation(async ({ input }) => {
      ...
      const [r] = await db.insert(polls).values({
        ...
        options: JSON.stringify(
          input.options.map((o, i) => ({ index: i, text: o, votes: 0 }))
        ),
        endsAt,
      });
  ...
  getPolls: publicProcedure
    .input(
      z.object({ postId: z.number().optional(), limit: z.number().default(20) })
    )
    .query(async ({ input }) => {
      const db = await getDb();
      if (!db) return [];
  ...
  votePoll: protectedProcedure
    .input(
      z.object({ pollId: z.number(), optionIndex: z.number().min(0).max(5) })
    )
  ...
  joinAudioRoom: protectedProcedure
    .input(
      z.object({ roomId: z.number(), asSpeaker: z.boolean().default(false) })
    )
    .mutation(async ({ input }) => {
      const db = await getDb();
      if (!db) throw new Error("DB unavailable");
      await db
        .update(audioRooms)
        .set({ listenerCount: sql`listener_count + 1` })
  ...
  addBookmark: protectedProcedure
    .input(
      z.object({
        postId: z.number(),
        collectionName: z.string().default("Saved"),
      })
    )
    .mutation(async ({ ctx, input }) => {
      const db = await getDb();
  ...
  getBroadcastChannels: publicProcedure
    ...
    .query(async ({ input }) => {
      ...
      return db
        ...
        .orderBy(desc(broadcastChannels.subscriberCount))
        .limit(input.limit);
    }),
  ...
  sendDM: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (!db) throw new Error("DB unavailable");
      const disappearsAt =
        input.isDisappearing && input.disappearsInMinutes
        ...
        .where(
          or(
            and(
              ...
              eq(directMessages.recipientId, input.withUserId)
            ),
            and(
              eq(directMessages.senderId, input.withUserId),
