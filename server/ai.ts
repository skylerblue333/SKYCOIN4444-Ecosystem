...
function fallbackModeration(content: string): ModerationResult {
  ...
  const toxicPatterns = [
    {
      words: ["spam", "buy now", "click here", "free money"],
      category: "spam",
    },
...
Return JSON array: [{"index": <number>, "relevance": <0-1>, "engagement_prediction": <0-1>, "reason": "<brief>"}]`,
    ...
    return posts.map(post => ({
      ...
      relevanceScore: Math.min(
        1,
        (post.likeCount + post.commentCount * 2) / 100
      ),
...
export async function analyzeTradingSignal(tokenData: {
  price: number;
  volume24h: number;
  change24h: number;
  marketCap: number;
