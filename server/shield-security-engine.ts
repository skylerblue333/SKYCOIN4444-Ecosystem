...
export class FraudDetectionEngine {
  ...
  recordAction(userId: number, action: string): void {
    ...
    this.recentActions.set(
      userId,
      actions.filter(a => a.timestamp > cutoff).slice(-100)
    );
  }
  ...
  analyzeTransaction(
    ...
    const actions = this.recentActions.get(userId) || [];
    const recentCount = actions.filter(
    ...
    if (recentCount > 20) {
      signals.push({
        ...
        weight: 0.3,
        detected: true,
        details: `${recentCount} actions in 5 minutes`,
      });
      riskScore += 30;
    }
  ...
  async analyzeWithAI(context: Record<string, unknown>): Promise<{
    riskScore: number;
    explanation: string;
...
export class BotDetectionEngine {
  ...
  isBot(
    ...
    const times = this.requestPatterns.get(ip) || [];
    if (times.length > 100) {
      reasons.push(`High request rate: ${times.length}/min`);
      botScore += 40;
    }
    ...
    if (times.length > 2) {
      ...
      const variance =
        ...
        intervals.length;
      if (variance < 100) {
        reasons.push("Suspiciously regular request intervals");
        botScore += 30;
      }
    }
...
export class AuditLogEngine {
  ...
  query(filter: {
    ...
    if (filter.resource)
      result = result.filter(l => l.resource.includes(filter.resource!));
  ...
  export(format: "json" | "csv"): string {
    if (format === "json")
      return JSON.stringify(this.logs.slice(-1000), null, 2);
    const headers = "id,userId,action,resource,ipAddress,success,timestamp";
    const rows = this.logs
      .slice(-1000)
      .map(
        l =>
          `${l.id},${l.userId || ""},${l.action},${l.resource},${l.ipAddress},${l.success},${l.timestamp}`
      );
...
export class ThreatDetectionEngine {
  ...
  getThreats(filter?: {
    ...
    if (filter?.severity)
      result = result.filter(t => t.severity === filter.severity);
    if (filter?.type) result = result.filter(t => t.type === filter.type);
    if (filter?.since)
      result = result.filter(t => t.timestamp >= filter.since!);
    return result.slice(-(filter?.limit || 100)).reverse();
  }
  ...
  getThreatStats(): {
    ...
    return {
      ...
      blocked: this.threats.filter(t => t.blocked).length,
      bySeverity,
      byType,
    };
  }
...
export class ShieldSecurityEngine {
  ...
  async validateRequest(req: {
    ...
    if (botResult.isBot) {
      riskScore += botResult.confidence * 0.5;
      reasons.push(...botResult.reasons);
    }

    if (req.body) {
      if (this.threatDetection.detectSQLInjection(req.body)) {
        this.threatDetection.recordThreat({
          type: "sql_injection",
          severity: "high",
          sourceIp: req.ip,
          userId: req.userId,
          endpoint: req.endpoint,
          payload: req.body.slice(0, 200),
          blocked: true,
          details: {},
        });
        return {
