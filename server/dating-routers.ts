...
router.post("/api/dating/like/:targetUserId", async (req: any, res) => {
  try {
    ...
    const existing = await db
      ...
      .where(
        and(
          ...
          eq(datingLikes.likedUserId, targetUserId)
        )
      );
    ...
    const mutualLike = await db
      ...
      .where(
        and(
          eq(datingLikes.userId, targetUserId),
  ...
  try {
    ...
    if (!hasAccess) {
      ...
      if (!canSuperLike) {
        return res
          .status(429)
          .json({ error: "Daily super like limit exceeded" });
      }
    }
...
router.get(
  "/api/dating/conversation-starters/:matchUserId",
  async (req: any, res) => {
    try {
      const userId = req.user?.id;
      ...
      res
        .status(500)
        .json({ error: "Failed to generate conversation starters" });
    }
  }
);
