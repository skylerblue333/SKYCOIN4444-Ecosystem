...
router.get("/wallet/transactions", async (req, res) => {
  try {
    ...
    const transactions = await walletManager.getTransactionHistory(
      process.env.ADMIN_WALLET_ADDRESS || "0x",
      limit
    );
...
router.post("/wallet/swap-quote", async (req, res) => {
  try {
    ...
    const quote = await baseSwapEngine.getSwapQuote(
      inputCoin,
      outputCoin,
      inputAmount
    );
...
router.post("/wallet/swap", async (req, res) => {
  try {
    const { inputCoin, outputCoin, inputAmount, fromAddress, toAddress } =
      req.body;
    ...
    const swap = await baseSwapEngine.executeSwap(
      inputCoin,
      outputCoin,
      inputAmount,
      fromAddress,
...
router.post("/wallet/swap-and-deposit", async (req, res) => {
  try {
    ...
    const swaps = await baseSwapEngine.swapAndDepositToWallet(
      minedCoins,
      fromAddress,
      toWalletAddress
    );
