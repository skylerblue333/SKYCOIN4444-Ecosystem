  ...
  return [
    // ── Navigation ──────────────────────────────────────────────
    {
      ...
      action: (_m, transcript) => {
        ...
        if (route) {
          navigate(route);
          toast.success(`🎙️ Navigating to ${cleanTranscript}`, {
            duration: 2000,
          });
        } else {
          // Fuzzy match
          ...
          const match = keys.find(
            k => cleanTranscript.includes(k) || k.includes(cleanTranscript)
          );
          if (match) {
            navigate(ROUTE_MAP[match]);
            toast.success(`🎙️ Navigating to ${match}`, { duration: 2000 });
          } else {
            toast.error(`🎙️ Unknown destination: "${cleanTranscript}"`, {
              duration: 3000,
            });
          }
        }
    ...
    // ── Crypto: Stake ────────────────────────────────────────────
    {
      ...
      action: m => {
        ...
        toast.success(
          `🎙️ Opening staking${amount ? ` — ${amount} SKY444` : ""}`,
          { duration: 2500 }
        );
      },
    ...
    // ── Crypto: Burn ─────────────────────────────────────────────
    {
      ...
      action: m => {
        ...
        toast.success(`🎙️ Opening burn${amount ? ` — ${amount} SKY444` : ""}`, {
          duration: 2500,
        });
      },
...
export function useVoiceCommands(
  callbacks?: Parameters<typeof buildCommands>[1]
) {
