...
import { CheckCircle, Circle, Zap, Gift } from "lucide-react";
      ...
      <div className="space-y-2">
        {quests.map(quest => (
          <button
            ...
            onClick={() =>
              setQuests(prev =>
                prev.map(q => (q.id === quest.id ? { ...q, done: !q.done } : q))
              )
            }
          ...
          >
            ...
            <span
              className={`text-sm flex-1 ${quest.done ? "line-through text-muted-foreground" : ""}`}
            >
              {quest.label}
            </span>
      ...
      {completed === quests.length && (
        <div className="mt-3 flex items-center gap-2 p-2 bg-purple-600/10 rounded-lg border border-purple-500/20">
          ...
          <span className="text-xs text-purple-400 font-medium">
            All quests complete! Claim your reward →
          </span>
        </div>
      )}
    </div>
