...
export function PageShell({
  ...
  return (
    <div className={`min-h-screen bg-background ${className}`}>
      {/* Page header */}
      <div
        className={`relative border-b border-border/40 bg-gradient-to-br ${gradient} overflow-hidden`}
      >
        {/* Subtle grid overlay */}
        ...
        <div className="relative max-w-screen-xl mx-auto px-4 lg:px-6 py-6">
          ...
          {breadcrumbs && breadcrumbs.length > 0 && (
            <nav className="flex items-center gap-1 text-xs text-muted-foreground mb-3">
              <Link href="/">
                <Home className="w-3 h-3 hover:text-foreground transition-colors" />
              </Link>
    ...
    <div className="bg-card border border-border/50 rounded-xl p-4 hover:border-border transition-colors">
      <div className="flex items-start justify-between gap-2">
        <div>
          ...
          {trend && (
            <p
              className={`text-xs font-medium mt-1 ${trend.up ? "text-purple-400" : "text-red-400"}`}
            >
