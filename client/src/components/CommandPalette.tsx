...
const COMMANDS = [
  ...
  {
    id: "ai-agents",
    label: "AI Market Agents",
    icon: Bot,
    route: "/ai-market-agents",
    category: "AI",
    shortcut: "",
  },
  {
      ...
      <div className="relative w-full max-w-2xl mx-4 bg-[#0a0a0f] border border-white/10 rounded-2xl shadow-2xl overflow-hidden animate-in fade-in slide-in-from-top-4 duration-200">
        ...
        <div className="max-h-[60vh] overflow-y-auto scrollbar-none">
          ...
          {filtered.map((cmd, i) => {
            const Icon = cmd.icon;
            const catColor =
              CATEGORY_COLORS[cmd.category] || "text-muted-foreground";
            return (
              <button
                key={cmd.id}
                onClick={() => execute(cmd)}
              ...
              >
                ...
                >
                  <Icon
                    className={`w-4 h-4 ${i === selected ? "text-primary" : "text-muted-foreground"}`}
                  />
                </div>
        ...
        <div className="flex items-center justify-between px-4 py-2 border-t border-white/10 bg-white/2">
          <div className="flex items-center gap-3 text-xs text-muted-foreground">
            <span className="flex items-center gap-1">
              <kbd className="px-1 bg-white/5 border border-white/10 rounded">
                ↑↓
              </kbd>{" "}
              navigate
            </span>
