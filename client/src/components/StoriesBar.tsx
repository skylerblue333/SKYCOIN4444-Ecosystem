    ...
    <div className="flex gap-3 overflow-x-auto pb-2 scrollbar-hide">
      ...
      {DEMO_STORIES.map(story => {
        ...
        return (
          ...
          >
            <div
              className={`w-14 h-14 rounded-full p-0.5 ${isViewed ? "bg-muted" : `bg-gradient-to-br ${story.gradient}`}`}
            >
            ...
            <span
              className={`text-xs whitespace-nowrap max-w-[56px] truncate ${isViewed ? "text-muted-foreground" : "text-foreground"}`}
            >
              {story.username}
            </span>
          </div>
