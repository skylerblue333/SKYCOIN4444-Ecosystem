...
export function WalletConnectButton() {
  ...
  const handleConnect = async (walletId: string) => {
    ...
    setConnected({
      ...
      balance: "Connect real wallet to see balance",
    });
    setConnecting(null);
    setOpen(false);
    toast.success(
      "Wallet UI connected — link your real wallet to see live balance"
    );
  };
        ...
        {showDropdown && (
          <div className="absolute right-0 top-full mt-2 bg-card border border-border rounded-xl shadow-xl z-50 w-64 p-3">
            ...
            <div className="space-y-1">
              <button
                onClick={handleCopy}
                className="w-full flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-primary/10 text-sm transition-colors"
              >
                {copied ? (
                ...
                <div className="flex-1">
                  <div className="flex items-center gap-2">
                    ...
                    {w.popular && (
                      <Badge
                        variant="outline"
                        className="text-[10px] border-primary/40 text-primary py-0"
                      >
                        Popular
                      </Badge>
                    )}
