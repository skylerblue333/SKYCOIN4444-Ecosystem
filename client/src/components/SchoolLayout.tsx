...
export default function SchoolLayout({
  ...
  return (
    <div className="min-h-screen bg-background flex">
      ...
      {/* Sidebar */}
      <aside
        className={`fixed lg:sticky top-0 left-0 h-screen w-64 bg-card/50 border-r border-border/50 flex flex-col z-50 transition-transform duration-300 ${sidebarOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}
      >
        {/* School Header */}
        ...
        <div className="p-4 border-b border-border/50">
          ...
          <p className="text-xs text-muted-foreground mt-1">
            {1000 - (totalXP % 1000)} XP to Level {level + 1}
          </p>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto p-3 space-y-1">
          {SCHOOL_NAV.map(item => {
            ...
            return (
              <Link key={item.label} href={item.href}>
                <div
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all cursor-pointer ${active ? "bg-primary/10 text-primary border border-primary/20" : "text-muted-foreground hover:bg-card/80 hover:text-foreground"}`}
                >
          ...
          <div className="pt-2">
            <button
              onClick={() => setCoursesExpanded(p => !p)}
            ...
            >
              ...
              Active Courses
              <ChevronDown
                className={`h-3 w-3 ml-auto transition-transform ${coursesExpanded ? "rotate-180" : ""}`}
              />
            </button>
            {coursesExpanded && (
              <div className="space-y-1 mt-1">
                {ACTIVE_COURSES.map(course => (
                  <Link
                    key={course.slug}
                    href={`/school/course/${course.slug}`}
                  >
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        <div className="sticky top-0 z-30 border-b border-border/50 bg-background/95 backdrop-blur px-4 py-3 flex items-center gap-3">
          <button
            className="lg:hidden p-1.5 rounded-lg hover:bg-card/50 transition-colors"
