...
export default function AIPersonaFeed() {
  ...
  const { data: feedData, isLoading: feedLoading } =
    trpc.aiPersonas.getBlendedFeed.useQuery(
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-gray-400 text-sm">
              Hybrid AI + real user content engine
            </p>
          </div>
        </div>
        {user && (
          <div className="flex gap-2">
            ...
            >
              <RefreshCw
                className={`w-4 h-4 mr-1 ${runCycle.isPending ? "animate-spin" : ""}`}
              />
              Generate Posts
            </Button>
          </div>
          ...
          <div className="flex gap-2 flex-wrap mb-4">
            ...
            {topics.map(t => (
              <button
                key={t}
                onClick={() =>
                  setSelectedTopic(t === selectedTopic ? undefined : t)
                }
                    ...
                    <div className="flex items-start gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 flex-wrap">
                          {post.topic && (
                            <span
                              className={`text-xs px-2 py-0.5 rounded-full capitalize ${TOPIC_COLORS[post.topic] ?? "bg-gray-800 text-gray-400"}`}
                            >
                              {post.topic}
                            </span>
                          )}
                  ...
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <Badge
                          className={`text-xs ${persona.postFrequency === "high" ? "bg-green-900/50 text-green-300" : persona.postFrequency === "medium" ? "bg-yellow-900/50 text-yellow-300" : "bg-gray-800 text-gray-400"}`}
                        >
                      ...
                      <div className="flex flex-wrap gap-1 mt-2">
                        {persona.topics.map(t => (
                          <span
                            key={t}
                            className={`text-xs px-1.5 py-0.5 rounded capitalize ${TOPIC_COLORS[t] ?? "bg-gray-800 text-gray-400"}`}
                          >
                            {t}
                          </span>
                        ))}
              ...
              <Card className="bg-gray-900 border-gray-800">
                ...
                <CardContent className="flex flex-wrap gap-2">
                  {statsData.topTopics.map(t => (
                    <div
                      key={t.topic}
                      className={`px-3 py-1.5 rounded-full text-sm capitalize ${TOPIC_COLORS[t.topic] ?? "bg-gray-800 text-gray-400"}`}
                    >
