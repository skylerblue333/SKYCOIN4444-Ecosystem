  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      <div className="sticky top-0 z-40 border-b border-border/50 bg-background/95 backdrop-blur px-4 py-2.5 flex items-center gap-3">
        ...
        <Badge
          variant="outline"
          className="text-xs font-mono text-purple-400 border-purple-500/30"
        >
          Audio
        </Badge>
      ...
      <div className="container max-w-3xl py-10">
        ...
        <div className="flex gap-2 mb-6 flex-wrap">
          {(
            ...
            <button
              key={t.value}
              onClick={() => setAudioType(t.value)}
              className={`flex items-center gap-2 px-4 py-2 rounded-xl border text-sm font-medium transition-all ${audioType === t.value ? "border-purple-500/50 bg-purple-500/10 text-purple-400" : "border-border/50 text-muted-foreground hover:border-border"}`}
            >
        ...
        <div className="space-y-4 mb-8">
          ...
          <textarea
            ...
            className="w-full h-24 rounded-xl border border-border/50 bg-card/30 p-4 text-sm resize-none focus:outline-none focus:border-purple-500/50"
          />
        </div>
        ...
        <div className="rounded-2xl border border-purple-500/20 bg-gradient-to-br from-purple-500/5 to-violet-500/5 p-8 mb-6 text-center">
          ...
          <div className="flex items-center justify-center gap-0.5 h-16 mb-6">
            {Array.from({ length: 40 }).map((_, i) => (
              <div
                key={i}
                className={`w-1 rounded-full transition-all ${recording ? "bg-purple-400 animate-pulse" : hasRecording ? "bg-purple-500/60" : "bg-border/50"}`}
                style={{
                  height: recording
                    ? `${20 + Math.random() * 40}px`
                    : hasRecording
                      ? `${8 + Math.sin(i * 0.4) * 20 + 20}px`
                      : "8px",
                }}
              />
            ))}
          ...
          <div className="flex items-center justify-center gap-4">
            {!recording ? (
              <Button
                ...
                className="bg-red-500 hover:bg-red-600 text-white gap-2 h-12 px-6"
              >
              ...
              <Button
                ...
                className="bg-red-500 hover:bg-red-600 text-white gap-2 h-12 px-6 animate-pulse"
              >
            ...
            <div className="space-y-2">
              {(
                ...
                <button
                  ...
                  className={`w-full flex items-center gap-2 p-2.5 rounded-lg border text-sm transition-all ${visibility === v.value ? "border-purple-500/50 bg-purple-500/10" : "border-border/30 hover:border-border/60"}`}
                >
          ...
          <div className="rounded-xl border border-yellow-500/30 bg-yellow-500/5 p-4">
            ...
            <div className="space-y-2 text-sm text-muted-foreground">
              <div className="flex justify-between">
                <span>Per 1K plays</span>
                <span className="text-yellow-400">~22 SKY444</span>
              </div>
              <div className="flex justify-between">
                <span>Tip revenue</span>
                <span className="text-purple-400">100%</span>
              </div>
              <div className="flex justify-between">
                <span>Subscriber plays</span>
