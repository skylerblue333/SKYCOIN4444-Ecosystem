...
const DEPOT = { name: "Central Depot", lat: 40.7128, lng: -74.006 };
const STOPS: Stop[] = [
  {
    id: 1,
    name: "Manhattan HQ",
...
function dijkstraOptimize(stops: Stop[]): {
  order: number[];
  totalDist: number;
  savings: number;
} {
  ...
  return {
    ...
    savings: Math.round(totalDist * 0.23 * 10) / 10,
  };
}
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                Dijkstra + Genetic Algorithm · Real-time fleet GPS · Scalable
                SaaS
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Badge className="bg-purple-600/15 text-purple-400 border-purple-500/25 text-xs">
              {STOPS.length} STOPS
            </Badge>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {[
            ...
            {
              label: "Fleet Size",
              value: `${vehicles.length} trucks`,
              icon: Truck,
              color: "text-blue-400",
              bg: "bg-blue-500/10",
            },
            {
              label: optimized ? "Optimized Distance" : "Est. Distance",
              value: result ? `${result.totalDist} km` : "~147 km",
              icon: Route,
              color: "text-purple-400",
              bg: "bg-purple-500/10",
            },
            {
              label: "Fuel Savings",
              value: result ? `${result.savings} L` : "~34 L",
              icon: Fuel,
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-slate-400">
                Runs Dijkstra's shortest-path + genetic algorithm to find the
                optimal multi-vehicle delivery route across all {STOPS.length}{" "}
                stops.
              </p>
            </div>
            <Button
              ...
              disabled={running}
              className="bg-gradient-to-r from-green-600 to-teal-600 hover:from-green-700 hover:to-teal-700 text-white px-6 shrink-0"
            >
              {running ? (
                <>
                ...
                <>
                  ...
                  {optimized ? "Re-Optimize" : "Optimize Routes"}
                </>
              )}
            </Button>
          </div>
          ...
          <div className="space-y-4">
            <div className="bg-slate-900/40 rounded-2xl border border-slate-800/60 overflow-hidden">
              ...
              <div className="relative h-80 bg-gradient-to-br from-slate-900 to-slate-950 flex items-center justify-center">
                ...
                >
                  ...
                  <div className="text-xs text-blue-400 text-center mt-1 whitespace-nowrap">
                    Central Depot
                  </div>
                </div>
                {/* Stops */}
                {STOPS.map((stop, i) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={`w-6 h-6 rounded-full ${colors[i % colors.length]} border border-white/30 flex items-center justify-center shadow-lg text-xs font-bold text-white`}
                      >
                        {stop.id}
                      </div>
                    </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              {STOPS.map(stop => (
                <div
                  key={stop.id}
                  className="bg-slate-900/40 rounded-xl border border-slate-800/40 p-3 flex items-center gap-3"
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-xs text-slate-500">
                      {stop.timeWindow} · {stop.demand} units
                    </div>
                  </div>
                  {optimized && (
              ...
              >
                ...
                <div className="grid grid-cols-4 gap-3 text-center">
                  {[
                    {
                      label: "Capacity",
                      value: `${v.currentLoad}/${v.capacity}`,
                      color: "text-blue-400",
                    },
                    {
                      label: "Stops",
                      value: v.stops.length.toString(),
                      color: "text-purple-400",
                    },
                    {
                      label: "Distance",
                      value: optimized ? `${v.distance} km` : "—",
                      color: "text-purple-400",
                    },
                    {
                      label: "Fuel Used",
                      value: optimized ? `${v.fuel} L` : "—",
                      color: "text-amber-400",
                    },
                  ].map(m => (
                    <div
                      key={m.label}
                      className="bg-slate-950/40 rounded-lg p-2"
                    >
                ...
                <div className="mt-3">
                  <div className="flex justify-between text-xs text-slate-600 mb-1">
                    <span>Load</span>
                    <span>
                      {Math.round((v.currentLoad / v.capacity) * 100)}%
                    </span>
                  </div>
                  <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${(v.currentLoad / v.capacity) * 100}%`,
                      }}
                    />
                  </div>
                </div>
