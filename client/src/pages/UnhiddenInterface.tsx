...
export default function UnhiddenInterface() {
  ...
  const { data: worldState, refetch: refetchWorld } =
    trpc.simulation.getWorldState.useQuery();
  ...
  const systemState = {
    ...
    Simulation: worldState
      ? `Running (tick ${(worldState as any).tick ?? 0})`
      : "Loading",
      ...
      <div className="sticky top-0 z-10 bg-background/90 backdrop-blur border-b border-border px-4 py-3 flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-muted-foreground">
            Raw API console, event log, system state
          </p>
        </div>
        <button
      ...
      <div className="max-w-4xl mx-auto px-4 py-4">
        ...
        {tab === "Event Log" && (
          <div className="space-y-2">
            ...
            <div
              ref={logRef}
              className="bg-secondary/20 border border-border/50 rounded-xl p-3 h-96 overflow-y-auto space-y-1 font-mono"
            >
              {logs.map((log, i) => (
            ...
            <div className="bg-secondary/20 border border-border/50 rounded-xl overflow-hidden">
              ...
              <div className="divide-y divide-border/30">
                {dbTables.map(t => (
                  <div
                    key={t.name}
                    className="px-4 py-2.5 flex items-center justify-between hover:bg-secondary/20 transition-colors"
                  >
                    <div className="flex items-center gap-2">
                      <div
                        className={`w-1.5 h-1.5 rounded-full ${t.status === "healthy" ? "bg-green-400" : "bg-red-400"}`}
                      />
                      <span className="text-xs font-mono text-foreground">
                        {t.name}
                      </span>
