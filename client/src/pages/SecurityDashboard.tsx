...
export default function SecurityDashboard() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "events" | "sessions"
  >("overview");

  const { data: secDashboard, refetch } = trpc.security.getDashboard.useQuery(
    undefined,
    {
      enabled: !!user,
    }
      ...
      <div className="max-w-5xl mx-auto px-4 py-8 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            {
              label: "WAF Status",
              value: secData?.wafStatus || "ACTIVE",
              icon: Shield,
              color: "text-emerald-400",
              status: "good",
            },
        ...
        {activeTab === "overview" && (
          <div className="space-y-4">
            <Card className="bg-zinc-900 border-zinc-800">
              ...
              <CardContent className="space-y-3">
                {[
                  ...
                  <div
                    key={i}
                    className={`flex items-start gap-3 p-3 border rounded-lg ${rec.bg}`}
                  >
                    <rec.icon
                      className={`w-4 h-4 ${rec.color} shrink-0 mt-0.5`}
                    />
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      className="border-zinc-700 text-xs shrink-0"
                    >
                      Fix
                    </Button>
                  </div>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <Card className="bg-zinc-900 border-zinc-800">
                <CardHeader>
                  <CardTitle className="text-sm">Moderation Stats</CardTitle>
                </CardHeader>
                <CardContent className="space-y-2">
                  {[
                    {
                      label: "Total Actions",
                      value: secData?.totalModerationActions || 0,
                    },
                    {
                      label: "AI-Assisted",
                      value: secData?.aiModerationActions || 0,
                    },
                    {
                      label: "Last 30 Days",
                      value: secData?.last30dActions || 0,
                    },
                  ].map(item => (
                    <div
                      key={item.label}
          ...
          <Card className="bg-zinc-900 border-zinc-800">
            <CardHeader>
              <CardTitle className="text-sm">
                Security Events (your audit log)
              </CardTitle>
            </CardHeader>
            <CardContent className="p-0">
              {!Array.isArray(auditActivity) || auditActivity.length === 0 ? (
                ...
                (auditActivity as any[]).map((event: any, i: number) => (
                  <div
                    key={event.id ?? i}
                    className={`flex items-start gap-3 p-4 hover:bg-zinc-800/50 transition-colors ${i < (auditActivity as any[]).length - 1 ? "border-b border-zinc-800" : ""}`}
                  >
                    <div
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-sm text-white">
                        {event.action ?? event.message ?? "Activity recorded"}
                      </p>
