...
export default function SystemObservability() {
  ...
  const filtered =
    filter === "all"
      ? events
      : events.filter(e => e.type === filter || e.level === filter);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-white/40">
                Live event stream · API tracer · WebSocket monitor · Feed
                inspector
              </p>
            </div>
          </div>
                ...
                <div className="font-mono text-xs max-h-[500px] overflow-y-auto">
                  {filtered.map(ev => (
                    ...
                    >
                      ...
                      <span
                        className={`uppercase shrink-0 w-14 font-bold ${levelColor[ev.level]}`}
                      >
                        [{ev.type}]
                      </span>
                      ...
                      <Badge
                        variant="outline"
                        className={`text-[9px] shrink-0 ${levelColor[ev.level]} border-current/30`}
                      >
                        {ev.level}
                      </Badge>
                    </div>
              ...
              <CardContent>
                <div className="space-y-2">
                  {[
                    ...
                    <div
                      key={i}
                      className="flex items-center gap-3 p-2 rounded-lg bg-white/5 font-mono text-xs"
                    >
                      <Badge
                        variant="outline"
                      ...
                      <span
                        className={`w-8 text-right ${req.status >= 400 ? "text-[oklch(0.7_0.2_25)]" : "text-[oklch(0.75_0.2_145)]"}`}
                      >
                        {req.status}
                      </span>
                      <span
                        className={`w-16 text-right ${req.ms > 1000 ? "text-[oklch(0.85_0.18_90)]" : "text-white/40"}`}
                      >
                        {req.ms}ms
                      </span>
                ...
                <div className="space-y-2">
                  {Array.from({ length: 8 }, (_, i) => ({
                    ...
                    >
                      ...
                      <span className="text-white/60 font-mono w-28">
                        {sess.id}
                      </span>
                      <span className="text-white/70 flex-1">{sess.user}</span>
                      <Badge
                        variant="outline"
                        className="text-[9px] text-[oklch(0.8_0.15_200)] border-[oklch(0.8_0.15_200)]/30"
                      >
                        {sess.room}
                      </Badge>
                      ...
                      <span className="text-white/40 w-12 text-right">
                        {sess.latency}ms
                      </span>
                      <span className="text-white/25 w-20 text-right">
                        {sess.connected}
                  ...
                  {LATENCY_DATA.map(row => (
                    <div key={row.endpoint} className="space-y-1">
                      <div className="flex items-center justify-between text-xs">
                        ...
                        <span className="text-white/30 ml-4">
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <div className="flex items-center gap-2 mb-1">
                          <Badge
                            variant="outline"
                            className="text-[9px] text-[oklch(0.85_0.18_90)] border-[oklch(0.85_0.18_90)]/30"
                          >
                            {err.type}
                          </Badge>
