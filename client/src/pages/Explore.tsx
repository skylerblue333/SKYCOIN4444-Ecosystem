...
export default function Explore() {
  ...
  const { data: suggestions } = trpc.user.suggestedFollows.useQuery(undefined, {
    retry: false,
  });
  ...
  return (
    <div className="container py-8 max-w-6xl animate-page-in space-y-8">
      <PageHeader
        ...
        icon={Compass}
        title="Explore"
        subtitle="Discover everything the SKYCOIN4444 ecosystem has to offer"
      />
          ...
          {filteredCategories.map(c => (
            <Link key={c.label} href={c.href}>
              <div className="card p-4 text-center hover:border-primary/40 hover:shadow-glow-sm transition-all duration-200 cursor-pointer group">
                <div
                  className={`w-10 h-10 rounded-xl ${c.bg} flex items-center justify-center mx-auto mb-2 group-hover:scale-110 transition-transform`}
                >
                ...
                <div className="flex items-start gap-3">
                  <Avatar className="w-8 h-8 shrink-0">
                    <AvatarFallback className="text-xs">
                      {(post.author?.name || post.authorName || "U")[0]}
                    </AvatarFallback>
                  </Avatar>
                  <div className="flex-1 min-w-0">
                    <div className="font-semibold text-sm">
                      {post.author?.name || post.authorName || "Creator"}
                    </div>
