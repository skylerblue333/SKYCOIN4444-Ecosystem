...
export default function GTMStrategy() {
  const [tab, setTab] = useState<"revenue" | "channels" | "milestones">(
    "revenue"
  );
  const totalARR = REVENUE_STREAMS.reduce(
    (s, r) => s + parseInt(r.arr.replace(/[$K]/g, "")) * 1000,
    0
  );
      ...
      <div className="max-w-2xl mx-auto p-4 space-y-4">
        <div className="card p-4 bg-gradient-to-br from-orange-500/10 to-yellow-500/10 border border-orange-500/20">
          ...
          <p className="text-sm text-foreground">
            "Every conversation should produce a real outcome — payment, match,
            task, or earning."
          </p>
          <p className="text-xs text-muted-foreground mt-1">
            We monetize execution, not attention.
          </p>
        </div>
            ...
            {GROWTH_CHANNELS.map(c => (
              <div key={c.channel} className="card p-3 flex items-center gap-3">
                ...
                <div className="text-right">
                  ...
                  <span
                    className={`text-xs ${c.priority === "high" ? "text-green-400" : c.priority === "medium" ? "text-yellow-400" : "text-muted-foreground"}`}
                  >
                    {c.priority}
                  </span>
                </div>
