...
type Language =
  ...
  | "html";
type CodeMode =
  "generate" | "review" | "refactor" | "test" | "document" | "audit";
type AgentCategory =
  | "all"
  | "frontend"
  | "backend"
...
function ChatPanel({ agent }: { agent: Agent }) {
  ...
  const send = useCallback(() => {
    ...
    chatMut.mutate({userId: user?.id || "guest", 
      message: `[${agent.name} - ${agent.specialty}] ${msg}`,
      systemPrompt: `You are ${agent.name}, an AI coding agent specializing in ${agent.specialty}. ${agent.desc}. Provide expert, production-ready code and advice.`,
    });
    setTimeout(
      () => scrollRef.current?.scrollTo({ top: 99999, behavior: "smooth" }),
      50
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto p-3 space-y-3">
        ...
        {msgs.map((m, i) => (
          ...
          >
            ...
            <div
              className={`rounded-lg px-3 py-2 text-sm max-w-[85%] ${m.role === "user" ? "bg-purple-600/30 text-foreground" : "bg-white/5 text-foreground"}`}
            >
...
function SprintPanel() {
  ...
  const runSprint = async () => {
    ...
    try {
      const res = await chatMut.mutateAsync({userId: user?.id || "guest", 
        message: plan,
        systemPrompt:
    ...
    <div className="space-y-4">
      <div className="p-4 rounded-xl bg-gradient-to-r from-purple-900/30 to-cyan-900/30 border border-purple-500/20">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          Select agents and define a sprint goal. TITAN will coordinate all
          selected agents into a unified implementation plan.
        </p>
        <div className="mb-4">
...
function CodePanel() {
  ...
  const genMut = trpc.ai.generateCode.useMutation({
    onSuccess: d => setOutput(d.code),
  });
  const reviewMut = trpc.ai.reviewCode.useMutation({
    onSuccess: (d: any) => setOutput(d.review || JSON.stringify(d)),
  });
  const refactorMut = trpc.ai.chat.useMutation({
    onSuccess: d => setOutput(d.reply),
  });
  const testMut = trpc.ai.chat.useMutation({
    onSuccess: d => setOutput(d.reply),
  });
  const auditMut = trpc.ai.chat.useMutation({
    onSuccess: d => setOutput(d.reply),
  });
  ...
  const run = () => {
    ...
    const ctx = agent
      ? `You are ${agent.name}, specializing in ${agent.specialty}. ${agent.desc}.`
      : undefined;
    if (mode === "generate") genMut.mutate({userId: user?.id || "guest",  prompt, language: lang });
    else if (mode === "review") reviewMut.mutate({userId: user?.id || "guest",  code });
    else if (mode === "refactor")
      refactorMut.mutate({userId: user?.id || "guest", 
        message: `Refactor this ${lang} code. Goal: ${prompt}\n\n${code}`,
        systemPrompt: ctx,
      });
    else if (mode === "test")
      testMut.mutate({userId: user?.id || "guest", 
        message: `Write comprehensive tests for this ${lang} code:\n\n${code}`,
        systemPrompt: ctx,
      });
    else if (mode === "audit")
      auditMut.mutate({userId: user?.id || "guest", 
        message: `Security audit this ${lang} code. List all vulnerabilities with severity and fixes:\n\n${code}`,
        systemPrompt: ctx,
      });
    else
      genMut.mutate({userId: user?.id || "guest",  prompt: `Document this code:\n${code}`, language: lang });
  };

  const isPending =
    genMut.isPending ||
    reviewMut.isPending ||
    refactorMut.isPending ||
    testMut.isPending ||
    auditMut.isPending;
    ...
    <div className="space-y-4">
      ...
      {mode !== "generate" && (
        <Textarea
          ...
          className="bg-white/5 border-white/10 font-mono text-xs min-h-[160px]"
        />
      )}
      <Button
        onClick={run}
  ...
  return (
    <div className="min-h-screen">
      <PageHeader
        ...
        subtitle="44 Specialized Agents · Multi-Agent Sprints · Production Code"
      />
      <div className="container py-6">
                    ...
                    >
                      ...
                      <div className="flex">
                        {Array.from({ length: 5 }).map((_, i) => (
                          <span
                            key={i}
                            className={`w-1.5 h-1.5 rounded-full mx-0.5 ${i < agent.level ? "bg-current opacity-80" : "bg-white/20"}`}
                            style={{ color: agent.color }}
                          />
                        ))}
                      </div>
        ...
        <div className="mt-8 grid grid-cols-2 md:grid-cols-4 gap-4">
          {[
            ...
            >
              <div className={`${s.color} mx-auto mb-2 flex justify-center`}>
