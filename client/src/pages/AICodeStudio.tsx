...
function CodeOutput({
  ...
  return (
    <div className="rounded-xl overflow-hidden border border-border/40">
      ...
      <pre className="p-4 bg-black/50 overflow-x-auto text-xs font-mono text-purple-400 leading-relaxed max-h-[500px] overflow-y-auto whitespace-pre-wrap">
        {code}
      </pre>
    </div>
  );
...
function CodeGenPanel({ bots }: { bots: any[] }) {
  ...
  const startSSEStream = useCallback(() => {
    ...
    const params = new URLSearchParams({
      prompt: `[${language}/${mode}] ${prompt}${context ? `\n\nContext:\n${context}` : ""}`,
    ...
    es.addEventListener("chunk", e => {
      try {
        ...
        setStreamedCode(prev => prev + (d.text || ""));
      } catch {}
    });
      ...
      try {
        ...
        setResult(
          prev =>
            prev || {
              code: streamedCode,
              explanation: `Generated ${d.totalChars} chars via ${d.model}`,
              linesGenerated: streamedCode.split("\n").length,
              suggestions: [],
            }
        );
      } catch {}
      ...
      <div className="w-72 shrink-0 flex flex-col gap-4 overflow-y-auto">
        ...
        {showContext && (
          <Textarea
            ...
            className="resize-none text-xs font-mono bg-black/30 border-border/40 min-h-[100px]"
            rows={4}
          />
        )}
        <div>
      ...
      <div className="flex-1 flex flex-col gap-4 min-w-0 overflow-y-auto">
        <div>
          <label className="text-xs text-muted-foreground mb-1.5 block">
            What should {botId} build?
          </label>
          <Textarea
            value={prompt}
            onChange={e => setPrompt(e.target.value)}
            placeholder={`Describe what you want ${botId} to generate, review, or refactor...`}
            className="resize-none min-h-[120px] text-sm bg-secondary/30 border-border/40 font-sans"
            rows={5}
          />
        </div>
        <Button
        ...
        >
          {isStreaming || genMut.isPending ? (
            <>
              ...
              {BOT_ICONS[botId]} {botId} is coding...
            </>
          ) : (
            <>
              <Zap className="w-4 h-4" />
              {BOT_ICONS[botId]} Generate with {botId} (SSE)
            </>
          )}
        </Button>
          ...
          <div className="rounded-xl border border-primary/30 bg-black/40 overflow-hidden">
            <div className="flex items-center gap-2 px-3 py-2 border-b border-border/30 bg-primary/5">
              ...
              <span className="text-xs font-mono text-primary">
                Live output — {streamedCode.split("\n").length} lines
              </span>
            </div>
            <pre className="p-3 text-xs font-mono text-green-300 overflow-x-auto max-h-64 overflow-y-auto whitespace-pre-wrap">
              {streamedCode}
            </pre>
          </div>
        )}
            ...
            <div className="flex items-center gap-3 p-3 rounded-xl bg-purple-600/10 border border-purple-500/20">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-muted-foreground">
                  {result.linesGenerated} lines generated
                </p>
              </div>
            </div>
            {result.suggestions.length > 0 && (
              <div className="p-3 rounded-xl bg-secondary/30 border border-border/40">
                ...
                {result.suggestions.map((s, i) => (
                  <div
                    key={i}
                    className="flex items-start gap-2 text-xs text-muted-foreground mb-1"
                  >
...
function AutonomousPanel() {
  const { data: stats, refetch: refetchStats } =
    trpc.aiEngineer.getStats.useQuery(undefined, { refetchInterval: 3000 });
  const { data: log, refetch: refetchLog } = trpc.aiEngineer.getLog.useQuery(
    { limit: 50 },
    { refetchInterval: 3000 }
  );
  const { data: pushHistory } = trpc.aiEngineer.getPushHistory.useQuery(
    { limit: 30 },
    { refetchInterval: 5000 }
  );
  const { data: sessions } = trpc.aiEngineer.getSessions.useQuery(undefined, {
    refetchInterval: 3000,
  });
      ...
      <div className="w-72 shrink-0 flex flex-col gap-4 overflow-y-auto">
        <div className="p-4 rounded-xl bg-gradient-to-br from-primary/10 to-purple-500/10 border border-primary/20">
          <div className="flex items-center justify-between mb-3">
            ...
            <Badge
              variant="outline"
              className={`text-[10px] ${stats?.isAutonomousRunning ? "border-purple-500/30 text-purple-400" : "border-border/40 text-muted-foreground"}`}
            >
            ...
            {[
              {
                ...
                value: (stats?.totalLinesGenerated || 0).toLocaleString(),
                color: "text-purple-400",
              },
              {
                label: "Tasks Done",
                value: (stats?.totalTasksCompleted || 0).toLocaleString(),
                color: "text-cyan-400",
              },
              {
                label: "Total Pushes",
                value: (stats?.totalPushes || 0).toLocaleString(),
                color: "text-purple-400",
              },
              {
                label: "Active Bots",
                value: `${stats?.activeBots || 0}/12`,
        ...
        <Button
          ...
          disabled={cycleMut.isPending || stats?.isAutonomousRunning}
        ...
        {cycleMut.data && (
          <div className="p-3 rounded-xl bg-purple-600/10 border border-purple-500/20 text-xs">
            ...
            <p className="text-muted-foreground">
              {cycleMut.data.tasksRun} tasks · {cycleMut.data.linesGenerated}{" "}
              lines generated
            </p>
          </div>
        )}
          ...
          <div className="grid grid-cols-3 gap-1.5">
            {(sessions || []).map((s: any) => {
              ...
              return (
                <div
                  key={s.botId}
                  className="p-2 rounded-lg bg-secondary/20 border border-border/30 text-center"
                >
                  ...
                  <div
                    className={`w-1.5 h-1.5 rounded-full mx-auto mt-1 ${statusColors[s.status] || "bg-secondary"}`}
                  />
                </div>
              );
            })}
      ...
      <div className="flex-1 flex flex-col gap-4 min-w-0">
        ...
        <div className="flex-1 min-h-0">
          <div className="flex items-center justify-between mb-2">
            ...
            <Badge
              variant="outline"
              className="text-[10px] border-purple-500/30 text-purple-400 animate-pulse"
            >
              LIVE
            </Badge>
          </div>
          <div
            ref={logRef}
            className="h-48 overflow-y-auto bg-black/50 rounded-xl border border-border/40 p-3 font-mono text-xs space-y-1"
          >
            {(log || []).length === 0 && (
              <p className="text-muted-foreground">
                Waiting for autonomous cycle... Click "Run Autonomous Cycle" to
                start.
              </p>
            )}
            {(log || []).map((entry: any, i: number) => (
              <div key={i} className="flex gap-2">
                ...
                <span className={logColors[entry.level] || "text-foreground"}>
                  {entry.message}
                </span>
              </div>
            ))}
          ...
          <div className="space-y-2">
            ...
            {(pushHistory || []).map((push: any, i: number) => {
              ...
              return (
                <div
                  key={push.id || i}
                  className="p-3 rounded-xl bg-secondary/20 border border-border/30 hover:border-border/50 transition-all"
                >
                  <div className="flex items-center justify-between mb-1.5">
                    <div className="flex items-center gap-2">
                      <span className="text-sm">
                        {BOT_ICONS[push.botId] || "🤖"}
                      </span>
                      <span className="text-xs font-bold" style={{ color }}>
                        {push.botId}
                      </span>
                  ...
                  <div className="flex items-center gap-3 text-[10px] text-muted-foreground">
                    ...
                    <span className="flex items-center gap-1 text-purple-400">
                      <TrendingUp className="w-3 h-3" />+{push.linesAdded} lines
                    </span>
                  </div>
                  {push.code && (
                    <details className="mt-2">
                      ...
                      <pre className="mt-2 p-3 bg-black/40 rounded-lg text-[10px] font-mono text-purple-400 overflow-x-auto max-h-[200px] overflow-y-auto whitespace-pre-wrap">
                        {push.code.slice(0, 2000)}
                        {push.code.length > 2000 ? "\n... (truncated)" : ""}
                      </pre>
                    </details>
                  )}
                </div>
    ...
    <div className="flex flex-col gap-4 h-full overflow-y-auto">
      ...
      <div>
        ...
        <Textarea
          ...
          className="resize-none min-h-[200px] text-xs font-mono bg-black/30 border-border/40"
          rows={8}
        />
      </div>
      <Button
        onClick={() => analyzeMut.mutate({ code, language, analysisType })}
      ...
      {result && (
        <div className="space-y-4">
          <div className="flex items-center gap-4 p-4 rounded-xl bg-secondary/30 border border-border/40">
            <div
              className={`text-4xl font-bold font-mono ${result.score >= 80 ? "text-purple-400" : result.score >= 60 ? "text-yellow-400" : "text-red-400"}`}
            >
              {result.score}
            </div>
            <div className="flex-1">
              <div className="w-full bg-secondary/50 rounded-full h-2 mb-2">
                <div
                  className={`h-2 rounded-full ${result.score >= 80 ? "bg-purple-600" : result.score >= 60 ? "bg-yellow-400" : "bg-red-400"}`}
          ...
          {result.issues.length > 0 && (
            <div className="space-y-2">
              ...
              {result.issues.map((issue: any, i: number) => (
                <div
                  key={i}
                  className={`p-3 rounded-xl border ${severityColors[issue.severity] || "border-border/40 bg-secondary/20"}`}
                >
                  <div className="flex items-center gap-2 mb-1">
...
export default function AICodeStudio() {
  ...
  const { data: stats } = trpc.aiEngineer.getStats.useQuery(undefined, {
    refetchInterval: 5000,
  });
      ...
      <div className="min-h-screen flex items-center justify-center">
        <div className="p-12 text-center max-w-md rounded-2xl border border-border/40 bg-secondary/10">
          ...
          <p className="text-muted-foreground mb-6">
            Sign in to access the AI self-coding system — 12 bots generating
        ...
        <div className="grid grid-cols-4 gap-3 mb-6">
          {[
            ...
            return (
              <div
                key={i}
                className="p-4 rounded-xl bg-secondary/20 border border-border/40 flex items-center gap-3"
              >
