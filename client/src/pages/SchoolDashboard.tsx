          ...
          {MY_COURSES.map(c => (
            <Link key={c.id} href={`/school/course/${c.id}`}>
              <div className="rounded-2xl border border-slate-800/60 bg-slate-900/40 p-5 hover:border-slate-700 transition-all cursor-pointer flex items-center gap-4">
                <div
                  className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${c.color} flex items-center justify-center shrink-0`}
                >
                ...
                <Button
                  size="sm"
                  className="bg-cyan-500/20 border border-cyan-500/30 text-cyan-400 gap-1 shrink-0"
                >
            ...
            >
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  Minted on-chain · SKY School
                </p>
              </div>
              <Button
                size="sm"
                variant="outline"
                className="border-amber-500/30 text-amber-400 ml-auto shrink-0"
              >
                View
              </Button>
            </div>
