...
function AgentCard({ agent }: { agent: Agent }) {
  ...
  return (
    <Card
      className={`bg-black/60 border ${colors.border} hover:scale-[1.01] transition-transform duration-200`}
    >
      <CardContent className="p-5">
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className={`font-black text-lg ${colors.text}`}>
                {agent.name}
              </div>
          ...
          <div className="flex items-center gap-1.5">
            <div
              className={`w-2 h-2 rounded-full ${status.dot} ${agent.status === "working" ? "animate-pulse" : ""}`}
            />
...
export default function AgentCity() {
  ...
  const { data: agentMetrics } =
    trpc.enterprise.freeWill.systemSnapshot.useQuery();
            ...
            return (
              <Card key={i} className="bg-black/60 border-white/10">
                <CardContent className="p-4">
                  ...
                  <div className={`text-xl font-black font-mono ${c.text}`}>
                    {stat.value}
                  </div>
                </CardContent>
              </Card>
        ...
        <div className="flex gap-2">
          {["all", "working", "idle", "learning"].map(f => (
            ...
            >
              {f === "all"
                ? `ALL (${AGENTS.length})`
                : `${f} (${AGENTS.filter(a => a.status === f).length})`}
            </Button>
          ))}
        </div>
            ...
            <CardContent>
              <div className="grid md:grid-cols-3 gap-4">
                {[
                  {
                    label: "Goals Active",
                    value: agentMetrics?.activeGoals ?? "—",
                  },
                  {
                    label: "Economy Health",
                    value: agentMetrics?.economyHealth ?? "—",
                  },
                  {
                    label: "Nation Status",
                    value: agentMetrics?.digitalNationStatus ?? "—",
                  },
                ].map((item, i) => (
                  <div
                    key={i}
