        ...
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-4 text-sm">
            {assets.slice(0, 4).map(a => (
              <div
                key={a.symbol}
                className="flex items-center gap-2 border-r border-slate-800 pr-4 last:border-0"
              >
                <span className="text-slate-400 font-bold">{a.symbol}</span>
                <span className="font-mono">{a.price}</span>
                <span
                  className={a.change >= 0 ? "text-purple-400" : "text-red-400"}
                >
                  {a.change >= 0 ? "+" : ""}
                  {a.change}%
                </span>
      ...
      <div className="flex-1 grid grid-cols-12 gap-px bg-slate-800 overflow-hidden">
              ...
              <div
                ...
                className={`p-4 border-b border-slate-900 hover:bg-slate-900 cursor-pointer transition ${
                  selectedAsset.symbol === asset.symbol
                    ? "bg-slate-900 border-l-2 border-l-purple-500"
                    : ""
                }`}
              >
                ...
                <div className="flex justify-between items-center">
                  ...
                  <span
                    className={`text-xs font-bold ${asset.change >= 0 ? "text-purple-400" : "text-red-400"}`}
                  >
                    {asset.change >= 0 ? "+" : ""}
                    {asset.change}%
                  </span>
          ...
          <div className="p-6 border-b border-slate-900 flex justify-between items-center">
            <div>
              ...
              <p className="text-slate-400 text-sm">
                {selectedAsset.name} Ecosystem Token
              </p>
            </div>
          ...
          <div className="h-48 border-t border-slate-900 overflow-hidden flex flex-col">
            ...
            <div className="flex-1 overflow-y-auto p-2">
              {[1, 2, 3, 4, 5].map(i => (
                <div
                  key={i}
                  className="grid grid-cols-3 text-xs p-2 hover:bg-slate-900 rounded"
                >
                  <span
                    className={i % 2 === 0 ? "text-purple-400" : "text-red-400"}
                  >
                    {selectedAsset.price}
                  </span>
        ...
        <div className="col-span-12 lg:col-span-3 bg-slate-950 p-6 flex flex-col gap-6">
          ...
          <div className="flex bg-slate-900 rounded p-1">
            <button className="flex-1 py-2 bg-slate-800 rounded font-bold text-sm">
              Buy
            </button>
            <button className="flex-1 py-2 hover:bg-slate-800 rounded font-bold text-sm text-slate-500">
              Sell
            </button>
          </div>
          ...
          <div className="space-y-4">
            ...
            <div className="flex gap-2">
              {["25%", "50%", "75%", "100%"].map(p => (
                <button
                  key={p}
                  className="flex-1 py-1 bg-slate-900 hover:bg-slate-800 border border-slate-800 rounded text-xs text-slate-400"
                >
                  {p}
                </button>
              ))}
