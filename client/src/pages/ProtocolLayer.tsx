    ...
    <div className="min-h-screen bg-background">
      <div className="sticky top-0 z-10 bg-background/80 backdrop-blur-sm border-b border-border/50 px-4 py-3 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Open API & developer platform — Phase 12
          </p>
        </div>
        <Link
          href="/api-keys"
          className="ml-auto text-xs px-3 py-1.5 bg-primary text-primary-foreground rounded-lg font-medium"
        >
          Get API Key
        </Link>
      </div>
      ...
      <div className="max-w-2xl mx-auto p-4 space-y-4">
        <div className="card p-4 bg-gradient-to-br from-green-500/10 to-blue-500/10 border border-green-500/20">
          ...
          <p className="text-xs text-muted-foreground">
            Build on top of the action OS. Every platform feature is accessible
            via API. Turn conversations into actions in your own apps.
          </p>
        </div>
            ...
            {API_ENDPOINTS.map((ep, i) => (
              <div key={i} className="card p-3">
                <div className="flex items-center gap-2 mb-1">
                  <span
                    className={`text-xs font-mono font-bold px-2 py-0.5 rounded ${METHOD_COLORS[ep.method] || "text-muted-foreground bg-secondary"}`}
                  >
                    {ep.method}
                  </span>
            ...
            <div className="space-y-2">
              {SDK_FEATURES.map(f => (
                <div key={f.name} className="card p-3 flex items-center gap-3">
                  ...
                  <span className="text-xs px-2 py-0.5 bg-secondary rounded-lg text-muted-foreground">
                    {f.lang}
                  </span>
                </div>
              ))}
            ...
            <div className="card p-4 space-y-3">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button className="text-xs px-2 py-1 bg-secondary rounded-lg hover:bg-secondary/70 transition-colors">
                    Copy
                  </button>
                </div>
              </div>
