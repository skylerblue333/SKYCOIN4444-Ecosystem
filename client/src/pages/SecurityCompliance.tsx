      ...
      <div className="sticky top-0 z-10 bg-background/80 backdrop-blur-sm border-b border-border/50 px-4 py-3 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Phase 32 — Production-grade security layer
          </p>
        </div>
      ...
      <div className="max-w-2xl mx-auto p-4 space-y-4">
            ...
            {SECURITY_LAYERS.map(layer => (
              <div key={layer.name} className="card p-3">
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span
                    className={`text-xs font-medium ${layer.status === "live" ? "text-green-400" : "text-yellow-400"}`}
                  >
                    {layer.status}
                  </span>
                </div>
                <div className="grid grid-cols-2 gap-1">
                  {layer.items.map(item => (
                    ...
                    >
                      <div
                        className={`w-1.5 h-1.5 rounded-full shrink-0 ${layer.status === "live" ? "bg-green-400" : "bg-yellow-400"}`}
                      />
                      {item}
                    </div>
                  ))}
          ...
          <div className="space-y-2">
            ...
            {AUDIT_EVENTS.map((ev, i) => (
              <div key={i} className="card p-3 flex items-center gap-3">
                <div
                  className={`w-2 h-2 rounded-full shrink-0 ${ev.outcome === "allowed" ? "bg-green-400" : ev.outcome === "blocked" ? "bg-red-400" : ev.outcome === "throttled" ? "bg-yellow-400" : "bg-orange-400"}`}
                />
                ...
                <div className="text-right">
                  <div
                    className={`text-xs font-medium ${ev.risk === "high" ? "text-red-400" : ev.risk === "medium" ? "text-yellow-400" : "text-green-400"}`}
                  >
                    {ev.risk} risk
                  </div>
                  <div
                    className={`text-xs ${ev.outcome === "allowed" ? "text-green-400" : ev.outcome === "blocked" ? "text-red-400" : "text-yellow-400"}`}
                  >
                    {ev.outcome}
                  </div>
                </div>
        ...
        {activeTab === "trust" && (
          <div className="space-y-3">
            <p className="text-xs text-muted-foreground">
              Dynamic trust score (0–100) controls feature access, payment
              limits, and marketplace participation.
            </p>
            ...
            <div className="card p-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <div
                    key={sig.label}
                    className="flex items-center justify-between text-xs p-2 rounded-lg bg-secondary/20"
                  >
                    <span className="text-muted-foreground">{sig.label}</span>
                    <span
                      className={`font-bold ${sig.positive ? "text-green-400" : "text-red-400"}`}
                    >
                      {sig.effect}
                    </span>
                  </div>
        ...
        {activeTab === "flow" && (
          <div className="space-y-2">
            <p className="text-xs text-muted-foreground mb-3">
              Every user action passes through this security pipeline before
              execution.
            </p>
