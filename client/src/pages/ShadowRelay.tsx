...
export default function ShadowRelay() {
  ...
  const addToChain = (node: (typeof RELAY_NODES)[0]) => {
    if (chain.find(n => n.id === node.id)) {
      toast.error("Node already in chain");
      return;
    }
    if (chain.length >= 5) {
      toast.error("Maximum 5 hops allowed");
      return;
    }
  ...
  const activateRelay = () => {
    if (chain.length < 2) {
      toast.error("Add at least 2 nodes to the chain");
      return;
    }
    setRelayActive(true);
    toast.success(
      `Shadow Relay activated — ${chain.length}-hop chain routing all traffic`
    );
  };
    ...
    <div className="container py-8 max-w-5xl animate-page-in">
      <div className="mb-8">
        ...
        <p className="text-muted-foreground">
          Build your own onion-style routing chain. Route traffic through
          multiple trusted nodes to mask your origin.
        </p>
      </div>

      {/* Relay Chain Builder */}
      <Card
        className={`p-5 mb-6 border-2 transition-all ${relayActive ? "border-cyan-500/50 bg-cyan-500/5" : "border-border/30"}`}
      >
            ...
            <div className="flex items-center gap-2">
              ...
              {[2, 3, 4, 5].map(h => (
                <button
                  key={h}
                  onClick={() => setHops(h)}
                  className={`w-7 h-7 rounded-lg text-xs font-bold transition-all ${hops === h ? "bg-cyan-500/20 text-cyan-400 border border-cyan-500/30" : "bg-secondary/40 text-muted-foreground hover:bg-secondary"}`}
                >
                  {h}
                </button>
              ))}
            ...
            {relayActive ? (
              <Button
                ...
                onClick={() => {
                  ...
                  toast.info("Shadow Relay deactivated");
                }}
              >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {RELAY_NODES.map(node => (
          <Card
            key={node.id}
            className={`p-4 border-border/30 bg-card/50 hover:border-cyan-500/30 transition-all cursor-pointer group ${chain.find(n => n.id === node.id) ? "border-cyan-500/50 bg-cyan-500/5" : ""}`}
            onClick={() => addToChain(node)}
          >
            <div className="flex items-start justify-between mb-2">
              ...
              <Badge
                className={`text-[9px] h-4 border ${TYPE_COLORS[node.type]}`}
              >
                {node.type}
              </Badge>
            </div>
      ...
      <Card className="p-4 mt-6 border-border/30 bg-card/30">
        <div className="flex items-start gap-3">
          ...
          <p className="text-xs text-muted-foreground leading-relaxed">
            <span className="text-foreground font-medium">
              How Shadow Relay works:
            </span>{" "}
            Your traffic is encrypted in layers (like an onion) and routed
            through each node in your chain. Each node only knows the previous
            and next hop — no single node knows both your origin and
            destination. This is the same principle used by Tor, but with
            community-operated nodes you can trust and verify.
          </p>
        </div>
      </Card>
    </div>
