...
function LiveChatPanel() {
  ...
  const send = useCallback(() => {
    ...
    setInput("");
  }, [input, msgs, model, systemPrompt, chatMut, user]);
  ...
  return (
    <div className="flex flex-col h-full">
      <div className="flex items-center gap-3 p-3 border-b border-border/40 bg-background/30 shrink-0">
        ...
        <Badge
          variant="outline"
          className="text-[10px] border-purple-500/30 text-purple-400 shrink-0"
        >
          LIVE AI
        </Badge>
      </div>
      {showSystem && (
        <div className="px-3 py-2 border-b border-border/40 bg-secondary/10 shrink-0">
          <Textarea
            ...
            className="resize-none text-xs bg-transparent border-0 p-0 focus-visible:ring-0 min-h-[48px]"
            rows={2}
          />
        </div>
      )}
      ...
      >
        {msgs.map((m, i) => (
          ...
          >
            ...
            <div
              className={`max-w-[85%] rounded-xl px-4 py-3 text-sm leading-relaxed ${m.role === "user" ? "bg-primary/20 border border-primary/30 ml-auto" : "bg-secondary/40 border border-border/30"}`}
            >
      ...
      <div className="px-4 pt-2 flex gap-2 flex-wrap shrink-0">
        {QUICK_PROMPTS.map(p => (
          <button
            key={p}
            onClick={() => setInput(p)}
            className="text-[10px] px-2 py-1 rounded border border-border/40 text-muted-foreground hover:text-foreground hover:border-primary/30 transition-all"
          >
            {p}
          </button>
        ))}
      ...
      <div className="p-4 border-t border-border/40 shrink-0">
        <div className="flex gap-2">
          <Textarea
            ...
            className="resize-none min-h-[60px] max-h-[120px] text-sm bg-secondary/30 border-border/40"
            rows={2}
          />
          <Button
            onClick={send}
...
function CodeToolsPanel() {
  const [tab, setTab] = useState<"generate" | "debug" | "review" | "optimize">(
    "generate"
  );
  ...
  const genMut = trpc.ai.generateCode.useMutation({
    onSuccess: d => setResult(d.code),
  });
  const debugMut = trpc.ai.debugCode.useMutation({
    onSuccess: d => {
      setResult(d.fixed);
      setSuggestions(d.issues as string[]);
    },
  });
  const reviewMut = trpc.ai.reviewCode.useMutation({
    onSuccess: d => {
      setResult(d.review);
      setScore(d.score);
      setSuggestions((d as any).suggestions || []);
    },
  });
  const optimizeMut = trpc.ai.optimizeCode.useMutation({
    onSuccess: d => setResult(d.optimized),
  });
  ...
  const run = () => {
    ...
    if (tab === "generate") genMut.mutate({ prompt, language });
      ...
      debugMut.mutate({ code, error: error || undefined });
  ...
  return (
    <div className="flex flex-col gap-4 p-4 overflow-y-auto h-full">
        ...
        <>
          ...
          <div>
            ...
            <Textarea
              ...
              className="resize-none min-h-[100px] text-sm bg-secondary/30 border-border/40"
              rows={4}
            />
          </div>
        </>
        ...
        <>
          <div>
            ...
            <Textarea
              ...
              className="resize-none min-h-[150px] text-sm bg-black/30 border-border/40 font-mono text-xs"
              rows={6}
            />
          </div>
          {tab === "debug" && (
            <div>
              ...
              <Textarea
                ...
                className="resize-none min-h-[60px] text-sm bg-secondary/30 border-border/40 font-mono text-xs"
                rows={2}
              />
            </div>
          )}
      ...
      <Button
        onClick={run}
        disabled={
          isPending || (tab === "generate" ? !prompt.trim() : !code.trim())
        }
        ...
        <div className="space-y-3">
          {score !== null && (
            <div className="flex items-center gap-3 p-3 rounded-lg bg-secondary/30 border border-border/40">
              <div
                className={`text-3xl font-bold font-mono ${score >= 80 ? "text-purple-400" : score >= 60 ? "text-yellow-400" : "text-red-400"}`}
              >
                {score}
              </div>
              <div className="flex-1">
                <div className="w-full bg-secondary/50 rounded-full h-1.5 mb-1">
                  <div
                    className={`h-1.5 rounded-full ${score >= 80 ? "bg-purple-600" : score >= 60 ? "bg-yellow-400" : "bg-red-400"}`}
                    style={{ width: `${score}%` }}
                  />
                </div>
          ...
          <div className="relative">
            ...
            <pre className="p-4 bg-black/40 border border-t-0 border-border/40 rounded-b-lg overflow-x-auto text-xs font-mono text-purple-400 leading-relaxed max-h-[400px] overflow-y-auto whitespace-pre-wrap">
              {result}
            </pre>
          </div>
        </div>
...
function LearnPanel() {
  ...
  const learnMut = trpc.ai.learnTopic.useMutation({
    onSuccess: d => setLesson(d.lesson),
  });
  ...
  return (
    <div className="flex flex-col gap-4 p-4 overflow-y-auto h-full">
      <div className="p-4 rounded-xl bg-gradient-to-r from-primary/10 to-purple-500/10 border border-primary/20">
        ...
        <p className="text-xs text-muted-foreground">
          Ask SKYCOIN AI to teach you anything about Web3, DeFi, crypto, or
          development. Get structured lessons with examples.
        </p>
      </div>
      <div>
        <label className="text-xs text-muted-foreground mb-1.5 block">
        ...
        <Textarea
          ...
          className="resize-none min-h-[80px] text-sm bg-secondary/30 border-border/40"
          rows={3}
        />
      </div>
      <div>
      ...
      <Button
        onClick={() => learnMut.mutate({ topic })}
        disabled={!topic.trim() || learnMut.isPending}
        className="w-full gap-2 font-semibold bg-primary hover:bg-primary/90"
      >
        {learnMut.isPending ? (
          <>
    ...
    <div className="p-4 overflow-y-auto h-full">
      <div className="grid grid-cols-2 gap-3 mb-6">
        {[
          ...
          },
          {
            ...
            value: (modStats?.totalActions || 0).toLocaleString(),
            sub: "Auto-flagged",
      ...
      <div className="grid grid-cols-1 gap-3">
        {AI_MODULES.map((mod, i) => {
          const Icon = mod.icon;
          const realAccuracy =
            mod.name === "Content Moderation" && modStats?.accuracy
              ? Math.round(modStats.accuracy * 100)
              : mod.accuracy;
          return (
            <div
              key={i}
              className="p-4 rounded-xl bg-secondary/20 border border-border/40 hover:border-border/60 transition-all"
            >
              <div className="flex items-center gap-3 mb-2">
                <div
      ...
      <div className="min-h-screen flex items-center justify-center">
        <div className="p-12 text-center max-w-md rounded-2xl border border-border/40 bg-secondary/10">
          ...
          <p className="text-muted-foreground mb-6">
            Sign in to access the full AI suite — chat, code generation,
            learning, and more.
          </p>
          <a href={getLoginUrl()}>
    ...
    <div className="min-h-screen py-8">
      <div className="container">
        <PageHeader
          icon={Brain}
          title="AI Brain"
          subtitle="SKYCOIN4444 AI — smarter than ChatGPT, Grok, and Manus. Real LLM-powered chat, code generation, learning, and platform intelligence."
          badge="LIVE AI"
          backHref="/dashboard"
        />
        <div style={{ height: "calc(100vh - 240px)", minHeight: "600px" }}>
          <Tabs defaultValue="chat" className="flex flex-col h-full">
            <TabsList className="shrink-0 w-full justify-start bg-secondary/30 border border-border/40 rounded-xl p-1 mb-3">
              ...
              <TabsTrigger value="learn" className="gap-2 text-xs">
                <BookOpen className="w-3.5 h-3.5" />
                Learn
              </TabsTrigger>
              <TabsTrigger value="modules" className="gap-2 text-xs">
