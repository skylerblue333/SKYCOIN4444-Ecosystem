...
export default function GameBlockBuilder() {
  ...
  const dropBlock = useCallback(() => {
    ...
    const newWidth = Math.max(
      0,
      Math.min(topBlock.width, currentBlock.width) - overlap * 1.5
    );
    ...
    const nextBlock = generateBlock(
      isPerfect ? newBlock.width : Math.max(60, newBlock.width - 10)
    );
        ...
        {gameState === "idle" && (
          <div className="text-center">
            ...
            <p className="text-muted-foreground mb-6 max-w-sm mx-auto">
              Stack blockchain blocks as high as possible! Tap or press Space to
              drop each block. Perfect drops keep the full width. Each block = 1
              SKY444 donated to Hunger Relief.
            </p>
            <div className="grid grid-cols-3 gap-4 mb-8 max-w-xs mx-auto">
              {[
                ...
                <div
                  key={s.label}
                  className="rounded-xl border border-border/50 bg-card/30 p-3 text-center"
                >
            ...
            <Button
              onClick={startGame}
              className="bg-purple-500 hover:bg-purple-600 text-white gap-2 h-12 px-8 text-base"
            >
        ...
        {gameState === "playing" && (
          ...
          >
            <div className="flex items-center justify-between w-full max-w-xs mb-4">
              ...
              <div className="text-center">
                <p className="text-2xl font-bold text-purple-400">{donated}</p>
            ...
            {/* Game Area */}
            <div
              className="relative bg-gradient-to-b from-zinc-950 to-zinc-900 rounded-2xl border border-border/50 overflow-hidden cursor-pointer select-none"
              style={{ width: 320, height: 420 }}
            >
              {/* Moving block */}
              {currentBlock && (
                <div
                  className={`absolute top-4 h-10 rounded-md flex items-center justify-center text-white text-xs font-bold transition-none ${currentBlock.color}`}
                  style={{
                    width: currentBlock.width,
                    left: `calc(50% + ${position}px - ${currentBlock.width / 2}px)`,
                  }}
                >
                  {currentBlock.label}
                </div>
              ...
              <div className="absolute bottom-0 left-0 right-0 flex flex-col-reverse items-center pb-0">
                {visibleStack.map((block, i) => (
                  <div
                    key={block.id}
                    className={`h-10 rounded-md flex items-center justify-center text-white text-xs font-bold mb-0.5 transition-all ${block.color} ${block.perfect ? "ring-2 ring-yellow-400/50" : ""}`}
            ...
            <p className="text-xs text-muted-foreground mt-3">
              Tap anywhere or press Space to drop!
            </p>
          </div>
        )}
          ...
          <div className="text-center w-full">
            ...
            <div className="rounded-xl border border-purple-500/30 bg-purple-600/5 p-4 mb-6">
              ...
              <p className="text-xs text-muted-foreground">
                {donated} SKY444 donated to fight hunger!
              </p>
            </div>
