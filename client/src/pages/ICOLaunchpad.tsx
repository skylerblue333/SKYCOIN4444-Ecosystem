...
export default function ICOLaunchpad() {
  const [tab, setTab] = useState<
    "overview" | "tokenomics" | "vesting" | "security"
  >("overview");
    ...
    <div className="min-h-screen bg-[#0a0a0f] text-slate-100">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="flex gap-1 bg-slate-900/60 rounded-xl p-1 w-fit mb-6 border border-slate-800/50 flex-wrap">
          {(["overview", "tokenomics", "vesting", "security"] as const).map(
            t => (
              <button
                key={t}
                onClick={() => setTab(t)}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-gradient-to-br from-amber-950/30 to-orange-950/20 rounded-2xl border border-amber-800/30 p-6">
              ...
              <div className="mb-4 rounded-xl border border-red-500/30 bg-red-500/5 p-3 flex items-start gap-2">
                ...
                <div>
                  ...
                  <p className="text-xs text-slate-400 mt-0.5">
                    To prevent circular token economics and protect token value,
                    SKY444 is not accepted as ICO payment. Accepted: USDT · USDC
                    · ETH · BTC
                  </p>
                </div>
              </div>
              <div className="mb-4 flex gap-2">
                {["USDT", "USDC", "ETH", "BTC"].map(m => (
                  <button
                    key={m}
                    className="flex-1 py-2 rounded-lg border border-amber-500/30 bg-amber-500/10 text-amber-400 text-xs font-bold hover:bg-amber-500/20 transition-all"
                  >
                    {m}
                  </button>
                ))}
                <div className="flex-1 py-2 rounded-lg border border-red-500/20 bg-red-500/5 text-center">
                  <span className="text-xs text-red-400 line-through opacity-50">
                    SKY444
                  </span>
                </div>
              </div>
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-slate-950/60 border border-slate-700 rounded-xl px-4 py-3 text-white font-mono text-lg focus:outline-none focus:border-amber-500"
                    />
                  </div>
                  <div className="bg-slate-950/40 rounded-xl p-4">
                    <div className="flex justify-between text-sm mb-2">
                      ...
                      <span className="text-amber-400 font-bold">
                        {(
                          parseFloat(amount || "0") / activeStage.price
                        ).toLocaleString()}{" "}
                        SKY444
                      </span>
                    </div>
                    <div className="flex justify-between text-sm mb-2">
                      <span className="text-slate-400">Bonus (15%)</span>
                ...
                <div className="space-y-3">
                  ...
                  <div className="h-3 bg-slate-800 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${(activeStage.raised / activeStage.cap) * 100}%`,
                      }}
                    />
                  </div>
                  <div className="flex justify-between text-xs text-slate-500">
                    <span>
                      ${(activeStage.raised / 1000000).toFixed(1)}M raised
                    </span>
                    <span>${(activeStage.cap / 1000000).toFixed(0)}M cap</span>
                  </div>
                  {[
                    { label: "Price", val: `$${activeStage.price}` },
                    { label: "Bonus", val: activeStage.bonus },
                    { label: "Min Buy", val: "$100" },
                    { label: "Max Buy", val: "$50,000" },
                    { label: "Ends", val: activeStage.endDate },
                  ].map(r => (
                    <div
                      key={r.label}
                      className="flex justify-between py-1.5 border-b border-slate-800/40 last:border-0"
                    >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {STAGES.map(stage => (
                <div
                  key={stage.name}
                  className={`rounded-2xl border p-5 ${stage.status === "active" ? "bg-amber-500/5 border-amber-500/30" : stage.status === "completed" ? "bg-emerald-500/5 border-emerald-500/20" : "bg-slate-900/40 border-slate-800/60"}`}
                >
                  <div className="flex items-center justify-between mb-3">
                    <div className="font-bold text-white">{stage.name}</div>
                    <Badge
                      className={`text-xs border-0 ${stage.status === "active" ? "bg-amber-500/15 text-amber-400" : stage.status === "completed" ? "bg-emerald-500/10 text-emerald-400" : "bg-slate-700 text-slate-400"}`}
                    >
                      {stage.status === "active"
                        ? "🔴 LIVE"
                  ...
                  <div className="grid grid-cols-3 gap-2 text-center mb-3">
                    ...
                    <div>
                      ...
                      <div className="text-xs text-slate-600">Bonus</div>
                    </div>
                    <div>
                      <div className="text-sm font-bold text-purple-400">
                        ${(stage.cap / 1000000).toFixed(0)}M
                      </div>
                      <div className="text-xs text-slate-600">Cap</div>
                    </div>
                  </div>
                  <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full ${stage.status === "completed" ? "bg-emerald-500" : stage.status === "active" ? "bg-amber-500" : "bg-slate-600"}`}
                      style={{ width: `${(stage.raised / stage.cap) * 100}%` }}
                    />
                  </div>
          ...
          <div className="space-y-6">
              ...
              <div className="space-y-3">
                {TOKENOMICS.map(t => (
                  <div key={t.label} className="flex items-center gap-3">
                    <div
                      className={`w-3 h-3 rounded-full ${t.color} shrink-0`}
                    />
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                ...
                <div
                  key={m.label}
                  className="bg-slate-900/40 rounded-xl border border-slate-800/60 p-4 text-center"
                >
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="border-b border-slate-800">
                    {["Role", "Cliff", "Vesting", "TGE Unlock"].map(h => (
                      <th
                        key={h}
                        className="text-left py-3 pr-4 text-xs text-slate-500 font-medium"
                      >
                        {h}
                      </th>
                    ))}
              ...
              >
                <div className="flex items-start justify-between">
                  ...
                  <div className="text-right">
                    ...
                    <div className={`text-sm font-bold ${a.color}`}>
                      {a.score}
                    </div>
                  </div>
                </div>
