...
export default function CreatorStudio() {
  const [activeTab, setActiveTab] = useState<
    "overview" | "content" | "monetize" | "analytics"
  >("overview");
  ...
  return (
    <div className="animate-page-in">
      ...
      >
        ...
        <div
          className="glow-orb w-48 h-48 top-5 left-20 animate-hero-float"
          style={{
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 py-10 flex items-center justify-between gap-4 flex-wrap">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm desc-metallic">
                Your command center for content creation, publishing, and
                monetization
              </p>
            </div>
          </div>
      ...
      <div className="container py-8 max-w-6xl">
        ...
        <div className="flex gap-1 p-1 bg-secondary/50 rounded-xl mb-8 w-fit">
          {(["overview", "content", "monetize", "analytics"] as const).map(
            t => (
              <button
                key={t}
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                {CONTENT_TYPES.map(ct => (
                  <Link key={ct.label} href={ct.href}>
                    <div
                      className={`card-epic p-4 text-center cursor-pointer group hover:border-primary/40 transition-all duration-200`}
                    >
                      <div
                        className={`w-10 h-10 rounded-xl ${ct.bg} border ${ct.border} flex items-center justify-center mx-auto mb-2 group-hover:scale-110 transition-transform`}
                      >
              ...
              <div className="card divide-y divide-border/50">
                {[
                  ...
                  <div key={i} className="flex items-center gap-4 p-4">
                    <div
                      className={`w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 ${item.done ? "border-green-500 bg-green-500/20" : "border-border"}`}
                    >
                      {item.done && (
