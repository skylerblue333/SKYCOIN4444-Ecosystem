...
export default function Retention() {
  ...
  const [activeTab, setActiveTab] = useState<
    "overview" | "streaks" | "quests" | "badges"
  >("overview");

  const { data: streak, refetch: refetchStreak } =
    trpc.audienceLockIn.getStreak.useQuery(undefined, { enabled: !!user });
  const { data: loyalty } = trpc.audienceLockIn.getLoyaltyProfile.useQuery(
    undefined,
    { enabled: !!user }
  );
  const { data: badges } = trpc.audienceLockIn.getUserBadges.useQuery(
    undefined,
    { enabled: !!user }
  );
  const { data: quests, refetch: refetchQuests } =
    trpc.audienceLockIn.getActiveQuests.useQuery(undefined, {
      enabled: !!user,
    });
  const { data: fanLevel } = trpc.audienceLockIn.getFanLevel.useQuery(
    undefined,
    { enabled: !!user }
  );
  ...
  const recordActivity = trpc.audienceLockIn.recordActivity.useMutation({
    onSuccess: (data: any) => {
      refetchStreak();
      if (data?.increased)
        toast.success(`🔥 Streak extended! Day ${data.streak}`, {
          description: data.reward || `Multiplier: ${data.multiplier}x`,
        });
      else if (data?.broken)
      ...
      <div className="border-b border-border/50 bg-card/50 backdrop-blur-sm sticky top-0 z-10">
        <div className="container max-w-4xl py-4">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-bold flex items-center gap-2">
                  <Heart className="w-5 h-5 text-primary" /> Retention & Loyalty
                </h1>
            ...
            <Button
              size="sm"
              onClick={() => recordActivity.mutate()}
              disabled={recordActivity.isPending}
              className="bg-primary hover:bg-primary/90"
            >
              <Zap className="w-4 h-4 mr-1" />
              {recordActivity.isPending ? "Recording..." : "Check In"}
            </Button>
          </div>
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                },
                {
                  icon: Star,
                  color: tierColor,
                  value: tierName.charAt(0).toUpperCase() + tierName.slice(1),
                  label: "Loyalty Tier",
                },
                {
                  icon: Zap,
            ...
            <div className={`card p-5 bg-gradient-to-r ${tierBg} border`}>
              <div className="flex items-center justify-between mb-3">
                <div>
                  <div className={`text-lg font-bold capitalize ${tierColor}`}>
                    {rankName}
                  </div>
                  <div className="text-sm text-muted-foreground">
                    {loyaltyPoints.toLocaleString()} lifetime points
                  </div>
                </div>
              ...
              <Progress
                value={Math.min(100, (xp / xpToNext) * 100)}
                className="h-3"
              />
            </div>
            <div className="card p-5">
              <h3 className="font-semibold mb-4 flex items-center gap-2">
                <Gift className="w-4 h-4 text-primary" /> Your{" "}
                {tierName.charAt(0).toUpperCase() + tierName.slice(1)} Benefits
              </h3>
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  },
                  {
                    ...
                    value: (streak as any)?.freezesRemaining ?? 3,
                    icon: Shield,
                  },
                  {
                  ...
                  >
                    ...
                    <div>
                      ...
                      <div className="font-semibold text-sm">{b.value}</div>
                    </div>
                  </div>
                ))}
              </div>
            ...
            <div className="card p-5">
              <h3 className="font-semibold mb-4 flex items-center gap-2">
                <TrendingUp className="w-4 h-4 text-primary" /> Tier Progression
              </h3>
              <div className="space-y-2">
                {(
                  ...
                  return (
                    <div
                      key={tier}
                      className={`flex items-center justify-between p-3 rounded-lg border transition-all ${isActive ? "border-primary/50 bg-primary/10" : isUnlocked ? "border-green-500/30 bg-green-500/5" : "border-border/30 opacity-50"}`}
                    >
                      <div className="flex items-center gap-3">
                        {isUnlocked ? (
                        ...
                        <span
                          className={`font-medium capitalize ${TIER_COLORS[tier]}`}
                        >
                          {tier}
                        </span>
                        {isActive && (
          ...
          <div className="space-y-6">
            ...
            <div className="card p-5">
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div
                    key={m.days}
                    className={`flex items-center justify-between p-3 rounded-lg border ${currentStreak >= m.days ? "border-green-500/40 bg-green-500/10" : "border-border/30"}`}
                  >
                    <div className="flex items-center gap-3">
            ...
            <div className="card p-5">
              <h3 className="font-semibold mb-3 flex items-center gap-2">
                <Shield className="w-4 h-4 text-primary" /> Streak Shields
              </h3>
              ...
              <div className="flex items-center gap-3">
                {Array.from({ length: 3 }).map((_, i) => (
                  <div
                    key={i}
                    className={`w-12 h-12 rounded-full border-2 flex items-center justify-center ${i < ((streak as any)?.freezesRemaining ?? 3) ? "border-primary bg-primary/20" : "border-border/30 opacity-30"}`}
                  >
                    <Shield className="w-5 h-5 text-primary" />
                  </div>
                ))}
                <div className="text-sm text-muted-foreground ml-2">
                  {(streak as any)?.freezesRemaining ?? 3} shields remaining
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="font-semibold flex items-center gap-2">
                <Target className="w-4 h-4 text-primary" /> Active Quests
              </h2>
              <Button
                variant="outline"
                size="sm"
                  ...
                  <div className="flex items-start justify-between gap-3">
                    ...
                    <div className="text-right shrink-0">
                      <div className="text-sm font-bold text-primary">
                        +{quest.xpReward ?? quest.reward ?? 0} XP
                      </div>
                      {quest.skyReward && (
                        <div className="text-xs text-yellow-400">
                          {quest.skyReward} SKY444
                        </div>
                      )}
                    </div>
                  </div>
            ...
            <div className="card p-5 border-dashed">
              ...
              <div className="space-y-2">
                {[
                  ...
                  <div
                    key={i}
                    className={`flex items-center justify-between p-2.5 rounded-lg ${q.done ? "opacity-50" : ""}`}
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`text-sm ${q.done ? "line-through text-muted-foreground" : ""}`}
                      >
                        {q.title}
                      </span>
                    </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Complete quests, maintain streaks, and level up your loyalty
                tier.
              </p>
            </div>
            <Link href="/quests">
              <Button size="sm" className="shrink-0">
                View Quests <ArrowRight className="w-3 h-3 ml-1" />
              </Button>
            </Link>
          </div>
        </div>
