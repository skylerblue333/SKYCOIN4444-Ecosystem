...
export default function SchoolCourse() {
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      {/* Hero */}
      <div
        className={`bg-gradient-to-br ${course.color} border-b border-border/50`}
      >
          ...
          <div className="lg:col-span-2">
            ...
            <div className="rounded-xl bg-zinc-900 border border-border/50 aspect-video flex items-center justify-center mb-6 relative overflow-hidden">
              ...
              <div className="text-center relative z-10">
                ...
                <p className="text-sm text-muted-foreground">
                  Preview: {course.sections?.[0]?.lessons?.[0]?.title}
                </p>
              </div>
            </div>
            ...
            <div className="flex gap-1 border-b border-border/50 mb-6">
              {(
                ["overview", "curriculum", "instructor", "reviews"] as const
              ).map(tab => (
                <button
                  key={tab}
                  onClick={() => setActiveTab(tab)}
                  className={`px-4 py-2.5 text-sm font-medium border-b-2 transition-all -mb-px capitalize ${activeTab === tab ? "border-primary text-primary" : "border-transparent text-muted-foreground hover:text-foreground"}`}
                >
                  {tab}
                </button>
              ))}
              ...
              <div className="space-y-3">
                {course.sections?.map((section: any, si: number) => (
                  ...
                  >
                    <button
                      onClick={() =>
                        setExpandedSection(expandedSection === si ? -1 : si)
                      }
                    ...
                    >
                      ...
                      <ChevronRight
                        className={`h-4 w-4 text-muted-foreground transition-transform ${expandedSection === si ? "rotate-90" : ""}`}
                      />
                    </button>
                    {expandedSection === si && (
                      <div className="divide-y divide-border/30">
                        {section.lessons.map((lesson: any, li: number) => (
                          <div
                            key={li}
                            className="flex items-center gap-3 p-3 hover:bg-card/30 transition-colors"
                          >
                            {lesson.type === "video" ? (
            ...
            {activeTab === "instructor" && (
              <div className="rounded-xl border border-border/50 bg-card/30 p-6">
                <div className="flex items-center gap-4 mb-4">
                  <div className="w-16 h-16 rounded-full bg-primary/20 border-2 border-primary/30 flex items-center justify-center text-xl font-bold">
                    {course.instructor
                      ?.split(" ")
                      .map((n: string) => n[0])
                      .join("")}
            ...
            <div className="sticky top-20 rounded-xl border border-border/50 bg-card/50 p-6 shadow-lg">
              <div className="text-3xl font-bold mb-1 text-center">
                {course.price === 0 ? (
                  ...
                  <span className="text-primary">{course.price} SKY444</span>
                )}
              </div>
