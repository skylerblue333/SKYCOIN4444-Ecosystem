        ...
        <TabsContent value="agents" className="mt-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {AGENTS.map(agent => (
              <Card
                key={agent.id}
                className={`glass-card border cursor-pointer hover:scale-[1.01] transition-transform ${colorMap[agent.color].split(" ").slice(1).join(" ")}`}
              >
                <CardContent className="p-4">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-3">
                      <div
                        className={`w-9 h-9 rounded-lg flex items-center justify-center ${colorMap[agent.color].split(" ").slice(1).join(" ")}`}
                      >
                        <Bot
                          className={`w-4 h-4 ${colorMap[agent.color].split(" ")[0]}`}
                        />
                      </div>
                      <div>
                    ...
                    <div className="flex items-center gap-2">
                      <Badge
                        className={`text-xs border ${statusBadge[agent.status]}`}
                      >
                        {agent.status}
                      </Badge>
              ...
              <CardContent className="p-4">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    <Badge
                      variant="outline"
                      className={`text-xs border ${task.priority === "critical" ? "border-red-500/30 text-red-300" : task.priority === "high" ? "border-orange-500/30 text-orange-300" : "border-white/10 text-white/40"}`}
                    >
                      {task.priority}
                    </Badge>
                    <Badge
                      className={`text-xs border ${statusBadge[task.status]}`}
                    >
                      {task.status}
                    </Badge>
                    <span className="text-xs font-mono text-white/30">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {AGENTS.map(agent => (
                  <div key={agent.id} className="text-center">
                    ...
                    <div
                      className={`p-3 rounded-xl border ${colorMap[agent.color].split(" ").slice(1).join(" ")} text-center`}
                    >
                      <Bot
                        className={`w-5 h-5 ${colorMap[agent.color].split(" ")[0]} mx-auto mb-1`}
                      />
                      <div className="text-xs font-semibold">{agent.name}</div>
                      <div
                        className={`text-xs mt-1 ${agent.status === "active" ? "text-green-400" : "text-white/30"}`}
                      >
