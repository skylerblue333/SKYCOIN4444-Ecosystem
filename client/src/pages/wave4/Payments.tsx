...
export default function PaymentsPage() {
  ...
  const { data: history, isLoading: historyLoading } =
    trpc.wave4Payments.getBillingHistory.useQuery({
      limit: 20,
    });
  const { data: subscriptions } =
    trpc.wave4Payments.getSubscriptions.useQuery();
  const { data: payouts } = trpc.wave4Payments.getPayouts.useQuery({
    limit: 20,
  });
                ...
                {methods.map((m: any) => (
                  ...
                  >
                    <div>
                      ...
                      {m.isDefault && (
                        <p className="text-xs text-blue-600 mt-1">Default</p>
                      )}
                    </div>
