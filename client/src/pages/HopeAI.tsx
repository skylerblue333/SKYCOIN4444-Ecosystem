        ...
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div className="flex items-center gap-3">
              <div className="relative">
                <div
                  className="w-10 h-10 rounded-full flex items-center justify-center text-base font-black shadow-lg"
                  style={{
                    background:
                      "linear-gradient(135deg, oklch(0.65 0.30 305), oklch(0.65 0.28 200))",
                    boxShadow: "0 0 20px oklch(0.65 0.30 305 / 0.4)",
                  }}
                >
                  H
                </div>
              ...
              <div>
                <div className="font-black text-sm text-rainbow-fast">
                  Hope AI
                </div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span className={`text-xs font-bold capitalize ${sm.color}`}>
                {currentState}
              </span>
            </div>
            {/* Tone indicator */}
            {selectedTone !== "auto" && (
              <div
                className="flex items-center gap-1.5 rounded-full px-3 py-1.5 border border-fuchsia-500/20"
                style={{ background: "oklch(0.12 0.04 290)" }}
              >
                <span className="text-sm">{TONE_META[selectedTone].emoji}</span>
                <span
                  className={`text-xs font-bold ${TONE_META[selectedTone].color}`}
                >
                  {TONE_META[selectedTone].label}
                </span>
              </div>
            )}
            <Button
              ...
              className="text-xs border-fuchsia-500/30 bg-fuchsia-500/10 hover:bg-fuchsia-500/20 text-fuchsia-300"
            >
              ...
              {activeTones.map(tone => {
                ...
                return (
                  ...
                  >
                    ...
                    <span className={`${tm.color} font-medium`}>
                      {tm.label}
                    </span>
                  </button>
                );
        ...
        <div className="fixed right-4 top-24 w-72 z-30 bg-[#0a0a1a]/95 border border-red-500/20 rounded-2xl p-4 space-y-3 backdrop-blur-xl shadow-2xl shadow-red-500/5 max-h-[calc(100vh-120px)] overflow-y-auto">
          ...
          {dominantSignal && (
            <div className="px-3 py-2 rounded-lg bg-red-500/5 border border-red-500/10">
              <p className="text-[10px] text-red-400/50 uppercase tracking-widest mb-0.5">
                Dominant Signal
              </p>
          ...
          <div className="space-y-1.5">
            ...
            {graySignals.length === 0 ? (
              ...
              graySignals.map((sig, i) => (
                <div
                  key={i}
                  className={`px-2.5 py-1.5 rounded-lg border text-[10px] ${SEVERITY_COLORS[sig.severity] || SEVERITY_COLORS.low}`}
                >
                  <div className="flex justify-between">
                    <span className="font-medium">{sig.feature}</span>
                    <span className="opacity-60">{sig.score}</span>
                  </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-2 gap-1">
              {[
                ...
                <div
                  key={i}
                  className="text-[9px] text-white/20 px-1.5 py-0.5 rounded bg-white/2 border border-white/3"
                >
                  {n}
                </div>
              ))}
        ...
        <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
            ...
            >
              {msg.role === "assistant" && (
                <div className="flex gap-3 max-w-[85%]">
                  <div className="w-8 h-8 rounded-full bg-gradient-to-br from-fuchsia-500 to-cyan-500 flex-shrink-0 flex items-center justify-center text-xs font-bold mt-1">
                    H
                  </div>
                  <div className="space-y-2">
                    ...
                    {(msg.tone || msg.state) && (
                      <div className="flex items-center gap-2 flex-wrap">
                        {msg.tone && (
                          <span
                            className={`text-xs ${TONE_META[msg.tone]?.color || "text-white/40"}`}
                          >
                            {TONE_META[msg.tone]?.emoji}{" "}
                            {TONE_META[msg.tone]?.label}
                          </span>
                        )}
                        {msg.state && (
                          <span
                            className={`text-xs ${STATE_META[msg.state]?.color || "text-white/40"}`}
                          >
                            {STATE_META[msg.state]?.emoji} {msg.state}
                          </span>
                        )}
                      </div>
                    )}
                    ...
                    {msg.innerThought && (
                      <div className="text-xs text-white/20 italic pl-1">
                        💭 {msg.innerThought}
                      </div>
                    )}
                    {/* Follow-up prompts */}
          ...
          {isLoading && (
            <div className="flex gap-3">
              <div className="w-8 h-8 rounded-full bg-gradient-to-br from-fuchsia-500 to-cyan-500 flex-shrink-0 flex items-center justify-center text-xs font-bold">
                H
              </div>
              <div className="bg-white/5 border border-white/10 rounded-2xl rounded-tl-sm px-4 py-3">
