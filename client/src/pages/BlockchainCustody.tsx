...
function SendDialog({
  ...
  const estimateQuery = trpc.blockchain.estimateGas.useQuery(
    {
      from: wallet?.address ?? "",
      to: to.length >= 42 ? to : (wallet?.address ?? ""),
      valueWei: amount ? String(Math.floor(parseFloat(amount) * 1e18)) : "0",
  ...
  const valueWei = amount
    ? String(Math.floor(parseFloat(amount || "0") * 1e18))
    : "0";
        ...
        {step === "form" && (
          <div className="space-y-4">
            ...
            <div>
              ...
              {to.length >= 42 && (
                <p
                  className={`text-xs mt-1 ${isValidAddress ? "text-emerald-400" : "text-red-400"}`}
                >
                  {isValidAddress
              ...
              <div className="bg-zinc-800/60 rounded-lg p-3 space-y-1.5">
                <div className="flex justify-between text-xs">
                  ...
                  <span className="text-zinc-300">
                    {estimateQuery.data.estimatedCostEth} ETH
                  </span>
                </div>
                <div className="flex justify-between text-xs">
                  <span className="text-zinc-500">Max Fee/Gas</span>
                  <span className="text-zinc-300">
                    {(Number(estimateQuery.data.maxFeePerGas) / 1e9).toFixed(2)}{" "}
                    Gwei
                  </span>
                </div>
              </div>
            ...
            <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-4">
              ...
              <div className="space-y-2 text-xs">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="text-zinc-300">
                    {weiToEth(signedTx.estimatedGasCost)} ETH
                  </span>
                </div>
                <div className="flex justify-between">
                  <span className="text-zinc-500">Tx Hash</span>
                  <span className="text-zinc-400 font-mono">
                    {truncateAddress(signedTx.txHash)}
                  </span>
                </div>
              </div>
            </div>
            <p className="text-xs text-zinc-500">
              This transaction will be broadcast to the {wallet?.chainName}{" "}
        ...
        {step === "broadcasting" && (
          <div className="flex flex-col items-center py-8 gap-4">
            ...
            <p className="text-white font-semibold">
              Broadcasting to {wallet?.chainName}…
            </p>
            <p className="text-xs text-zinc-500">
...
function TxRow({ tx }: { tx: Record<string, unknown> }) {
  ...
  return (
    <div className="flex items-center gap-3 py-3 border-b border-zinc-800/60 last:border-0">
      <div
        className={`w-8 h-8 rounded-full flex items-center justify-center ${cfg.color} bg-zinc-800`}
      >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <span className="text-xs text-zinc-400 font-mono">
            {truncateAddress(tx.toAddress as string)}
          </span>
          {txHash && explorer && (
            <a
...
export default function BlockchainCustody() {
  ...
  const [sendWallet, setSendWallet] =
    useState<Parameters<typeof SendDialog>[0]["wallet"]>(null);
  const [registerChain, setRegisterChain] = useState<
    "ethereum" | "polygon" | "bsc" | "base"
  >("ethereum");

  const utils = trpc.useUtils();

  const walletsQuery = trpc.blockchain.myWallets.useQuery(undefined, {
    enabled: !!user,
  });
  const txQuery = trpc.blockchain.myTransactions.useQuery(
    { limit: 50 },
    { enabled: !!user }
  );
  ...
  const stats = useMemo(
    () => ({
      ...
      pendingTxs: txs.filter(t => ["broadcast", "signed"].includes(t.status))
        .length,
    ...
    return (
      <div className="min-h-screen bg-zinc-950 flex flex-col items-center justify-center gap-4">
        ...
        <p className="text-zinc-400 text-sm">
          Sign in to access your blockchain wallet
        </p>
        <Link href="/">
          <Button className="bg-amber-500 text-black">Go Home</Button>
        </Link>
      </div>
    );
  }
        ...
        <div className="bg-zinc-900/40 border border-zinc-800 rounded-xl p-4 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-400 leading-relaxed">
              Your wallet addresses are derived from a BIP-44 HD key tree.
              Private keys are{" "}
              <strong className="text-white">never stored</strong> in the
              database — they are derived ephemerally at signing time and
              immediately discarded. Only your public address and derivation
              path are recorded.
            </p>
          </div>
        </div>
            ...
            <Card className="bg-zinc-900/60 border-zinc-800">
              ...
              <CardContent>
                {txQuery.isLoading ? (
                  ...
                  <div>
                    {txs.map(tx => (
                      <TxRow key={tx.id} tx={tx as Record<string, unknown>} />
                    ))}
                  </div>
                )}
              </CardContent>
              ...
              <CardContent className="space-y-4">
                <div>
                  ...
                  <Select
                    value={registerChain}
                    onValueChange={v =>
                      setRegisterChain(v as typeof registerChain)
                    }
                  >
                ...
                <div className="bg-zinc-800/60 rounded-lg p-3 text-xs text-zinc-400 space-y-1">
                  <p>
                    ...
                    <code className="text-zinc-300">m/44'/60'/0'/0/userId</code>
                  </p>
                  <p>
                ...
                <Button
                  onClick={() =>
                    registerMutation.mutate({ chain: registerChain })
                  }
