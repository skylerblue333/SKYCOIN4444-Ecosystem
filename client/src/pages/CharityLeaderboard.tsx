...
export default function CharityLeaderboard() {
  ...
  const { data: stats, isLoading: statsLoading } =
    trpc.charity.stats.useQuery();
  const { data: campaigns, isLoading: campaignsLoading } =
    trpc.charity.campaigns.useQuery({ limit: 10 });
  const { data: leaderboard, isLoading: leaderboardLoading } =
    trpc.charity.leaderboard.useQuery({ limit: 20 });
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div>
          <h3 className="font-bold mb-4 flex items-center gap-2">
            <Trophy className="w-4 h-4 text-yellow-400" /> Top Donors
          </h3>
          {leaderboardLoading ? (
            ...
            <div className="space-y-3">
              {(leaderboard as any[]).map((d: any, i: number) => (
                <Card
                  key={d.id ?? i}
                  className={`p-4 border-border/50 flex items-center gap-3 ${i < 3 ? "border-yellow-500/20 bg-yellow-500/5" : "bg-card/60"}`}
                >
                  <span
                    className={`w-8 h-8 rounded-full flex items-center justify-center font-bold text-sm flex-shrink-0 ${i === 0 ? "bg-yellow-500/20 text-yellow-400" : i === 1 ? "bg-gray-400/20 text-gray-400" : i === 2 ? "bg-orange-500/20 text-orange-400" : "bg-muted text-muted-foreground"}`}
                  >
                    {i + 1}
                  </span>
                  <div className="flex-1 min-w-0">
                    <p className="font-semibold text-sm truncate">
                      {d.isAnonymous
                        ? "Anonymous"
                        : (d.donorName ?? d.username ?? `User #${d.donorId}`)}
                    </p>
                    <p className="text-xs text-muted-foreground">
                      {d.campaignCount ?? 1} campaign
                      {(d.campaignCount ?? 1) !== 1 ? "s" : ""}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-sm font-bold text-purple-400">
                      {Number(d.totalDonated ?? d.amount ?? 0).toLocaleString()}{" "}
                      SKY444
                    </p>
                    <Badge
                      variant="outline"
        ...
        <div>
          <h3 className="font-bold mb-4 flex items-center gap-2">
            <TrendingUp className="w-4 h-4 text-primary" /> Active Campaigns
          </h3>
          {campaignsLoading ? (
            <div className="flex justify-center py-8">
              <Loader2 className="w-6 h-6 animate-spin text-muted-foreground" />
