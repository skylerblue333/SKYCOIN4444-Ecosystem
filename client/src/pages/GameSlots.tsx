...
function checkWin(
  ...
  for (let row = 0; row < ROWS; row++) {
    ...
    for (let col = 0; col <= REELS - 3; col++) {
      if (
        rowSymbols[col] === rowSymbols[col + 1] &&
        rowSymbols[col + 1] === rowSymbols[col + 2]
      ) {
    ...
    const animInterval = setInterval(() => {
      ...
      if (frames >= maxFrames) {
        ...
        if (winAmount > 0) {
          ...
          if (winAmount >= betAmt * 25)
            toast.success(`🎰 JACKPOT! +${winAmount} SKY444!`);
          else if (winAmount >= betAmt * 5)
            toast.success(`⭐ Big Win! +${winAmount} SKY444!`);
        ...
        <div className="bg-gradient-to-b from-[#1a0a2e] to-[#0e0a1a] border border-purple-500/20 rounded-3xl p-6 mb-4 shadow-2xl shadow-purple-900/20">
          ...
          <div className="bg-black/40 rounded-2xl p-4 mb-4 border border-white/5">
            {displayGrid.map((row, rowIdx) => (
              <div
                key={rowIdx}
                className={`flex justify-around mb-2 last:mb-0 rounded-xl py-2 transition-all duration-200 ${winLines.includes(rowIdx) ? "bg-yellow-500/10 border border-yellow-500/30" : ""}`}
              >
                {row.map((symbol, colIdx) => (
                  <div
                    key={colIdx}
                    className={`text-4xl transition-all duration-100 ${spinning ? "blur-sm scale-110" : "blur-0 scale-100"}`}
                  >
                    {symbol}
                  </div>
                ))}
            ...
            <div className="flex gap-2">
              {[5, 10, 25, 50, 100].map(amt => (
                <Button
                  ...
                  size="sm"
                  variant="outline"
                  className="border-white/10 text-slate-400 text-xs px-2"
        ...
        <div className="bg-[#0e0a1a] border border-white/5 rounded-2xl p-4">
          ...
          <div className="grid grid-cols-2 gap-2 mb-4">
            {SYMBOLS.map(sym => (
              <div
                key={sym}
                className="flex items-center justify-between bg-white/3 rounded-lg px-3 py-1.5 text-xs"
              >
                <span className="text-xl">{sym}</span>
                <span className="text-slate-400">
                  {SYMBOL_VALUES[sym]}x bet
                </span>
              </div>
            ))}
          </div>
          <div className="space-y-1">
            {PAYLINES.map((pl, i) => (
              ...
              >
                <span>{pl.name}</span>
                <span className="text-slate-600">{pl.mult}</span>
              </div>
            ))}
          </div>
          <div className="mt-3 p-3 bg-yellow-500/5 border border-yellow-500/20 rounded-lg flex items-start gap-2">
            ...
            <p className="text-[10px] text-yellow-600">
              RTP ~96%. Play responsibly. SKY444 is a platform token, not real
              currency.
            </p>
          </div>
        </div>
