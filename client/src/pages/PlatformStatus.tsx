  ...
  return (
    <div className="container py-8 max-w-4xl animate-page-in">
      <PageHeader
        ...
        subtitle="Real-time health and uptime for all SKYCOIN4444 services"
      />

      {/* Overall Status Banner */}
      <div
        className={`rounded-xl p-4 mb-8 flex items-center gap-3 border ${allOperational ? "bg-success/10 border-success/30" : "bg-warning/10 border-warning/30"}`}
      >
        {allOperational ? (
        ...
        <div>
          <div
            className={`font-semibold ${allOperational ? "text-success" : "text-warning"}`}
          >
            {allOperational
      ...
      <div className="card mb-6 overflow-hidden">
        ...
        {SERVICES.map((s, i) => (
          ...
          >
            ...
            <div
              className={`text-xs font-semibold ${s.status === "operational" ? "text-success" : "text-warning"}`}
            >
              {s.uptime}
            </div>
            <div
      ...
      <div className="card p-5">
        ...
        {INCIDENTS.map((inc, i) => (
          ...
          >
            <div
              className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${inc.status === "resolved" ? "bg-success" : "bg-warning"}`}
            />
            ...
            <div
              className={`text-xs px-2 py-0.5 rounded-full ${inc.status === "resolved" ? "bg-success/10 text-success" : "bg-warning/10 text-warning"}`}
            >
              {inc.status}
            </div>
          </div>
