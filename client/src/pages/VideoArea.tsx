      ...
      >
        ...
        <div className="absolute top-4 left-4 right-4 flex items-center justify-between">
          <div className="flex gap-1">
            {reel.tags.map(tag => (
              <Badge
                key={tag}
                className="bg-black/40 text-white border-white/20 text-[10px] backdrop-blur-sm"
              >
                #{tag}
              </Badge>
            ))}
        ...
        <div className="absolute bottom-4 left-4 right-16">
          <div className="flex items-center gap-2 mb-2">
            ...
            <Button
              size="sm"
              className="h-6 text-xs bg-purple-500/80 hover:bg-purple-500 text-white px-2 py-0"
            >
              Follow
            </Button>
          </div>
        ...
        <div className="absolute right-2 bottom-4 flex flex-col items-center gap-4">
          ...
          >
            <div
              className={`w-10 h-10 rounded-full flex items-center justify-center ${liked.has(reel.id) ? "bg-red-500" : "bg-black/40 backdrop-blur-sm"}`}
            >
              <Heart
                className={`w-5 h-5 ${liked.has(reel.id) ? "fill-white text-white" : "text-white"}`}
              />
            </div>
            <span className="text-white text-xs">
              {((reel.likes + (liked.has(reel.id) ? 1 : 0)) / 1000).toFixed(1)}K
            </span>
          </button>
          ...
          >
            <div
              className={`w-10 h-10 rounded-full flex items-center justify-center ${saved.has(reel.id) ? "bg-yellow-500" : "bg-black/40 backdrop-blur-sm"}`}
            >
              <Bookmark
                className={`w-5 h-5 ${saved.has(reel.id) ? "fill-white text-white" : "text-white"}`}
              />
            </div>
        ...
        <button
          onClick={prev}
          disabled={current === 0}
          className="absolute left-1/2 -translate-x-1/2 top-2 w-8 h-8 rounded-full bg-black/40 backdrop-blur-sm flex items-center justify-center text-white disabled:opacity-30"
        >
        ...
        <button
          ...
          className="absolute left-1/2 -translate-x-1/2 bottom-2 w-8 h-8 rounded-full bg-black/40 backdrop-blur-sm flex items-center justify-center text-white disabled:opacity-30"
        >
      ...
      <div className="absolute right-4 top-1/2 -translate-y-1/2 flex flex-col gap-1.5">
        {reels.map((_, i) => (
          <button
            ...
            className={`w-1.5 rounded-full transition-all ${i === current ? "h-6 bg-white" : "h-1.5 bg-white/30"}`}
          />
        ))}
      </div>
    </div>
    ...
    <div className="min-h-screen bg-[#07050f] text-white">
      ...
      <div className="sticky top-0 z-30 bg-[#07050f]/95 backdrop-blur-xl border-b border-white/5 px-4 py-3">
        <div className="max-w-6xl mx-auto flex items-center gap-3">
          <h1 className="text-xl font-black bg-gradient-to-r from-purple-400 to-cyan-400 bg-clip-text text-transparent whitespace-nowrap">
            SKY VIDEO
          </h1>
          ...
          <Button
            size="sm"
            className="bg-purple-500/20 hover:bg-purple-500/30 text-purple-300 border border-purple-500/30 shrink-0"
        ...
        <Tabs value={tab} onValueChange={setTab}>
          <TabsList className="bg-white/5 border border-white/10 mb-6">
            ...
            >
              ...
              Live
              <Badge className="ml-1.5 bg-red-500 text-white text-[9px] px-1 py-0">
                {LIVE_STREAMS.length}
              </Badge>
            </TabsTrigger>
            <TabsTrigger
              ...
              {filtered.map(video => (
                ...
                >
                  <div className="relative rounded-xl overflow-hidden mb-2 bg-black aspect-video">
                    ...
                    <div className="absolute bottom-2 right-2 bg-black/70 rounded px-1.5 py-0.5 text-white text-[10px] font-mono">
                      {video.duration}
                    </div>
                    ...
                    <Badge className="absolute top-2 left-2 bg-purple-500/80 text-white border-0 text-[10px]">
                      {video.category}
                    </Badge>
                  </div>
            ...
            <div className="space-y-4">
              <div className="p-3 bg-red-500/10 border border-red-500/20 rounded-xl flex items-center gap-2">
                ...
                <span className="text-red-400 text-sm font-semibold">
                  {LIVE_STREAMS.length} streams live now
                </span>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {LIVE_STREAMS.map(stream => (
                  <div
                    key={stream.id}
                    className="group cursor-pointer bg-[#0e0a1a] border border-white/5 hover:border-red-500/30 rounded-xl overflow-hidden transition-all"
                    onClick={() => toast.info("Opening stream...")}
                  >
                    ...
                    <div className="p-3">
                      <h3 className="text-white text-sm font-medium group-hover:text-red-300 transition-colors">
                        {stream.title}
                      </h3>
            ...
            <div className="space-y-3">
              {MOCK_VIDEOS.sort((a, b) => b.likes - a.likes).map((video, i) => (
                ...
                >
                  ...
                  <Badge className="shrink-0 self-start bg-purple-500/20 text-purple-400 border-purple-500/30 text-[10px]">
                    {video.category}
                  </Badge>
                </div>
              ))}
