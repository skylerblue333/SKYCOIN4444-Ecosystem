...
const ACTION_BILLING_FLOW = [
  { step: "User Request", desc: "User submits action via chat", icon: Zap },
  {
    ...
    desc: "AI calculates execution cost",
    icon: DollarSign,
  },
  {
    step: "Payment Check",
    desc: "Verify wallet balance or card",
    icon: CreditCard,
  },
  {
    step: "Deduct Credits",
    desc: "Atomic deduction from balance",
    icon: ArrowDownLeft,
  },
  { step: "Execute Action", desc: "AI performs the task", icon: RefreshCw },
  { step: "Return Result", desc: "Deliver output to user", icon: CheckCircle },
];
...
export default function PaymentInfra() {
  const [activeTab, setActiveTab] = useState<
    "overview" | "billing" | "transactions" | "systems"
  >("overview");
    ...
    <div className="min-h-screen bg-background">
      <div className="sticky top-0 z-10 bg-background/80 backdrop-blur-sm border-b border-border/50 px-4 py-3 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Phase 31 — Real-time AI execution economy
          </p>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto p-4 space-y-4">
        ...
        <div className="flex gap-1 bg-secondary/30 rounded-xl p-1">
          {(["overview", "billing", "transactions", "systems"] as const).map(
            t => (
              <button
                key={t}
          ...
          <div className="space-y-3">
              ...
              <div className="flex items-center gap-1 flex-wrap text-xs">
                {[
                  ...
                  <span key={step} className="flex items-center gap-1">
                    <span
                      className={`px-2 py-1 rounded ${i === 3 ? "bg-green-500/20 text-green-400 font-medium" : "bg-secondary/50 text-muted-foreground"}`}
                    >
                      {step}
                    </span>
                    {i < arr.length - 1 && (
            ...
            <div className="card p-4">
              ...
              <div className="font-mono text-sm bg-secondary/30 rounded-lg p-3">
                ...
                <span className="text-red-400">AI Compute</span> -{" "}
                <span className="text-yellow-400">Infra Cost</span>
              </div>
              <p className="text-xs text-muted-foreground mt-2">
                Current margin: 92.9% — AI costs tracked per action to ensure
        ...
        {activeTab === "billing" && (
          <div className="space-y-3">
            <p className="text-xs text-muted-foreground">
              Every AI action goes through this billing pipeline before
              execution.
            </p>
            {ACTION_BILLING_FLOW.map((step, i) => (
              <div key={i} className="card p-3 flex items-center gap-3">
                ...
                <div className="text-xs text-muted-foreground">
                  {i + 1}/{ACTION_BILLING_FLOW.length}
                </div>
              </div>
            ))}
            ...
            {TRANSACTIONS.map(tx => (
              <div key={tx.id} className="card p-3 flex items-center gap-3">
                ...
                <div className="text-right">
                  <div
                    className={`text-sm font-bold ${tx.type === "credit" ? "text-green-400" : tx.type === "debit" ? "text-red-400" : "text-yellow-400"}`}
                  >
                    {tx.type === "credit"
                      ? "+"
                      : tx.type === "debit"
            ...
            {PAYMENT_SYSTEMS.map(sys => (
              <div key={sys.name} className="card p-4">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className={`text-xs font-medium ${sys.color}`}>
                    {sys.status}
                  </span>
                </div>
