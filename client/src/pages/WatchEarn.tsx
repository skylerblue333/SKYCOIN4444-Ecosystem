...
export default function WatchEarn() {
  ...
  const [currentPuzzle, setCurrentPuzzle] = useState<PuzzleQuestion | null>(
    null
  );
      ...
      intervalRef.current = setInterval(() => {
        setCurrentTime(t => {
          ...
          if (next >= selectedVideo.duration) {
            setIsPlaying(false);
            toast.success(
              `Video complete! +${Math.round((selectedVideo.duration / 60) * selectedVideo.xpPerMinute)} XP earned`
            );
        ...
        if (Math.random() < 0.33) {
          ...
          const skyGain = parseFloat(
            (selectedVideo.skyPerMinute * streak * 0.1).toFixed(4)
          );
  ...
  useEffect(() => {
    if (watchedMinutes >= nextPuzzleAt && isPlaying && !showPuzzle) {
      const q =
        PUZZLE_QUESTIONS[Math.floor(Math.random() * PUZZLE_QUESTIONS.length)];
      setCurrentPuzzle(q);
      setShowPuzzle(true);
  ...
  const handlePuzzleAnswer = (idx: number) => {
    ...
    if (idx === currentPuzzle.correct) {
      setSessionSky(s =>
        parseFloat((s + currentPuzzle.reward * 0.01).toFixed(4))
      );
      setSessionXp(x => x + currentPuzzle.reward);
      setStreak(s => Math.min(s + 0.5, 5));
      toast.success(
        `Correct! +${currentPuzzle.reward} XP +${(currentPuzzle.reward * 0.01).toFixed(2)} SKY444 🎉`
      );
    } else {
      toast.error(
        `Wrong! The answer was: ${currentPuzzle.options[currentPuzzle.correct]}`
      );
    }
    setTimeout(() => {
      setShowPuzzle(false);
  ...
  const progress =
    selectedVideo.duration > 0
      ? (currentTime / selectedVideo.duration) * 100
      : 0;
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-400">
                Earn SKY444 just by watching
              </p>
            </div>
          </div>
          <div className="flex items-center gap-4">
            <div
              className={`flex items-center gap-2 px-3 py-1.5 rounded-full bg-cyan-500/10 border border-cyan-500/30 transition-all ${earnAnimation ? "scale-110 border-cyan-400" : ""}`}
            >
              <Coins className="w-4 h-4 text-cyan-400" />
              <span className="text-sm font-bold text-cyan-400">
                {sessionSky.toFixed(3)} SKY
              </span>
            </div>
        ...
        <div className="lg:col-span-2 space-y-4">
            ...
            {isPlaying && (
              <div className="absolute top-4 right-4 flex items-center gap-2 px-3 py-1.5 rounded-full bg-purple-600/20 border border-purple-500/40 backdrop-blur-sm">
                ...
                <span className="text-xs text-cyan-400">
                  +{selectedVideo.skyPerMinute * streak}/min
                </span>
              </div>
            )}
          ...
          <div className="bg-[#111827] rounded-2xl p-5 border border-white/5">
            <div className="flex items-start justify-between gap-4">
              ...
              >
                <Heart
                  className={`w-4 h-4 ${likedVideos.has(selectedVideo.id) ? "fill-current" : ""}`}
                />
                <span className="text-sm">
                  {formatViews(
                    selectedVideo.likes +
                      (likedVideos.has(selectedVideo.id) ? 1 : 0)
                  )}
                </span>
              </button>
            </div>
            ...
            <div className="mt-4 grid grid-cols-3 gap-3">
              <div className="bg-cyan-500/10 border border-cyan-500/20 rounded-xl p-3 text-center">
                ...
                <div className="text-lg font-bold text-cyan-400">
                  {selectedVideo.skyPerMinute * streak}
                </div>
              ...
              <div className="bg-purple-500/10 border border-purple-500/20 rounded-xl p-3 text-center">
                ...
                <div className="text-lg font-bold text-purple-400">
                  {selectedVideo.xpPerMinute * streak}
                </div>
          ...
          <div className="bg-gradient-to-r from-cyan-500/10 to-purple-500/10 border border-white/10 rounded-2xl p-5">
            ...
            <div className="mt-3 text-xs text-gray-500">
              Total all-time: {totalSkyEarned.toFixed(3)} SKY · {totalXpEarned}{" "}
              XP
            </div>
          </div>
        </div>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-xs text-cyan-400 flex items-center gap-1">
                      ...
                      {video.skyPerMinute}/min
                    </span>
                    <span className="text-xs text-purple-400 flex items-center gap-1">
                      <Zap className="w-3 h-3" />
                      {video.xpPerMinute} XP
                    </span>
                  </div>
                </div>
              </div>
          ...
          <div className="bg-[#111827] border border-white/5 rounded-2xl p-4 mt-4">
            ...
            <div className="space-y-2 text-sm text-gray-400">
              <div className="flex items-start gap-2">
                <div className="w-5 h-5 rounded-full bg-cyan-500/20 text-cyan-400 flex items-center justify-center text-xs flex-shrink-0 mt-0.5">
                  1
                </div>
                <span>Watch any video — earn SKY444 every minute</span>
              </div>
              <div className="flex items-start gap-2">
                <div className="w-5 h-5 rounded-full bg-purple-500/20 text-purple-400 flex items-center justify-center text-xs flex-shrink-0 mt-0.5">
                  2
                </div>
                <span>Answer puzzle questions for bonus SKY444</span>
              </div>
              <div className="flex items-start gap-2">
                <div className="w-5 h-5 rounded-full bg-orange-500/20 text-orange-400 flex items-center justify-center text-xs flex-shrink-0 mt-0.5">
                  3
                </div>
                <span>Build a streak to multiply your earnings up to 5x</span>
              </div>
              <div className="flex items-start gap-2">
                <div className="w-5 h-5 rounded-full bg-purple-600/20 text-purple-400 flex items-center justify-center text-xs flex-shrink-0 mt-0.5">
                  4
                </div>
            ...
            <div className="flex items-center gap-3 mb-4">
              ...
              <div>
                ...
                <p className="text-sm text-gray-400">
                  Answer correctly for +{currentPuzzle.reward} XP & +
                  {(currentPuzzle.reward * 0.01).toFixed(2)} SKY444
                </p>
              </div>
            </div>
