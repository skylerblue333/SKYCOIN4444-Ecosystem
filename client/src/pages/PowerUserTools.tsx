...
export default function PowerUserTools() {
  ...
  const handleExport = async () => {
    ...
    a.href = url;
    a.download = `shadowchat-export-${Date.now()}.json`;
    a.click();
                    ...
                    >
                      ...
                      <div className="flex items-center gap-0.5">
                        {sc.keys.map((k, i) => (
                          <span
                            key={i}
                            className="px-1.5 py-0.5 bg-secondary border border-border rounded text-xs font-mono"
                          >
                            {k}
                          </span>
                        ))}
            ...
            {FEATURE_FLAGS.map(flag => (
              ...
              >
                <div>
                  ...
                  {!flag.enabled && (
                    <p className="text-xs text-yellow-400/70">Experimental</p>
                  )}
                </div>
                <button
                ...
                >
                  <div
                    className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform ${flags[flag.key] ? "translate-x-5" : "translate-x-0.5"}`}
                  />
                </button>
              </div>
            ))}
        ...
        <div className="bg-secondary/20 border border-border/50 rounded-xl overflow-hidden">
          ...
          <div className="px-4 pb-3">
            <button
              ...
              disabled={exporting === "running" || selected.size === 0}
            ...
            >
              ...
              {exporting === "running"
                ? "Preparing export…"
                : `Export ${selected.size > 0 ? `(${selected.size})` : ""} as JSON`}
            </button>
          </div>
        </div>
