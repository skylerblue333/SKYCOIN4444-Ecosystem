      ...
      <div className="max-w-4xl mx-auto px-4 py-8">
        ...
        <div className="space-y-3">
          {tasks.map(task => (
            <div
              key={task.id}
              className="rounded-2xl border border-slate-800/60 bg-slate-900/40 p-4 flex items-center gap-4"
            >
              <div
                className={`w-8 h-8 rounded-xl flex items-center justify-center shrink-0 ${task.status === "done" ? "bg-emerald-500/20" : task.status === "running" ? "bg-cyan-500/20" : "bg-slate-700/40"}`}
              >
                {task.status === "done" ? (
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="text-xs text-slate-500">
                  Agent: {task.agent} · {task.output}
                </div>
              </div>
              <div className="text-right shrink-0">
                <Badge
                  className={`text-xs border-0 ${task.status === "done" ? "bg-emerald-500/15 text-emerald-400" : task.status === "running" ? "bg-cyan-500/15 text-cyan-400" : "bg-slate-700/50 text-slate-400"}`}
                >
                  {task.status}
                </Badge>
