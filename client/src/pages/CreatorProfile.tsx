...
export default function CreatorProfile() {
  ...
  return (
    <div className="min-h-screen bg-[#050308] text-white">
      {/* Cover */}
      <div
        className={`h-40 md:h-56 bg-gradient-to-br ${creator.coverGradient} relative`}
      >
          ...
          <div className="mb-8">
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {creator.tiers.map(tier => (
                <div
                  key={tier.id}
                  className={`border rounded-xl p-4 ${TIER_COLORS[tier.name]}`}
                >
                  ...
                  <ul className="space-y-1 mb-4">
                    {tier.perks.map(perk => (
                      ...
                      >
                        <Star className="w-3 h-3 text-purple-400 mt-0.5 shrink-0" />
                        {perk}
                      </li>
                    ))}
                  </ul>
          ...
          <div className="mb-6 flex items-center gap-3 p-3 bg-green-900/20 border border-green-500/20 rounded-xl">
            ...
            <div>
              <p className="text-sm font-bold text-white">
                ...
                {creator.tiers.find(t => t.id === selectedTier)?.name} tier
              </p>
            ...
            <Link href="/subscriptions">
              <Button
                size="sm"
                variant="outline"
                className="ml-auto border-slate-700 text-slate-400 hover:text-white text-xs"
              >
                Manage
              </Button>
            </Link>
            ...
            >
              ...
              <div className={viewMode === "list" ? "flex-1 min-w-0" : "p-2.5"}>
                ...
                <div className="flex items-center gap-2 mt-1">
                  <Badge
                    className={`text-[9px] px-1 py-0 ${TIER_COLORS[item.tier]}`}
                  >
                    {item.tier}
                  </Badge>
