      ...
      <div className="border-b border-slate-800/60 bg-[#0d0d14]/90 backdrop-blur sticky top-0 z-20">
        <div className="max-w-7xl mx-auto px-4 py-4 flex items-center justify-between flex-wrap gap-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <Badge
              className={`text-xs border-0 ${online ? "bg-emerald-500/15 text-emerald-400" : "bg-rose-500/15 text-rose-400"}`}
            >
              {online ? (
            ...
            <Button
              size="sm"
              onClick={saveLayout}
              className={`text-xs ${saved ? "bg-emerald-600" : "bg-slate-700 hover:bg-slate-600"} text-white`}
            >
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 grid grid-cols-1 lg:grid-cols-4 gap-6">
        ...
        <div className="space-y-4">
          {[
            ...
            {
              label: "Sync Status",
              value: online ? "Live" : "Local",
              icon: online ? Wifi : WifiOff,
              color: online ? "text-emerald-400" : "text-rose-400",
            },
          ].map(s => (
            <div
              key={s.label}
              className="bg-slate-900/40 rounded-xl border border-slate-800/60 p-4"
            >
                ...
                <div key={t.id} className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1">
                    ...
                    <button
                      ...
                      className="text-slate-700 hover:text-rose-400 ml-1"
                    >
        ...
        <div className="lg:col-span-3">
          ...
          >
            {/* Stage */}
            <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-slate-700/60 border border-slate-600/60 rounded-lg px-12 py-3 text-xs text-slate-400 font-semibold tracking-widest uppercase">
              STAGE
            </div>
            {tables.map(t => (
              <div
              ...
              >
                <div
                  className={`h-2 w-2 rounded-full ${t.color} mx-auto mb-1`}
                />
            ...
            <div className="absolute bottom-3 right-3 text-xs text-slate-700">
              ...
              {online ? "Auto-syncing" : "Saved locally"}
            </div>
          </div>
        </div>
      </div>
