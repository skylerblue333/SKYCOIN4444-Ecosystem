    ...
    <div className="bg-gradient-to-br from-violet-900/40 to-blue-900/40 border border-violet-500/30 rounded-2xl p-6">
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center justify-center gap-4 text-xs text-zinc-500">
          ...
          <span className="flex items-center gap-1">
            <Lock className="w-3 h-3" /> KYC Protected
          </span>
          <span className="flex items-center gap-1">
            <CheckCircle className="w-3 h-3" /> Instant Credit
          </span>
        </div>
      </div>
    </div>
...
export default function InvestorPortal() {
  ...
  const { data: kpis } = trpc.investor.kpis.useQuery(undefined, {
    enabled: !!user,
    retry: false,
  });
  const { data: myPurchases } = trpc.ico.getMyPurchases.useQuery(undefined, {
    enabled: !!user,
    retry: false,
  });
  const { data: myReferral } = trpc.ico.getMyReferralCode.useQuery(undefined, {
    enabled: !!user,
    retry: false,
  });
  ...
  return (
    <div className="min-h-screen bg-[#07050f] text-white">
      ...
      >
        {/* Floating glow orbs */}
        <div
          className="glow-orb glow-orb-gold w-96 h-96 -top-20 -right-20 animate-hero-float"
          style={{ animationDelay: "0s" }}
        />
        <div
          className="glow-orb glow-orb-purple w-80 h-80 top-40 -left-20 animate-hero-float"
          style={{ animationDelay: "2s" }}
        />
        <div
          className="glow-orb glow-orb-pink w-64 h-64 bottom-0 right-1/3 animate-hero-float"
          ...
          <div className="grid lg:grid-cols-2 gap-12 items-center">
            <div>
              ...
              <h1 className="text-5xl lg:text-7xl font-black mb-5 leading-[0.95] text-rainbow">
                ...
                <br />
                <span className="text-slate-300 text-3xl lg:text-4xl font-bold">
                  The Future of Social Web3
                </span>
              </h1>

              <p className="text-base mb-8 leading-relaxed max-w-lg desc-metallic">
                SKY444 powers the ShadowChat ecosystem — an AI-driven Web3
                social OS where every action earns real economic value. Buy at
                Pre-Sale price before the public launch.
              </p>
              ...
              <div className="grid grid-cols-3 gap-3 mb-8">
                {[
                  ...
                  <div
                    key={m.label}
                    className={`text-center rounded-xl border p-3 ${m.bg}`}
                  >
                    <div
                      className={`text-2xl font-black stat-number ${m.color}`}
                    >
                      {m.value}
                    </div>
              ...
              <div className="bg-slate-900/70 border border-slate-700/60 rounded-2xl p-5 backdrop-blur-sm">
                ...
                <div className="flex justify-between text-xs">
                  ...
                  <span className="text-slate-500">
                    ${(hardCap - totalRaised).toLocaleString()} remaining
                  </span>
                </div>
              </div>
          ...
          <TabsContent value="overview">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
              ...
              <StatCard
                icon={Users}
                label="Investors"
                value={String(icoStats?.totalParticipants ?? 0)}
                sub="unique wallets"
                color="blue"
              />
              <StatCard
                icon={Coins}
                label="Tokens Sold"
                value={`${((icoStats?.totalTokensSold ?? 0) / 1000000).toFixed(1)}M`}
                sub="SKY444"
                color="emerald"
              />
              <StatCard
                ...
                {ICO_TIERS.map(tier => (
                  ...
                  >
                    ...
                    <div className="text-xs text-zinc-500 mb-3">
                      Hard Cap: ${(tier.hardCap / 1000000).toFixed(1)}M
                    </div>
                    <div className="flex items-center justify-between">
                      <Badge
                        className={`text-xs ${tier.bonus > 0 ? "bg-green-500/20 text-green-400 border-green-500/30" : "bg-zinc-700 text-zinc-400"}`}
                      >
                        +{tier.bonus}% bonus
                      </Badge>
                      <Badge
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                  <StatCard
                    icon={Users}
                    label="Total Users"
                    value={(kpis as any).totalUsers?.toLocaleString() ?? "—"}
                    color="blue"
                  />
                  <StatCard
                    icon={Activity}
                    label="Total Posts"
                    value={(kpis as any).totalPosts?.toLocaleString() ?? "—"}
                    color="emerald"
                  />
                  <StatCard
                    icon={Coins}
                    label="Tokens in Circulation"
                    value={`${((kpis as any).tokensInCirculation / 1000000 || 0).toFixed(1)}M`}
                    color="violet"
                  />
                  <StatCard
                    icon={TrendingUp}
                    label="Staking TVL"
                    value={`${((kpis as any).stakingTVL / 1000 || 0).toFixed(0)}K SKY`}
                    color="yellow"
                  />
                </div>
              </div>
            ...
            <div>
              ...
              <div className="grid md:grid-cols-2 gap-4">
                {[
                  ...
                  <div
                    key={item.label}
                    className="flex items-center gap-3 bg-zinc-900/40 border border-zinc-800 rounded-xl p-4"
                  >
                    <div
                      className={`w-10 h-10 rounded-lg ${item.color} flex items-center justify-center text-white font-bold text-sm flex-shrink-0`}
                    >
                      {item.pct}%
                    </div>
                    <div>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-zinc-900/60 border border-zinc-700 rounded-xl overflow-hidden">
                  {[
                    ...
                    <div
                      key={row.label}
                      className={`flex justify-between px-4 py-3 text-sm ${i % 2 === 0 ? "bg-zinc-800/30" : ""}`}
                    >
            ...
            {!user ? (
              <div className="text-center py-16">
                ...
                <p className="text-zinc-500 mb-6">
                  Track your SKY444 purchases, vesting schedule, and claimable
                  tokens.
                </p>
                ...
                {myReferral && (
                  <div className="bg-gradient-to-br from-violet-900/20 to-blue-900/20 border border-violet-500/30 rounded-xl p-5">
                    ...
                    <p className="text-xs text-zinc-500 mt-2">
                      Share this code. When someone uses it to buy SKY444, you
                      earn 5% of their purchase in bonus tokens.
                    </p>
                  </div>
                )}
                    ...
                    <div className="space-y-4">
                      {(myPurchases as any[]).map((purchase: any) => {
                        ...
                        const released = parseFloat(
                          purchase.tokens_released ?? 0
                        );
                        const vestingPct =
                          totalTokens > 0 ? (released / totalTokens) * 100 : 0;
                        return (
                          <div
                            key={purchase.id}
                            className="bg-zinc-900/60 border border-zinc-700 rounded-xl p-5"
                          >
                            ...
                            <div className="space-y-1">
                              ...
                              <div className="flex justify-between text-xs">
                                <span className="text-zinc-500">
                                  Released: {released.toLocaleString()} SKY444
                                </span>
                                <span className="text-zinc-500">
                                  Locked:{" "}
                                  {(totalTokens - released).toLocaleString()}{" "}
                                  SKY444
                                </span>
                              </div>
                            </div>
                    ...
                    <div
                      ...
                      className={`flex items-center gap-4 p-4 rounded-xl border ${
                        i === 0
                          ...
                          : i === 1
                            ? "border-zinc-400/30 bg-zinc-800/30"
                            : i === 2
                    ...
                    >
                      ...
                      <div className="flex-1">
                        <div className="font-semibold">
                          {entry.username ?? `Investor ${i + 1}`}
                        </div>
                        <div className="text-xs text-zinc-500">
                          {parseInt(entry.purchase_count ?? 0)} purchases
                        </div>
                      </div>
                      <div className="text-right">
                        <div className="font-bold text-violet-300">
                          {parseFloat(entry.total_tokens ?? 0).toLocaleString()}{" "}
                          SKY444
                        </div>
                        <div className="text-xs text-zinc-500">
                          ${parseFloat(entry.total_usd ?? 0).toLocaleString()}{" "}
                          invested
                        </div>
                      </div>
                    </div>
            ...
            <div>
              <div className="bg-yellow-900/20 border border-yellow-500/30 rounded-xl p-4 mb-6 flex items-start gap-3">
                ...
                <div className="text-sm text-yellow-200">
                  <strong>Important:</strong> Please read all legal documents
                  before participating in the ICO. Purchasing SKY444 tokens
                  involves significant risk, including the risk of total loss.
                  These documents constitute binding legal agreements.
                </div>
              </div>
                ...
                {LEGAL_DOCS.map(({ name, icon: Icon, path, desc }) => (
                  ...
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="font-semibold text-sm mb-1 group-hover:text-violet-300 transition-colors">
                        {name}
                      </div>
          ...
          <TabsContent value="faq">
            <div className="max-w-3xl mx-auto space-y-3">
              {faqs.map((faq, i) => (
                <div
                  key={i}
                  className="bg-zinc-900/60 border border-zinc-700 rounded-xl overflow-hidden"
                >
                  <button
      ...
      <div className="border-t border-zinc-800 bg-gradient-to-br from-zinc-950 to-violet-950/20">
        <div className="max-w-7xl mx-auto px-4 py-12 text-center">
          ...
          <p className="text-zinc-400 mb-6">
            Pre-Sale closes when $2.5M hard cap is reached. Don't miss the 15%
            bonus.
          </p>
