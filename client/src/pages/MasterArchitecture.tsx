...
export default function MasterArchitecture() {
  ...
  const [activeTab, setActiveTab] = useState<
    "architecture" | "ux-flow" | "data-flow" | "principles"
  >("architecture");
    ...
    <div className="min-h-screen bg-background p-4 max-w-5xl mx-auto space-y-6">
      ...
      <div>
        <div className="flex items-center gap-3 mb-2">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Phase 43 — Single source of truth
            </p>
          </div>
        </div>
        <blockquote className="mt-3 p-3 bg-gradient-to-r from-purple-500/10 to-cyan-500/10 border border-purple-500/20 rounded-xl text-sm italic text-muted-foreground">
          "ShadowChat is a distributed AI-native operating system composed of
          event-driven microservices, a real-time simulation engine, and a
          dual-interface frontend (legacy + OS), unified by a shared data and
          event architecture."
        </blockquote>
      </div>
      ...
      <div className="flex gap-1 p-1 bg-secondary/50 rounded-xl w-full overflow-x-auto">
        {(["architecture", "ux-flow", "data-flow", "principles"] as const).map(
          tab => (
            <button
              key={tab}
        ...
        <div className="space-y-4">
            ...
            <div className="card p-3 border border-blue-500/30 bg-blue-500/5">
              ...
              <div className="flex gap-1 mt-2">
                {["Feed", "Chat", "Profile"].map(m => (
                  <span
                    key={m}
                    className="px-1.5 py-0.5 rounded bg-blue-500/20 text-blue-300 text-xs"
                  >
                    {m}
                  </span>
                ))}
          ...
          {LAYERS.map(layer => {
            ...
            return (
              <div
                key={layer.id}
                className={`card border ${layer.bg} transition-all`}
              >
                <button
                ...
                >
                  <div
                    className={`w-8 h-8 rounded-lg bg-gradient-to-br ${layer.color} flex items-center justify-center shrink-0`}
                  >
      ...
      {activeTab === "ux-flow" && (
        <div className="space-y-2">
          <p className="text-sm text-muted-foreground">
            What the user experiences from open to action.
          </p>
      ...
      {activeTab === "data-flow" && (
        <div className="space-y-2">
          <p className="text-sm text-muted-foreground">
            What the system does for every user action.
          </p>
          {DATA_FLOW.map((step, idx) => (
            <div key={idx} className="flex items-center gap-3">
              <div className="flex flex-col items-center shrink-0 w-8">
                <div
                  className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${idx === 0 ? "bg-purple-500 text-white" : idx === DATA_FLOW.length - 1 ? "bg-green-500 text-white" : "bg-secondary text-muted-foreground"}`}
                >
                  {idx + 1}
                </div>
                {idx < DATA_FLOW.length - 1 && (
                  <div className="w-0.5 h-3 bg-border mt-0.5" />
                )}
              </div>
              <div
                className={`card p-2.5 flex-1 text-sm ${idx === 0 ? "border-purple-500/30 bg-purple-500/5" : idx === DATA_FLOW.length - 1 ? "border-green-500/30 bg-green-500/5" : ""}`}
              >
                {step}
              </div>
            </div>
