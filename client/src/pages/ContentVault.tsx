        ...
        <div className="flex gap-3 mb-6 flex-wrap">
          ...
          <div className="flex border border-slate-800 rounded-lg overflow-hidden">
            <button
              ...
              className={`p-2 ${viewMode === "grid" ? "bg-slate-700 text-white" : "text-slate-500"}`}
            >
              <Grid className="w-4 h-4" />
            </button>
            <button
              onClick={() => setViewMode("list")}
              className={`p-2 ${viewMode === "list" ? "bg-slate-700 text-white" : "text-slate-500"}`}
            >
        ...
        >
          {filtered.map(item => (
            ...
            >
              {/* Thumbnail */}
              <div
                className={`relative bg-gradient-to-br from-slate-800 to-slate-900 ${viewMode === "list" ? "w-16 h-16 rounded-lg shrink-0" : "aspect-square"} flex items-center justify-center`}
              >
              ...
              <div className={viewMode === "list" ? "flex-1 min-w-0" : "p-3"}>
                <p
                  className={`text-sm font-semibold truncate mb-1 ${item.unlocked ? "text-white" : "text-slate-500"}`}
                >
                  {item.title}
                </p>
                <div className="flex items-center gap-2 mb-2">
                  <Badge
                    className={`text-[9px] px-1.5 py-0.5 border ${TIER_COLORS[item.tier]}`}
                  >
                    {item.tier}
                  </Badge>
                ...
                <div className="flex items-center justify-between">
                  ...
                  {item.unlocked ? (
                    <Button
                      size="sm"
                      className="h-6 px-2 text-[10px] bg-green-700 hover:bg-green-600 text-white"
                    >
                    ...
                    <Button
                      size="sm"
                      className="h-6 px-2 text-[10px] bg-red-700 hover:bg-red-600 text-white"
                    >
