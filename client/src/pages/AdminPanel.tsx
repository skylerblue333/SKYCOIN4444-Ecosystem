...
export default function AdminPanel() {
  ...
  const { data: adminStats, isLoading: statsLoading } =
  ...
  } = trpc.admin.moderationQueue.useQuery(
    { limit: 50 },
    { enabled: tab === "reports" }
  );
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
        {[
          ...
          },
          {
            ...
            value: statsLoading
              ? "…"
              : (adminStats?.onlineUsers ?? 0).toLocaleString(),
          ...
          },
          {
            ...
            value: statsLoading
              ? "…"
              : (adminStats?.connections ?? 0).toLocaleString(),
            color: "text-yellow-400",
          },
          {
            icon: CheckCircle,
            label: "Status",
            value: statsLoading ? "…" : (adminStats?.health ?? "—"),
            color: "text-green-400",
          },
        ].map((s, i) => (
                ...
                <tbody>
                  {(usersData as any[]).map((u: any) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full ${u.role === "admin" ? "bg-primary/10 text-primary" : "bg-secondary text-muted-foreground"}`}
                        >
                          {u.role}
                        </span>
                      </td>
                      <td className="px-4 py-3">
                        <span
                          className={`text-xs px-2 py-0.5 rounded-full ${u.isBanned ? "bg-destructive/10 text-destructive" : "bg-green-500/10 text-green-400"}`}
                        >
              ...
              <div key={r.id} className="card p-4">
                <div className="flex items-start justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <AlertTriangle
                      className={`w-4 h-4 ${r.action === "ban" ? "text-destructive" : "text-yellow-400"}`}
                    />
                    <span className="text-sm font-medium capitalize">
                      {r.action ?? r.type ?? "report"}
                    </span>
                    <span className="text-xs text-muted-foreground">
                      by {r.moderatorName ?? r.moderatorId} on user{" "}
                      {r.targetUserId ?? r.targetId}
                    </span>
                  </div>
                  <span className="text-xs text-muted-foreground">
                    {r.createdAt
                ...
                {r.reason && (
                  <p className="text-sm text-muted-foreground bg-secondary/50 rounded px-3 py-2 italic">
                    "{r.reason}"
                  </p>
                )}
              </div>
          ...
          {statsLoading ? (
            ...
            <>
              {[
                ...
                {
                  label: "Total Posts",
                  value: (adminStats?.totalPosts ?? 0).toLocaleString(),
                },
                {
                  label: "Total Streams",
                  value: (adminStats?.totalStreams ?? 0).toLocaleString(),
                },
                {
                  label: "Total Communities",
                  value: (adminStats?.totalCommunities ?? 0).toLocaleString(),
                },
                {
                  label: "Staking Positions",
                  value: (
                    adminStats?.totalStakingPositions ?? 0
                  ).toLocaleString(),
                },
                {
                  label: "Online Now",
                  value: (adminStats?.onlineUsers ?? 0).toLocaleString(),
                },
                {
                  label: "Active Connections",
                  value: (adminStats?.connections ?? 0).toLocaleString(),
                },
