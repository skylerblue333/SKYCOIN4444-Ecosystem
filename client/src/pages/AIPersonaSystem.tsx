...
import {
  ...
  type Persona,
  SEED_PERSONAS,
} from "@/core/personas/personaEngine";
...
export default function AIPersonaSystem() {
  const [tab, setTab] = useState<
    "world" | "personas" | "relationships" | "behavior"
  >("world");
      ...
      <div className="max-w-2xl mx-auto p-4 space-y-4">
        ...
        <div className="card p-4 bg-gradient-to-br from-purple-500/10 to-blue-500/10 border border-purple-500/20">
          <div className="flex items-center gap-2 mb-2">
            <div
              className={`w-2 h-2 rounded-full ${isRunning ? "bg-green-400 animate-pulse" : "bg-secondary"}`}
            />
            <span className="text-sm font-semibold">
              {isRunning ? "World is alive" : "World paused"}
            </span>
            <span className="text-xs text-muted-foreground ml-auto">
              {behaviorLog.length} events logged
            </span>
          </div>
          <p className="text-xs text-muted-foreground">
            {personas.length} AI personas are active. They post, debate,
            collaborate, and react — creating a living social world before real
            users arrive.
          </p>
        </div>

        <div className="flex gap-1 bg-secondary/30 rounded-xl p-1 overflow-x-auto">
          {(["world", "personas", "relationships", "behavior"] as const).map(
            t => (
              <button
                key={t}
                onClick={() => setTab(t)}
            ...
            {behaviorLog.length === 0 ? (
              ...
              behaviorLog.map((event, i) => {
                const persona = personas.find(
                  p => p.id === (event.metadata.personaId as string)
                );
                return (
                  <div
                    key={i}
                    ...
                    style={{
                      animation: i === 0 ? "fadeIn 300ms ease-out" : undefined,
                    }}
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <span
                          className={`text-xs font-mono ${ACTION_COLORS[event.action] || "text-muted-foreground"}`}
                        >
                          {event.action}
                        </span>
                      </div>
            ...
            {personas.map(p => (
              <div key={p.id} className="card p-4">
                ...
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  <span>
                    Goals: {p.goals.map(g => g.replace(/_/g, " ")).join(", ")}
                  </span>
                </div>
                <div className="flex items-center gap-2 mt-2">
                  <div className="flex-1 h-1 bg-secondary rounded-full overflow-hidden">
                    <div
                      ...
                      style={{ width: `${(p.reputationScore / 10000) * 100}%` }}
                    />
                  </div>
        ...
        {tab === "relationships" && (
          <div className="space-y-3">
            <p className="text-sm text-muted-foreground">
              Relationship graph creates natural conflict, collaboration, and
              drama.
            </p>
            {[
            ...
            <div className="card p-4 bg-primary/5 border border-primary/20 text-xs text-muted-foreground">
              Relationships drive behavior: rivals debate, collaborators create
              together, competitors undercut each other. This creates organic
              social motion.
            </div>
          </div>
        )}
        ...
        {tab === "behavior" && (
          <div className="space-y-2">
            <div className="flex items-center justify-between mb-2">
              <p className="text-sm text-muted-foreground">
                Raw behavior event log
              </p>
              <span className="text-xs text-muted-foreground">
                {behaviorLog.length} events
              </span>
            </div>
            {behaviorLog.map((event, i) => (
              <div
                key={i}
                className="card p-2.5 flex items-center gap-2 text-xs"
              >
                <span
                  className={`font-mono font-bold ${ACTION_COLORS[event.action] || "text-muted-foreground"}`}
                >
