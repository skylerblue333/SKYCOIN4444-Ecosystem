    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-2xl mx-auto p-4 space-y-4">
        <div className="card p-4 bg-gradient-to-br from-green-500/10 to-cyan-500/10 border border-green-500/20">
          ...
          <p className="text-sm">
            "A value extraction system where every interaction is a monetizable
            action."
          </p>
          <p className="text-xs text-muted-foreground mt-1">
            You don't wait for users. You design triggers that force value
              ...
              <div key={tier.tier} className={`card p-4 border ${tier.bg}`}>
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2">
                    <span className={`text-lg font-black ${tier.color}`}>
                      {tier.tier}
                    </span>
        ...
        {activeTab === "gap" && (
          <div className="space-y-3">
            <p className="text-xs text-muted-foreground">
              The gap between free and paid is where revenue lives. Design it
              deliberately.
            </p>
        ...
        {activeTab === "upsell" && (
          <div className="space-y-2">
            <p className="text-xs text-muted-foreground mb-3">
              One action = chain of monetization. AI always suggests the next
              paid step.
            </p>
            {UPSELL_CHAIN.map((step, i) => (
              <div key={i} className="card p-3 flex items-start gap-3">
                <div
                  className={`w-7 h-7 rounded-full flex items-center justify-center shrink-0 text-xs font-bold ${i === UPSELL_CHAIN.length - 1 ? "bg-yellow-500/20 text-yellow-400" : "bg-primary/20 text-primary"}`}
                >
                  {step.step}
                </div>
            ...
            <div className="card p-3 bg-green-500/10 border border-green-500/20 text-center">
              ...
              <div className="text-sm font-bold">
                Total from one logo request: $74.97
              </div>
        ...
        {activeTab === "flow" && (
          <div className="space-y-2">
            <p className="text-xs text-muted-foreground mb-3">
              First-time user monetization flow — designed to convert on first
              visit.
            </p>
            {CONVERSION_FLOW.map((step, i) => (
              <div key={i} className="flex items-center gap-3">
                <div
                  className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 text-xs font-bold ${i === 0 ? "bg-blue-500/20 text-blue-400" : i === CONVERSION_FLOW.length - 1 ? "bg-green-500/20 text-green-400" : "bg-primary/20 text-primary"}`}
                >
                  {i + 1}
                </div>
                <div
                  className={`flex-1 p-3 rounded-lg border ${i === 4 ? "bg-primary/5 border-primary/20 font-medium" : "bg-secondary/20 border-border/30"}`}
                >
            ...
            <div className="card p-3 bg-gradient-to-r from-green-500/10 to-cyan-500/10 border border-green-500/20 mt-4">
              ...
              <p className="text-xs text-muted-foreground mt-1">
                After result delivery, AI immediately suggests next paid action.
                Loop never ends.
              </p>
            </div>
          </div>
