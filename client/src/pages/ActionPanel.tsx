...
export default function ActionPanel() {
  ...
  const aiChatMut = trpc.ai.chat.useMutation({
    onSuccess: data => {
      const msg =
        (data as any)?.content ?? (data as any)?.message ?? "Task processed!";
  ...
  const handleTip = () => {
    ...
    tipMut.mutate({userId: user?.id || "guest",  recipientId: 2, amount: tipAmount, message: tipMessage });
  };
  ...
  const handleAITask = () => {
    ...
    aiChatMut.mutate({userId: user?.id || "guest", 
      message: aiTask,
      systemPrompt:
      ...
      <div className="min-h-screen flex items-center justify-center bg-background p-4">
        <div className="text-center space-y-4 max-w-sm">
          ...
          <p className="text-muted-foreground text-sm">
            The action has been executed. Value is flowing.
          </p>
      ...
      <div className="sticky top-0 z-10 bg-background/80 backdrop-blur-sm border-b border-border/50 px-4 py-3 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Execute real-world actions from chat
          </p>
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto p-4 space-y-6">
          ...
          <>
            <div className="grid grid-cols-2 gap-3">
              {[
                ...
                {
                  mode: "ai_task" as ActionMode,
                  icon: Bot,
                  label: "AI Task",
                  desc: "Let AI execute for you",
                  color: "from-purple-500/20 to-primary/20 border-primary/30",
                  iconColor: "text-purple-400",
                },
                {
                  mode: "request_service" as ActionMode,
                  icon: Briefcase,
                ...
                {
                  mode: "create_listing" as ActionMode,
                  icon: ShoppingBag,
          ...
          <div className="card p-5 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="font-bold text-lg flex items-center gap-2">
                <DollarSign className="w-5 h-5 text-yellow-400" /> Send Tip
              </h2>
              <button
            ...
            <div>
              ...
              <div className="flex gap-2 flex-wrap mb-2">
                {QUICK_TIP_AMOUNTS.map(amt => (
                  <button
                    ...
                    className={`px-3 py-1.5 rounded-xl text-sm font-medium transition-colors ${tipAmount === amt ? "bg-primary text-primary-foreground" : "bg-secondary/50 hover:bg-secondary"}`}
                  >
                    {amt}
                  </button>
                ))}
              ...
              <div className="flex items-center gap-2 bg-secondary/50 border border-border/50 rounded-xl px-3 py-2">
                ...
                <input
                  ...
                  className="flex-1 bg-transparent text-center text-sm font-bold"
                />
                <button
        ...
        {activeMode === "request_service" && (
          <div className="card p-5 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="font-bold text-lg flex items-center gap-2">
                <Briefcase className="w-5 h-5 text-blue-400" /> Request Service
              </h2>
              <button
                onClick={() => setActiveMode(null)}
                className="p-1.5 rounded-lg hover:bg-secondary/50"
              ...
              <div className="grid grid-cols-3 gap-2">
                {SERVICE_CATEGORIES.map(cat => (
                  <button
                    ...
                    className={`p-2.5 rounded-xl text-center transition-colors ${serviceCategory === cat.id ? "bg-primary/20 border border-primary/50" : "bg-secondary/30 hover:bg-secondary/50"}`}
                  >
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full bg-secondary/50 border border-border/50 rounded-xl px-3 py-2 text-sm h-20 resize-none"
                placeholder="I need a logo for my crypto startup..."
              />
            </div>
            <div>
              <label className="text-sm font-medium mb-1 block">
        ...
        {activeMode === "create_listing" && (
          <div className="card p-5 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="font-bold text-lg flex items-center gap-2">
                <ShoppingBag className="w-5 h-5 text-green-400" /> Create
                Listing
              </h2>
              <button
                onClick={() => setActiveMode(null)}
        ...
        {activeMode === "ai_task" && (
          <div className="card p-5 space-y-4">
            <div className="flex items-center justify-between">
              <h2 className="font-bold text-lg flex items-center gap-2">
                <Bot className="w-5 h-5 text-purple-400" /> AI Task
              </h2>
              <button
                onClick={() => setActiveMode(null)}
                className="p-1.5 rounded-lg hover:bg-secondary/50"
            ...
            <div className="bg-purple-500/10 border border-purple-500/20 rounded-xl p-3 text-sm text-muted-foreground">
              Describe what you want the AI to do. It can find services, execute
              payments, create listings, match you with people, and more.
            </div>
            <div>
              <label className="text-sm font-medium mb-1 block">
                Task Description
              ...
              <textarea
                ...
                className="w-full bg-secondary/50 border border-border/50 rounded-xl px-3 py-2 text-sm h-24 resize-none"
                placeholder="Find me a designer for $50 and send them a message..."
              />
            </div>
            <div className="grid grid-cols-2 gap-2">
              {[
                ...
                <button
                  key={ex}
                  onClick={() => setAiTask(ex)}
                  className="text-xs bg-secondary/30 hover:bg-secondary/50 rounded-xl px-3 py-2 text-left transition-colors"
                >
                  {ex}
                </button>
              ))}
