...
const PROMQL_QUERIES = [
  {
    ...
    query: `sum(rate(http_requests_total{job="fastapi-gateway-metrics"}[5m]))`,
    value: "2,847 req/s",
  },
  {
    name: "ML Latency P99",
...
export default function DevOps() {
  const [tab, setTab] = useState<
    "services" | "metrics" | "kafka" | "acquisition" | "compose"
  >("services");
      ...
      <div className="max-w-7xl mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
          {[
            ...
            {
              label: "Total Memory",
              value: `${(totalMem / 1024).toFixed(1)} GB`,
              icon: Database,
              color: "text-purple-400",
              bg: "bg-purple-500/10",
            },
        ...
        <div className="flex gap-1 bg-slate-900/60 rounded-xl p-1 w-fit mb-6 border border-slate-800/50 flex-wrap">
          {(
            ["services", "metrics", "kafka", "acquisition", "compose"] as const
          ).map(t => (
            <button
              key={t}
              onClick={() => setTab(t)}
        ...
        {tab === "services" && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {SERVICES.map(svc => (
              <div
                key={svc.name}
                className={`rounded-2xl border p-4 ${svc.bg} ${svc.border}`}
              >
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <div className={`font-bold text-sm ${svc.color}`}>
                      {svc.name}
                    </div>
                ...
                <div className="grid grid-cols-3 gap-2 text-center">
                  <div className="bg-slate-950/40 rounded-lg p-2">
                    <div className={`text-sm font-bold ${svc.color}`}>
                      {svc.cpu}%
                    </div>
                ...
                <div className="mt-3">
                  ...
                  <div className="h-1.5 bg-slate-800 rounded-full overflow-hidden">
                    <div
                      className={`h-full rounded-full ${svc.cpu > 70 ? "bg-rose-500" : svc.cpu > 40 ? "bg-amber-500" : "bg-emerald-500"}`}
                      style={{ width: `${svc.cpu}%` }}
                    />
                  </div>
                </div>
          ...
          <div className="space-y-6">
            <div className="bg-slate-900/40 rounded-2xl border border-slate-800/60 p-6">
              <h3 className="font-semibold text-white mb-4 flex items-center gap-2">
                <BarChart3 className="h-5 w-5 text-amber-400" /> Live PromQL
                Metrics
              </h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {PROMQL_QUERIES.map(q => (
                  <div
                    key={q.name}
                    className="bg-slate-950/60 rounded-xl p-4 border border-slate-800/40"
                  >
            ...
            <div className="bg-amber-950/20 border border-amber-800/30 rounded-2xl p-5">
              <h3 className="font-semibold text-white mb-3 flex items-center gap-2">
                <Eye className="h-5 w-5 text-amber-400" /> Grafana Dashboard
                Access
              </h3>
              <p className="text-sm text-slate-400 mb-3">
                Grafana is running at port 3001. Connect to view historical
                verification logs, request drop-offs, and live system memory
                stability graphs for enterprise acquisition audits.
              </p>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                {[
                  ...
                  <div
                    key={d}
                    className="bg-slate-950/40 rounded-lg p-3 text-center"
                  >
          ...
          <div className="space-y-6">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                ...
                <div
                  key={m.label}
                  className={`rounded-xl border border-slate-800/60 p-4 ${m.bg}`}
                >
                  ...
                  >
                    <div>
                      ...
                      <div className="text-xs text-slate-500">
                        {t.partitions} partitions · {t.msgs} messages
                      </div>
                    </div>
                    <Badge className="bg-emerald-500/10 text-emerald-400 border-0 text-xs">
                      Lag: {t.lag}
          ...
          <div className="space-y-6">
            <div className="bg-blue-950/20 border border-blue-800/30 rounded-2xl p-5 mb-2">
              <h3 className="font-semibold text-white mb-1">
                M&A Acquisition Readiness
              </h3>
              <p className="text-sm text-slate-400">
                Complete all items before initiating acquisition discussions.
                Target: $15M valuation at 10-15x ARR.
              </p>
            </div>
            {ACQUISITION_CHECKLIST.map(cat => (
              <div
                key={cat.category}
                className="bg-slate-900/40 rounded-2xl border border-slate-800/60 p-5"
              >
                ...
                <div className="space-y-2">
                  {cat.items.map(item => (
                    ...
                    >
                      ...
                      <span
                        className={`text-sm ${item.done ? "text-slate-300" : "text-slate-500"}`}
                      >
                        {item.label}
                      </span>
                      {item.done && (
                        <Badge className="ml-auto bg-emerald-500/10 text-emerald-400 border-0 text-xs">
                          Done
                        </Badge>
                      )}
                    </div>
                ...
                <div className="mt-3 flex items-center gap-2">
                  <div className="flex-1 h-1.5 bg-slate-800 rounded-full overflow-hidden">
                    <div
                      ...
                      style={{
                        width: `${(cat.items.filter(i => i.done).length / cat.items.length) * 100}%`,
                      }}
                    />
                  </div>
          ...
          <div className="space-y-4">
            ...
            <div className="bg-emerald-950/20 border border-emerald-800/30 rounded-2xl p-4">
              ...
              <p className="text-xs text-slate-500 mt-2">
                Spins up all 9 services. Frontend at :3000, API at :8000,
