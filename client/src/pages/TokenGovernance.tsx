...
export default function TokenGovernance() {
  const [tab, setTab] = useState<"proposals" | "treasury" | "params">(
    "proposals"
  );
      ...
      <div className="sticky top-0 z-10 bg-background/80 backdrop-blur-sm border-b border-border/50 px-4 py-3 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            DAO & on-chain governance — Phase 11
          </p>
        </div>
                ...
                <div key={p.id} className="card p-4">
                  <div className="flex items-start gap-2 mb-2">
                    ...
                    <div className="flex items-center gap-1 ml-auto">
                      ...
                      <span
                        className={`text-xs capitalize ${p.status === "active" ? "text-yellow-400" : p.status === "passed" ? "text-green-400" : "text-red-400"}`}
                      >
                        {p.status}
                      </span>
                    </div>
                  ...
                  {p.status === "active" && (
                    <div className="flex gap-2 mt-3">
                      <button className="flex-1 py-1.5 text-xs bg-green-500/20 text-green-400 rounded-lg hover:bg-green-500/30 transition-colors">
                        Vote For
                      </button>
                      <button className="flex-1 py-1.5 text-xs bg-red-500/20 text-red-400 rounded-lg hover:bg-red-500/30 transition-colors">
                        Vote Against
                      </button>
                    </div>
                  )}
