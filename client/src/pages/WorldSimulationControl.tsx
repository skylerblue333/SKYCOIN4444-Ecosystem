      ...
      <div className="border-b border-white/10 bg-[oklch(0.1_0.03_270)]/80 backdrop-blur-sm sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Badge
              variant="outline"
              className="text-[oklch(0.75_0.2_145)] border-[oklch(0.75_0.2_145)]/30 text-xs"
            >
            ...
            <div className="flex items-center justify-between mb-4">
              <div>
                ...
                <div className="text-xs text-white/40">
                  Tick #{tick.toLocaleString()} — {Math.floor(tick / 1440)} days
                  elapsed
                </div>
              </div>
              <div className="flex items-center gap-2">
          ...
          <TabsContent value="personas">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {personas.map(p => (
                <Card
                  key={p.id}
                  className={`border transition-all ${p.active ? "bg-white/8 border-[oklch(0.75_0.2_145)]/30" : "bg-white/5 border-white/10"}`}
                >
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        <div
                          className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold ${p.active ? "bg-[oklch(0.75_0.2_145)]/20 text-[oklch(0.75_0.2_145)]" : "bg-white/10 text-white/40"}`}
                        >
                          {p.name[0]}
                        </div>
                        <div>
                      ...
                      >
                        <div
                          className={`w-10 h-5 rounded-full transition-colors relative ${p.active ? "bg-[oklch(0.75_0.2_145)]" : "bg-white/20"}`}
                        >
                          <div
                            className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-transform ${p.active ? "translate-x-5" : "translate-x-0.5"}`}
                          />
                        </div>
                      </button>
                    </div>
              ...
              <CardContent className="p-4">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    size="sm"
                    onClick={injectTrend}
                    className="bg-[oklch(0.75_0.2_145)]/20 text-[oklch(0.75_0.2_145)] border-[oklch(0.75_0.2_145)]/30 hover:bg-[oklch(0.75_0.2_145)]/30"
                  >
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                ...
                <button
                  key={tag}
                  onClick={() => {
                    toast.success(`Trend injected: ${tag}`);
                  }}
              ...
              <CardContent>
                ...
                <div className="flex gap-2">
                  <Button
                    ...
                    variant="outline"
                    className="text-xs border-white/20 text-white/60 hover:text-white"
                    onClick={() => toast.success("10 synthetic users spawned")}
                  >
                    Spawn 10 Users
                  </Button>
                  <Button
                    ...
                    variant="outline"
                    className="text-xs border-white/20 text-white/60 hover:text-white"
                    onClick={() => toast.success("100 synthetic users spawned")}
                  >
                    Spawn 100 Users
                  </Button>
                  <Button
                    size="sm"
                    variant="outline"
                    className="text-xs border-[oklch(0.7_0.2_25)]/40 text-[oklch(0.7_0.2_25)] hover:bg-[oklch(0.7_0.2_25)]/10"
                    onClick={() => toast.success("All synthetic users removed")}
                  >
                    Clear All
                  </Button>
                </div>
