  ...
  return (
    ...
    >
      ...
      <div className="p-3">
        {/* Tier number */}
        <div
          className={`text-center text-xs font-bold mb-2 ${isSpecial ? "text-amber-400" : "text-muted-foreground"}`}
        >
          T{tier.tier}
        </div>
        ...
        <div className="mb-2">
          ...
          <button
            onClick={() =>
              !freeClaimed && reached && onClaim(tier.tier, "free")
            }
            ...
            className={`w-full p-2 rounded-lg text-center text-xs transition-all ${
              freeClaimed
                ...
                : reached
                  ? "bg-white/10 border border-white/20 text-white hover:bg-white/20 cursor-pointer"
            ...
            {freeClaimed ? (
              ...
              <span>
                {tier.freeReward.type === "xp"
                  ? `+${tier.freeReward.amount} XP`
                  : tier.freeReward.type === "sky444"
                    ? `${tier.freeReward.amount} SKY`
                    : "Reward"}
              </span>
            )}
          </button>
        ...
        <div>
          ...
          <button
            ...
            className={`w-full p-2 rounded-lg text-center text-xs transition-all ${
              premClaimed
                ...
                : !progress.isPremium
                  ? "bg-amber-950/20 border border-amber-500/20 text-amber-600 cursor-not-allowed"
                  : reached
          ...
          >
            {premClaimed ? (
              ...
              <span>
                {tier.premiumReward.type === "badge"
                  ? `🏆 ${tier.premiumReward.rarity}`
                  : tier.premiumReward.type === "sky444"
                    ? `${tier.premiumReward.amount} SKY`
                    : "Reward"}
              </span>
            )}
          </button>
...
export default function BattlePass() {
  ...
  const { data, refetch } = trpc.gamification.getBattlePass.useQuery(
    undefined,
    { enabled: !!user }
  );
  ...
  const claimTier = trpc.gamification.claimTier.useMutation({
    onSuccess: (data: any) => {
      toast.success(
        `Claimed! ${data.reward.type === "xp" ? `+${data.reward.amount} XP` : data.reward.type === "sky444" ? `+${data.reward.amount} SKY444` : "Badge unlocked!"}`
      );
      refetch();
    },
        ...
        <div className="container max-w-5xl mx-auto px-4 relative z-10">
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              ...
              <h1 className="text-4xl font-black rainbow-text mb-2">
                Battle Pass
              </h1>
            ...
            <div className="flex items-center gap-4 bg-white/5 border border-white/10 rounded-xl px-5 py-3">
              ...
              <div className="text-center">
                <div className="text-2xl font-black text-amber-300">
                  {progress.xpEarned?.toLocaleString() ?? 0}
                </div>
              ...
              <div className="text-center">
                <div
                  className={`text-2xl font-black ${progress.isPremium ? "text-amber-400" : "text-muted-foreground"}`}
                >
                  {progress.isPremium ? "✓" : "—"}
                </div>
      ...
      <div className="container max-w-5xl mx-auto px-4 py-10">
        {!user ? (
          <div className="text-center py-20 text-muted-foreground">
            Sign in to view your Battle Pass progress
          </div>
        ) : (
          <>
            {/* Page navigation */}
            <div className="flex items-center justify-between mb-6">
              <div className="text-sm text-muted-foreground">
                Tiers {page * tiersPerPage + 1}–
                {Math.min((page + 1) * tiersPerPage, 50)}
              </div>
            ...
            {!progress.isPremium && (
              <div className="mt-8 rounded-xl border border-amber-500/40 bg-gradient-to-br from-amber-950/40 to-purple-950/20 p-6 text-center">
                ...
                <p className="text-muted-foreground text-sm mb-4">
                  Unlock exclusive badges, bonus SKY444, and premium-only tier
                  rewards across all 50 tiers.
                </p>
                <button
                  onClick={() =>
