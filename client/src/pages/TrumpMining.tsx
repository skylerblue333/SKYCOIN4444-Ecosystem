...
const MINING_RATES: Record<
  string,
  { rate: number; power: number; cost: number; label: string; color: string }
> = {
  basic: {
    rate: 0.001,
    power: 10,
...
export default function TrumpMining() {
  ...
  const [mined, setMined] = useState(0.0);
  ...
  const startMining = () => {
    if (!isAuthenticated) {
      toast.error("Sign in to start mining");
      return;
    }
    setIsMining(true);
  ...
  const upgradeRig = (rigId: string) => {
    ...
    if (rigBalance < r.cost) {
      toast.error(`Need ${r.cost} SKY444 to upgrade`);
      return;
    }
  ...
  const activateBoost = (boostId: string) => {
    ...
    if (!boost) return;
    if (boost.cost > 0 && rigBalance < boost.cost) {
      toast.error(`Need ${boost.cost} SKY444`);
      return;
    }
  ...
  useEffect(() => {
    const t = setInterval(() => {
      setNetworkDiff(d =>
        parseFloat((d + (Math.random() - 0.5) * 0.5).toFixed(1))
      );
    }, 5000);
              ...
              <div className="lg:col-span-2 space-y-4">
                ...
                <div className="bg-[#0e0a1a] border border-orange-500/20 rounded-2xl p-6">
                  <div className="text-center mb-6">
                    <div
                      className={`text-6xl mb-2 transition-all duration-300 ${isMining ? "animate-bounce" : ""}`}
                    >
                      {isMining ? "⛏️" : "🔒"}
                    </div>
                  ...
                  <div className="bg-black/40 rounded-xl p-3 mb-4 font-mono text-xs text-center">
                    ...
                    <span
                      className={`${isMining ? "text-green-400" : "text-slate-700"}`}
                    >
                      {displayHash}
                    </span>
                  </div>
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[
                    ...
                    },
                    {
                      ...
                      value: `${dailyEarnings.toFixed(4)} TRUMP`,
                      icon: TrendingUp,
                      color: "text-orange-400",
                    },
                    {
                      label: "Daily USD",
                      value: `$${dailyUSD.toFixed(2)}`,
                      icon: Coins,
                      color: "text-yellow-400",
                    },
                    {
                      label: "Boost Multiplier",
                      value: `${boostMult.toFixed(1)}x`,
                      icon: Zap,
                      color: "text-purple-400",
                    },
                  ].map(stat => (
                    <div
                      key={stat.label}
                      className="bg-[#0e0a1a] border border-white/5 rounded-xl p-3 flex items-center gap-3"
                    >
              ...
              <div className="space-y-4">
                <div className="bg-[#0e0a1a] border border-white/5 rounded-2xl p-4">
                  ...
                  <div className="text-center py-3">
                    ...
                    <p className={`font-bold ${currentRig.color}`}>
                      {currentRig.label}
                    </p>
                    <p className="text-slate-500 text-xs mt-1">
                      {currentRig.power} H/s
                    </p>
                    <p className="text-orange-400 text-xs">
                      {currentRig.rate}/sec base rate
                    </p>
                  </div>
                    ...
                    <div className="space-y-2">
                      {activeBoosts.map(bid => {
                        ...
                        return b ? (
                          <div
                            key={bid}
                            className="flex items-center justify-between text-xs bg-purple-500/10 rounded-lg px-2 py-1.5"
                          >
                            <span>
                              {b.icon} {b.name}
                            </span>
          ...
          <TabsContent value="rigs">
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {Object.entries(MINING_RATES).map(([rigId, rigData]) => (
                <Card
                  key={rigId}
                  className={`bg-[#0e0a1a] border transition-all ${rig === rigId ? "border-orange-500/40 shadow-lg shadow-orange-900/20" : "border-white/5 hover:border-white/10"}`}
                >
                  <CardContent className="p-5">
                    <div className="text-center mb-4">
                      <div className="text-4xl mb-2">🖥️</div>
                      <h3 className={`font-bold text-lg ${rigData.color}`}>
                        {rigData.label}
                      </h3>
                      {rig === rigId && (
                    ...
                    <div className="space-y-2 mb-4">
                      ...
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="text-orange-400 font-semibold">
                          {rigData.rate} TRUMP/s
                        </span>
                      </div>
                      <div className="flex justify-between text-xs">
                        <span className="text-slate-500">Daily (base)</span>
                        <span className="text-green-400 font-semibold">
                          {(rigData.rate * 86400).toFixed(2)} TRUMP
                        </span>
                      </div>
                      <div className="flex justify-between text-xs">
                        ...
                        <span className="text-yellow-400 font-semibold">
                          $
                          {(rigData.rate * 86400 * TRUMP_PRICE_MOCK).toFixed(2)}
                        </span>
                      </div>
                    </div>
                    {rig === rigId ? (
                      <Button
                        className="w-full bg-orange-500/20 text-orange-400 border border-orange-500/30 text-sm"
                        disabled
                      >
                        Currently Active
                      </Button>
                      ...
                      <Button
                        className="w-full bg-green-500/20 text-green-400 border border-green-500/30 text-sm"
                        onClick={() => upgradeRig(rigId)}
                      >
                        Free — Activate
                      </Button>
                    ) : (
                      <Button
                        className={`w-full text-sm ${rigBalance >= rigData.cost ? "bg-gradient-to-r from-orange-500/20 to-yellow-500/20 text-yellow-300 border border-yellow-500/30 hover:from-orange-500/30 hover:to-yellow-500/30" : "bg-white/5 text-slate-600 border border-white/5"}`}
                        onClick={() => upgradeRig(rigId)}
                        disabled={rigBalance < rigData.cost}
                      >
                        {rigBalance >= rigData.cost ? (
                          `Upgrade — ${rigData.cost} SKY444`
                        ) : (
                          <>
                            <Lock className="w-3 h-3 mr-1" />
                            {rigData.cost} SKY444 needed
                          </>
                        )}
                      </Button>
                    )}
          ...
          <TabsContent value="boosts">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {BOOSTS.map(boost => (
                <Card
                  key={boost.id}
                  className={`bg-[#0e0a1a] border transition-all ${activeBoosts.includes(boost.id) ? "border-purple-500/40" : "border-white/5 hover:border-white/10"}`}
                >
                  <CardContent className="p-5">
                    <div className="flex items-start gap-3 mb-4">
                      ...
                      <Badge className="ml-auto bg-purple-500/20 text-purple-400 border-purple-500/30 shrink-0">
                        {boost.mult}x
                      </Badge>
                    </div>
                    {activeBoosts.includes(boost.id) ? (
                      <Button
                        className="w-full bg-purple-500/20 text-purple-400 border border-purple-500/30"
                        disabled
                      >
                        ✓ Active
                      </Button>
                      ...
                      <Button
                        className="w-full bg-white/5 hover:bg-white/10 text-slate-300 border border-white/10 text-sm"
                        onClick={() => activateBoost(boost.id)}
                      >
                        {boost.cost > 0
                          ? `Activate — ${boost.cost} SKY444`
                          : "Activate Free"}
                      </Button>
                    )}
                  </CardContent>
            ...
            <div className="mt-4 p-4 bg-orange-500/5 border border-orange-500/20 rounded-xl">
              ...
              <p className="text-orange-400/60 text-xs">
                ...
                {BOOSTS.reduce((m, b) => m * b.mult, 1)}x multiplier. At Quantum
                Rig + all boosts, you earn{" "}
                {(
                  MINING_RATES.quantum.rate *
                  BOOSTS.reduce((m, b) => m * b.mult, 1) *
                  86400
                ).toFixed(2)}{" "}
                TRUMP/day ($
                {(
            ...
            <div className="bg-[#0e0a1a] border border-white/5 rounded-2xl overflow-hidden">
              ...
              <div className="divide-y divide-white/5">
                {LEADERBOARD.map(entry => (
                  ...
                  >
                    <div
                      className={`w-8 h-8 rounded-full flex items-center justify-center font-black text-sm ${entry.rank === 1 ? "bg-yellow-500/20 text-yellow-400" : entry.rank === 2 ? "bg-slate-400/20 text-slate-300" : entry.rank === 3 ? "bg-orange-700/20 text-orange-400" : "bg-white/5 text-slate-500"}`}
                    >
                      {entry.rank === 1
                        ? "🥇"
                        : entry.rank === 2
                    ...
                    <div className="flex-1">
                      ...
                      <p
                        className={`text-xs ${MINING_RATES[entry.rig]?.color || "text-slate-500"}`}
                      >
                        {MINING_RATES[entry.rig]?.label}
                      </p>
                    </div>
                    <div className="text-right">
                      <p className="text-orange-400 font-bold text-sm">
                        {entry.mined.toLocaleString()} TRUMP
                      </p>
                      <p className="text-slate-600 text-xs">
                        ${(entry.mined * TRUMP_PRICE_MOCK).toLocaleString()}
                      </p>
                    </div>
                  </div>
                ))}
