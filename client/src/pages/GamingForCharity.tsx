    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="relative overflow-hidden border-b border-border/50 bg-gradient-to-br from-green-500/5 via-background to-purple-500/5">
        ...
        <div className="container py-12 relative z-10">
          <div className="flex items-center gap-2 mb-4">
            <Badge
              variant="outline"
              className="text-purple-400 border-purple-500/30 font-mono"
            >
              PLAY TO DONATE
            </Badge>
            <Badge
              variant="outline"
              className="text-yellow-400 border-yellow-500/30 font-mono"
            >
              LIVE
            </Badge>
          </div>
          ...
          <p className="text-lg text-muted-foreground mb-6 max-w-2xl">
            Play games, earn points, and automatically donate to verified
            charities. Every game you play generates real SKY444 donations.
          </p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            {[
              {
                label: "Total Donated",
                value: `${(totalDonated / 1000).toFixed(1)}K SKY444`,
                icon: Heart,
                color: "text-red-400",
                bg: "bg-red-500/10",
              },
              ...
              return (
                <div
                  key={stat.label}
                  className={`rounded-xl border border-border/50 ${stat.bg} p-4 flex items-center gap-3`}
                >
      ...
      <div className="container py-8">
        ...
        <div className="flex gap-1 border-b border-border/50 mb-8">
          {(["games", "charities", "leaderboard"] as const).map(tab => (
            <button
              ...
              className={`px-5 py-2.5 text-sm font-medium border-b-2 transition-all -mb-px capitalize ${activeTab === tab ? "border-purple-500 text-purple-400" : "border-transparent text-muted-foreground hover:text-foreground"}`}
            >
              {tab}
            </button>
          ))}
        ...
        {activeTab === "games" && (
          <div>
            ...
            <div className="flex flex-wrap gap-2 mb-6">
              {["all", "quiz", "arcade", "puzzle", "runner", "memory"].map(
                f => (
                  <button
                    key={f}
                    onClick={() => setFilter(f)}
                    className={`px-3 py-1.5 rounded-xl border text-sm capitalize transition-all ${filter === f ? "border-purple-500/50 bg-purple-600/10 text-purple-400" : "border-border/50 text-muted-foreground hover:border-border"}`}
                  >
                    {f}
                  </button>
                )
              ...
              <div className="grid md:grid-cols-3 gap-4">
                {GAMES.filter(g => g.featured).map(game => (
                  <Link key={game.id} href={game.href}>
                    <div
                      className={`rounded-xl border ${game.border} bg-gradient-to-br ${game.color} p-5 hover:scale-[1.02] transition-all cursor-pointer group`}
                    >
                      <div className="flex items-start justify-between mb-3">
                        <span className="text-3xl">{game.emoji}</span>
                        <Badge
                          variant="outline"
                          className={`text-xs ${game.iconColor} border-current`}
                        >
                          {game.category}
                        </Badge>
                      </div>
                      ...
                      <div className="flex items-center gap-2 p-2 rounded-lg bg-background/30 border border-border/20">
                        ...
                        <div className="flex-1">
                          ...
                          <p className={`text-xs ${game.iconColor} font-mono`}>
                            {game.donated.toLocaleString()} SKY444 raised
                          </p>
                        </div>
                    ...
                    <div className="rounded-xl border border-border/50 bg-card/30 p-4 hover:border-purple-500/30 hover:bg-purple-600/5 transition-all cursor-pointer flex items-center gap-4">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-3 mt-1.5 text-xs text-muted-foreground">
                          ...
                          <span
                            className={`flex items-center gap-1 ${game.iconColor}`}
                          >
                            <Heart className="h-3 w-3" />
                            {(game.donated / 1000).toFixed(1)}K donated
                          </span>
                        </div>
                      </div>
        ...
        {activeTab === "charities" && (
          <div className="space-y-4">
            <p className="text-muted-foreground text-sm mb-6">
              All donations are verified on-chain. 95% of every SKY444 earned
            ...
            {CHARITIES.map(charity => {
              ...
              return (
                <div
                  key={charity.name}
                  className={`rounded-xl border ${charity.border} ${charity.bg} p-5`}
                >
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div className="text-right">
                      <p className={`text-lg font-bold ${charity.color}`}>
                        {charity.raised.toLocaleString()}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        of {charity.goal.toLocaleString()} SKY444
                      </p>
                    </div>
                  </div>
        ...
        {activeTab === "leaderboard" && (
          <div>
            <p className="text-muted-foreground text-sm mb-6">
              Top donors this month. Play more games to climb the leaderboard
            ...
            <div className="space-y-3">
              {LEADERBOARD.map(player => (
                <div
                  key={player.rank}
                  className={`rounded-xl border p-4 flex items-center gap-4 ${player.rank <= 3 ? "border-yellow-500/30 bg-yellow-500/5" : "border-border/50 bg-card/30"}`}
                >
                  <div className="w-8 text-center">
                    <span className="text-lg">{player.badge}</span>
                  </div>
                  <div className="w-9 h-9 rounded-full bg-primary/20 border border-primary/30 flex items-center justify-center font-bold text-sm">
                    {player.avatar}
                  </div>
                  <div className="flex-1">
