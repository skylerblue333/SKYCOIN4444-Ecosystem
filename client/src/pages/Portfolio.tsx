    ...
    <div className="container py-8 max-w-4xl animate-page-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <StatCard
          ...
          change={(totalChange / totalValue) * 100}
          color="success"
        />
        <StatCard
          icon={BarChart3}
      ...
      {view === "holdings" && (
        <div className="space-y-2">
          {HOLDINGS.map(h => (
            <Link
              key={h.symbol}
              href={h.symbol === "SKY444" ? "/token-metrics" : "/crypto-hub"}
            >
              <div className="card p-4 flex items-center gap-4 hover:border-slate-700/60 active:scale-[0.99] transition-all cursor-pointer">
                <div
                  className={`w-10 h-10 rounded-full ${h.color} flex items-center justify-center text-white font-bold text-xs shrink-0`}
                >
                  {h.symbol.slice(0, 2)}
                </div>
                ...
                <div className="text-right shrink-0">
                  ...
                  <div
                    className={`text-xs font-medium flex items-center justify-end gap-0.5 ${h.change >= 0 ? "text-green-400" : "text-red-400"}`}
                  >
                    {h.change >= 0 ? (
        ...
        <div className="space-y-2">
          {HISTORY.map((h, i) => (
            <div key={i} className="card p-4 flex items-center gap-4">
              ...
              <div className="text-sm font-semibold w-20 text-right shrink-0">
                ${h.value.toLocaleString()}
              </div>
              <div
                className={`text-xs font-medium w-16 text-right shrink-0 ${h.change >= 0 ? "text-green-400" : "text-red-400"}`}
              >
                {h.change >= 0 ? "+" : ""}${h.change}
              </div>
            </div>
