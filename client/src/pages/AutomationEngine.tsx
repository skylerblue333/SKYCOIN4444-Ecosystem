          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-white/40">
                Workflow builder · Trigger system · Job manager · Feature flags
                · Kill-switch
              </p>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-6 py-6">
        <Tabs defaultValue="workflows">
                ...
                <Card key={wf.id} className="bg-white/5 border-white/10">
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-2 h-2 rounded-full ${wf.status === "active" ? "bg-[oklch(0.75_0.2_145)] animate-pulse" : "bg-white/20"}`}
                        />
                        <div>
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <Badge
                          variant="outline"
                          className={`text-[9px] ${statusColor[wf.status]}`}
                        >
                          {wf.status}
                        </Badge>
                        <div className="flex gap-1">
                          ...
                          <Button
                            size="sm"
                            variant="ghost"
                            className="h-6 w-6 p-0 text-white/40 hover:text-[oklch(0.7_0.2_25)]"
                          >
                  ...
                  <CardContent className="p-4">
                    <div className="flex items-center justify-between mb-2">
                      <div className="flex items-center gap-2">
                        {job.status === "running" && (
                          <RefreshCw className="w-3 h-3 text-[oklch(0.8_0.15_200)] animate-spin" />
                        )}
                        {job.status === "completed" && (
                          <CheckCircle className="w-3 h-3 text-[oklch(0.75_0.2_145)]" />
                        )}
                        {job.status === "failed" && (
                          <XCircle className="w-3 h-3 text-[oklch(0.7_0.2_25)]" />
                        )}
                        {job.status === "queued" && (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <Badge
                          variant="outline"
                          className={`text-[9px] ${statusColor[job.status]}`}
                        >
                          {job.status}
                        </Badge>
                      </div>
                  ...
                  <CardContent className="p-4 flex items-center justify-between">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <Badge
                          variant="outline"
                          className="text-[9px] text-white/40 border-white/20"
                        >
                          {flag.module}
                        </Badge>
          ...
          <TabsContent value="rate-limits">
            ...
            {/* Kill switch panel */}
            <Card
              className={`mt-4 border-[oklch(0.7_0.2_25)]/30 ${killActive ? "bg-[oklch(0.7_0.2_25)]/10" : "bg-white/5"}`}
            >
              <CardContent className="p-4">
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-xs text-white/50 mb-3">
                      Activating the kill-switch will: freeze all payments,
                      pause AI processing, disable action execution, and put the
                      platform in read-only mode. Use only in emergencies.
                    </p>
                    <Button
                      variant="outline"
                      size="sm"
