...
export function ReputationSection() {
  ...
  const board = trpc.hopeIntelligence.reputation.leaderboard.useQuery({
    limit: 10,
  });
    ...
    <div className="space-y-6">
      <Card className="border-white/10 bg-white/[0.02]">
        <CardHeader className="flex flex-row items-center justify-between space-y-0">
          ...
          >
            <RefreshCw
              className={`h-3.5 w-3.5 mr-1.5 ${recompute.isPending ? "animate-spin" : ""}`}
            />
            Recompute
          </Button>
        </CardHeader>
        <CardContent>
          {!r ? (
            <p className="text-sm text-white/40">
              No score yet. Click Recompute to generate it from your real
              activity.
            </p>
          ) : (
            <>
              <div className="flex items-end gap-2 mb-6">
                <span className="text-5xl font-black" style={{ color: GOLD }}>
                  {r.overall}
                </span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-5 gap-4">
                {(
                  [
                    ...
                    ["Builder", r.builderScore],
                    ["Teaching", r.teachingScore],
              ...
              >
                <div className="flex items-center gap-3">
                  ...
                  <span className="text-sm text-white/80">
                    {u.name ?? u.username ?? `User ${u.userId}`}
                  </span>
                </div>
