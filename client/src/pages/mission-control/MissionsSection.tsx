...
function MissionCard({ id }: { id: number }) {
  ...
  const detail = trpc.hopeIntelligence.missions.get.useQuery(
    { id },
    { enabled: open }
  );
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-white/[0.02]">
      ...
      >
        ...
        <ChevronDown
          className={`h-4 w-4 text-white/40 ml-3 transition-transform ${open ? "rotate-180" : ""}`}
        />
      </button>
      {open && (
              ...
              {detail.data.steps.map(s => (
                <li key={s.id} className="flex items-start gap-2">
                  <Checkbox
                    checked={s.done}
                    onCheckedChange={v =>
                      toggle.mutate({ missionId: id, stepId: s.id, done: !!v })
                    }
                    className="mt-0.5"
                  />
                  <div>
                    <span
                      className={`text-sm ${s.done ? "text-white/40 line-through" : "text-white/80"}`}
                    >
                      {s.title}
                    </span>
                    {s.detail && (
...
export function MissionsSection() {
  ...
  const [category, setCategory] =
    useState<(typeof CATEGORIES)[number]>("skill");
  const list = trpc.hopeIntelligence.missions.list.useQuery();
  ...
  return (
    <div className="space-y-6">
      <Card className="border-white/10 bg-white/[0.02]">
        <CardHeader>
          <CardTitle className="text-white">Start a Mission</CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex flex-col sm:flex-row gap-2">
            <Input
              ...
              placeholder="e.g. Learn React Native and ship an app"
            ...
            <Button
              style={{ backgroundColor: GOLD, color: "#000" }}
              disabled={create.isPending || title.trim().length < 4}
              onClick={() => create.mutate({ title: title.trim(), category })}
            >
              {create.isPending ? (
