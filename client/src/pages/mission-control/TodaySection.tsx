...
export function TodaySection() {
  ...
  const { data, isLoading, error } =
    trpc.hopeIntelligence.missionControl.today.useQuery({ withSuggestions });
    ...
    <div className="space-y-6">
      ...
      <Card className="border-white/10 bg-gradient-to-br from-amber-500/[0.06] to-transparent">
        <CardHeader className="flex flex-row items-center justify-between space-y-0">
          ...
          {!withSuggestions && (
            <Button
              ...
              onClick={() => setWithSuggestions(true)}
            >
              Generate
            </Button>
          )}
        ...
        <CardContent>
          {!withSuggestions ? (
            ...
            <p className="text-sm text-white/40">
              No suggestions right now — add a goal or mission to get tailored
              guidance.
            </p>
          ) : (
      ...
      <div className="grid lg:grid-cols-2 gap-4">
        ...
        <Card className="border-white/10 bg-white/[0.02]">
          <CardHeader>
            <CardTitle className="text-white text-base">Active Goals</CardTitle>
          </CardHeader>
      ...
      {data.reputation && (
        <Card className="border-white/10 bg-white/[0.02]">
          ...
          <CardContent className="grid grid-cols-2 md:grid-cols-5 gap-3">
            {(
              [
                ["Learning", data.reputation.learning],
                ["Builder", data.reputation.builder],
                ["Teaching", data.reputation.teaching],
                ["Community", data.reputation.community],
                ["Trust", data.reputation.trust],
              ] as const
            ).map(([label, v]) => (
              <div key={label}>
          ...
          {data.networkSuggestions.length === 0 ? (
            ...
            <div className="flex flex-wrap gap-2">
              {data.networkSuggestions.map(n => (
                <div
                  key={n.userId}
                  className="flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.03] pl-1 pr-3 py-1"
                >
                  <div className="h-7 w-7 rounded-full bg-white/10 flex items-center justify-center text-xs text-white/70 overflow-hidden">
                    {n.avatar ? (
                      ...
                      (n.name ?? n.username ?? "?").slice(0, 1).toUpperCase()
                    )}
                  </div>
                  <span className="text-sm text-white/80">
                    {n.name ?? n.username ?? `User ${n.userId}`}
                  </span>
                  <span className="text-xs text-white/40">
                    · {n.mutualCount} mutual
                  </span>
                </div>
              ))}
            </div>
