...
export default function CreatorDashboard() {
  ...
  const { data: earnings, isLoading: earningsLoading } =
    trpc.creator.earnings.useQuery(undefined, { enabled: isAuthenticated });
  const { data: subscriptions } = trpc.creator.mySubscriptions.useQuery(
    undefined,
    { enabled: isAuthenticated }
  );
  const { data: tips } = trpc.creator.myTips.useQuery(undefined, {
    enabled: isAuthenticated,
  });
      ...
      <div className="min-h-screen flex items-center justify-center">
        <div className="p-8 text-center max-w-md rounded-xl border border-border/50 bg-card/80">
          ...
          <p className="text-muted-foreground mb-6">
            Sign in to access your creator tools, analytics, and monetization
            settings.
          </p>
          <a href={getLoginUrl()}>
      ...
      <div className="container max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="p-4 rounded-xl border border-border/50 bg-card/80">
            ...
            <div className="text-2xl font-bold font-mono">
              ...
              <span className="text-xs text-muted-foreground">SKY444</span>
            </div>
        ...
        <Tabs defaultValue="monetization" className="w-full">
          <TabsList className="w-full bg-card/80 border border-border/50">
            <TabsTrigger value="monetization" className="flex-1">
              Monetization
            </TabsTrigger>
              ...
              <div className="p-5 rounded-xl border border-border/50 bg-card/80">
                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between p-3 rounded-lg bg-background/50 border border-border/30">
                    ...
                    <Badge
                      className={`text-[10px] ${subCount > 0 ? "bg-purple-600/10 text-purple-400 border-purple-500/30" : ""}`}
                    >
              ...
              <div className="p-5 rounded-xl border border-border/50 bg-card/80">
                ...
                <div className="space-y-3">
                  <div className="p-3 rounded-lg bg-background/50 border border-border/30">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Creators who post 3+ times per week earn 4x more on
                      average.
                    </p>
                  </div>
                  <div className="p-3 rounded-lg bg-background/50 border border-border/30">
          ...
          <TabsContent value="subscriptions" className="mt-4">
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
              {[
                ...
                <div
                  key={plan.tier}
                  className={`p-5 rounded-xl border ${plan.color}`}
                >
            ...
            <div className="p-5 rounded-xl border border-border/50 bg-card/80">
              <h3 className="font-semibold mb-4 flex items-center gap-2">
                <Users className="w-4 h-4 text-primary" /> Active Subscribers (
              ...
              {subscriptions && subscriptions.length > 0 ? (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="text-[10px] text-muted-foreground flex items-center gap-1">
                            <Clock className="w-2.5 h-2.5" /> Since{" "}
                            {new Date(sub.createdAt).toLocaleDateString()}
                          </div>
                        </div>
                      </div>
                      <Badge
                        className={`text-[10px] ${sub.tier === "vip" ? "bg-yellow-500/10 text-yellow-400 border-yellow-500/30" : sub.tier === "premium" ? "bg-primary/10 text-primary border-primary/30" : "bg-muted/20"}`}
                      >
                        {sub.tier}
                      </Badge>
                    </div>
                ...
                <div className="text-center py-8">
                  ...
                  <p className="text-sm text-muted-foreground">
                    No subscribers yet. Share your profile to attract
                    supporters!
                  </p>
                </div>
              )}
          ...
          <TabsContent value="tipping" className="mt-4">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="p-5 rounded-xl border border-border/50 bg-card/80">
                <h3 className="font-semibold mb-4 flex items-center gap-2">
                  <Heart className="w-4 h-4 text-red-400" /> Recent Tips (
                  {tips?.length ?? 0})
                </h3>
                {tips && tips.length > 0 ? (
                  <div className="space-y-2">
            ...
            <div className="p-5 rounded-xl border border-border/50 bg-card/80">
              ...
              <div className="space-y-3">
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    ...
                    <span className="font-mono">
                      {(earnings?.subRevenue ?? 0).toLocaleString()} SKY444
                    </span>
                  </div>
                  <div className="h-2 bg-background/50 rounded-full overflow-hidden border border-border/30">
                    <div
                      ...
                      style={{
                        width:
                          totalEarnings > 0
                            ? `${((earnings?.subRevenue ?? 0) / totalEarnings) * 100}%`
                            : "0%",
                      }}
                    />
                  </div>
                ...
                <div>
                  <div className="flex justify-between text-xs mb-1">
                    ...
                    <span className="font-mono">
                      {tipRevenue.toLocaleString()} SKY444
                    </span>
                  </div>
                  <div className="h-2 bg-background/50 rounded-full overflow-hidden border border-border/30">
                    <div
              ...
              <div className="mt-6 p-3 rounded-lg bg-primary/5 border border-primary/20">
                <div className="flex items-center gap-2 text-xs text-muted-foreground">
                  ...
                  <span>
                    Detailed analytics charts will appear as you accumulate more
                    data.
                  </span>
                </div>
              </div>
