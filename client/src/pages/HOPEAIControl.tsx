...
export default function HOPEAIControl() {
  ...
  const toggleAgent = (id: string) => {
    ...
    toast.success(
      `${agent?.name} ${agentStates[id] === "active" ? "paused" : "activated"}`
    );
  };
      ...
      <div className="sticky top-0 z-10 bg-background/90 backdrop-blur border-b border-border px-4 py-3 flex items-center gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="text-xs text-muted-foreground">
            Agent management, ethics & deployment
          </p>
        </div>
      ...
      <div className="max-w-4xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            {
              label: "Avg Load",
              value: `${Math.round(HOPE_AGENTS.reduce((s, a) => s + a.load, 0) / HOPE_AGENTS.length)}%`,
              color: "text-yellow-400",
            },
            {
              label: "Ethics Checks",
              value: Object.values(ethics).filter(Boolean).length.toString(),
        ...
        <div className="bg-secondary/20 border border-border/50 rounded-xl overflow-hidden">
          <div className="px-4 py-3 border-b border-border/50 flex items-center justify-between">
            ...
            <button
              onClick={() => toast.info("Restarting all agents…")}
          ...
          <div className="divide-y divide-border/30">
            {HOPE_AGENTS.map(agent => (
              <div
                key={agent.id}
                className="px-4 py-3 flex items-center gap-3 hover:bg-secondary/20 transition-colors"
              >
                <div
              ...
              >
                ...
                <button
                  onClick={() => {
                    setEthics(prev => ({
                      ...prev,
                      [ctrl.label]: !prev[ctrl.label],
                    }));
                    toast.success(
                      `${ctrl.label} ${ethics[ctrl.label] ? "disabled" : "enabled"}`
                    );
                  }}
                  className={`relative w-10 h-5 rounded-full transition-colors ${ethics[ctrl.label] ? "bg-green-500" : "bg-secondary"}`}
                >
                  <div
                    className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform ${ethics[ctrl.label] ? "translate-x-5" : "translate-x-0.5"}`}
                  />
                </button>
              </div>
            ))}
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="text-center">
                <p className="text-lg font-bold text-blue-400">
                  {(simData as any).entities?.length ?? 0}
                </p>
                <p className="text-xs text-muted-foreground">Entities</p>
              </div>
              <div className="text-center">
                <p className="text-lg font-bold text-purple-400">
                  {(simData as any).events?.length ?? 0}
                </p>
                <p className="text-xs text-muted-foreground">Events</p>
              </div>
              <div className="text-center">
                <p className="text-lg font-bold text-green-400">
                  {(simData as any).tick ?? 0}
                </p>
