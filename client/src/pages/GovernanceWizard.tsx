    ...
    return (
      <div className="max-w-2xl mx-auto px-4 py-6 text-center">
        ...
        <p className="text-muted-foreground mb-6">
          Your governance proposal is now live for community voting. Voting
      ...
      <div className="flex items-center gap-2 mb-8">
        {STEPS.map((s, i) => (
          <div key={i} className="flex items-center gap-2">
            <div
              className={`w-8 h-8 rounded-full flex items-center justify-center text-sm font-bold transition-all ${i < step ? "bg-purple-600 text-white" : i === step ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}
            >
              {i < step ? "✓" : i + 1}
            </div>
            {i < STEPS.length - 1 && (
              <div
                className={`h-0.5 w-8 transition-all ${i < step ? "bg-purple-600" : "bg-border"}`}
              />
            )}
          </div>
        ))}
            ...
            <div className="space-y-3">
              ...
              <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-lg p-4 text-sm">
                ...
                <p className="text-muted-foreground">
                  You need 1,000 SKY444 staked to submit a proposal. Your
                  current stake: 2,500 SKY444 ✓
                </p>
              </div>
            </div>
          </div>
