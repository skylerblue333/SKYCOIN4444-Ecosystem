    ...
    <div className="container py-8 max-w-6xl animate-page-in">
      ...
      <div className="flex items-center justify-between mb-6 card p-4">
        <div className="flex items-center gap-3">
          <div
            className={`w-3 h-3 rounded-full ${healingActive ? "bg-green-400 animate-pulse" : "bg-red-400"}`}
          />
          <span className="font-medium">Self-Healing Engine</span>
          <span className="text-sm text-muted-foreground">
            {healingActive ? "Active — monitoring all services" : "Disabled"}
          </span>
        </div>
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => {
              setHealingActive(!healingActive);
              toast.success(
                `Self-healing ${!healingActive ? "enabled" : "disabled"}`
              );
            }}
          ...
          {SERVICES.map(svc => (
            <div key={svc.id} className="card p-4">
              <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                  <div
                    className={`w-2 h-2 rounded-full ${svc.status === "healthy" ? "bg-green-400" : "bg-yellow-400 animate-pulse"}`}
                  />
                  <span className="font-medium text-sm">{svc.name}</span>
                </div>
                <span
                  className={`text-xs px-2 py-0.5 rounded-full ${svc.status === "healthy" ? "bg-green-500/20 text-green-400" : "bg-yellow-500/20 text-yellow-400"}`}
                >
                  {svc.status}
                </span>
              </div>
              <div className="grid grid-cols-3 gap-2 text-xs text-muted-foreground">
                ...
                <span>
                  Restarts:{" "}
                  <span
                    className={`font-medium ${svc.restarts > 0 ? "text-yellow-400" : "text-foreground"}`}
                  >
                    {svc.restarts}
                  </span>
                </span>
