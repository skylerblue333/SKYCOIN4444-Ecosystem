...
export default function InvestorRoom() {
  ...
  const { data: kpis, isLoading: kpisLoading } = trpc.investor.kpis.useQuery(
    undefined,
    {
      enabled: !!user,
      retry: false,
  ...
  const { data: revenue, isLoading: revenueLoading } =
    trpc.investor.revenue.useQuery(undefined, {
      enabled: !!user,
      retry: false,
    });

  const { data: treasury, isLoading: treasuryLoading } =
    trpc.investor.treasury.useQuery(undefined, {
      enabled: !!user,
      retry: false,
    });
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {isLoading
              ...
              : [
                  {
                    label: "Total Users",
                    value: (kpis as any)?.totalUsers?.toLocaleString() || "0",
                    icon: Users,
                    color: "text-blue-400",
                  },
                  {
                    label: "Monthly Revenue",
                    value: `$${((revenue as any)?.total || 0).toLocaleString()}`,
                    icon: DollarSign,
                    color: "text-emerald-400",
                  },
                  {
                    label: "Treasury",
                    value: `$${((treasury as any)?.total || 0).toLocaleString()}`,
                    icon: Coins,
                  ...
                  },
                  {
                    ...
                    value:
                      (kpis as any)?.activeStakers?.toLocaleString() || "0",
                    icon: Zap,
            ...
            <div className="pt-2 border-t border-zinc-800 grid grid-cols-3 gap-4 text-center">
              ...
              <div>
                <div className="text-lg font-bold text-emerald-400">
                  {(
                    tokenomicsData as any
                  )?.circulatingSupply?.toLocaleString() || "~1.2B"}
                </div>
                <div className="text-xs text-zinc-500">Circulating</div>
              </div>
              <div>
                <div className="text-lg font-bold text-yellow-400">
                  ${(tokenomicsData as any)?.price?.toFixed(4) || "0.0044"}
                </div>
              ...
              {ROADMAP.map(phase => (
                ...
                >
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    {phase.status === "active" && (
                      <Zap className="w-4 h-4 text-violet-400 ml-auto animate-pulse" />
                    )}
                  </div>
        ...
        <Card className="bg-gradient-to-r from-violet-600/20 to-blue-600/20 border-violet-500/30">
          <CardContent className="p-6 flex items-center justify-between gap-4">
            <div>
              ...
              <p className="text-sm text-zinc-400 mt-1">
                Contact the team for private sale allocation and partnership
                opportunities.
              </p>
            </div>
