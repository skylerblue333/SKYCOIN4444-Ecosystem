...
const AiCorePage: React.FC = () => {
  const [activeTab, setActiveTab] = useState<
    "chat" | "content" | "analysis" | "learn" | "logs"
  >("chat");
  const [chatMessage, setChatMessage] = useState("");
  const [contentType, setContentType] = useState<
    "blog" | "social" | "email" | "code" | "documentation"
  >("blog");
        ...
        <Card className="w-full max-w-md">
          ...
          <CardContent>
            <p className="text-sm text-muted-foreground">
              Please log in to access AI Core.
            </p>
          </CardContent>
        </Card>
      </div>
  ...
  return (
    <div className="flex-1 space-y-4 p-8 pt-6">
      ...
      <div className="flex gap-2 border-b overflow-x-auto">
        {(["chat", "content", "analysis", "learn", "logs"] as const).map(
          tab => (
            <button
              key={tab}
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            ...
            {scanMarketsMutation.data && (
              <div className="p-3 rounded-lg bg-muted max-h-96 overflow-y-auto">
                <p className="text-sm whitespace-pre-wrap">
                  {scanMarketsMutation.data.analysis}
                </p>
              </div>
            )}
          </CardContent>
                ...
                <div className="grid gap-4 md:grid-cols-3">
                  <div className="p-3 rounded-lg bg-muted">
                    ...
                    <p className="text-2xl font-bold">
                      {analyticsQuery.data.totalLogs}
                    </p>
                  </div>
                  <div className="p-3 rounded-lg bg-muted">
                    <p className="text-sm text-muted-foreground">Tokens Used</p>
                    <p className="text-2xl font-bold">
                      {analyticsQuery.data.totalTokens}
                    </p>
                  </div>
                  <div className="p-3 rounded-lg bg-muted">
                    <p className="text-sm text-muted-foreground">Types</p>
                    <p className="text-2xl font-bold">
                      {Object.keys(analyticsQuery.data.byType).length}
                    </p>
                  </div>
                </div>
              </CardContent>
