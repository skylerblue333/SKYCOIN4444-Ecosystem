    ...
    <div className="min-h-screen bg-[#050508] text-white">
      ...
      <div className="sticky top-0 z-50 border-b border-white/5 bg-[#050508]/95 backdrop-blur px-4 py-2 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            onClick={() => navigate("/")}
            className="px-3 py-1 rounded-lg text-xs text-muted-foreground hover:text-white transition-colors border border-white/10"
          >
            {isZh ? "全球版" : "Global"}
          </button>
        </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2 max-w-4xl mx-auto">
            {ADVANTAGES.map(a => {
              ...
              return (
                <div
                  key={a.en}
                  className="flex items-center gap-1.5 px-2 py-1.5 rounded-lg bg-white/5 border border-white/8 text-xs"
                >
            ...
            return (
              ...
              >
                <div className="flex items-center gap-3 mb-3">
                  <div
                    className={`w-10 h-10 rounded-xl ${m.bg} flex items-center justify-center border ${m.border}`}
                  >
                  ...
                  <ArrowRight
                    className={`w-4 h-4 ${m.color} ml-auto opacity-0 group-hover:opacity-100 transition-opacity`}
                  />
                </div>
                ...
                <div className="flex flex-wrap gap-1">
                  {(isZh ? m.features_zh : m.features_en).map(f => (
                    <span
                      key={f}
                      className={`text-xs px-2 py-0.5 rounded-full ${m.bg} ${m.color} border ${m.border}`}
                    >
                      {f}
                    </span>
                  ))}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-3">
            {INTEGRATIONS.map(i => (
              ...
              >
                ...
                <div className={`text-xs font-bold ${i.color}`}>
                  {isZh ? i.name : i.en}
                </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          {FEATURE_GROUPS.map(g => {
            ...
            return (
              <div
                key={g.en}
                className={`rounded-xl border border-white/10 ${g.bg} p-4`}
              >
                ...
                <div className="space-y-1.5">
                  {items.map((item, idx) => (
                    ...
                    >
                      <CheckCircle
                        className={`w-3 h-3 ${g.color} flex-shrink-0`}
                      />
                      {item}
                    </div>
                  ))}
        ...
        <div className="container max-w-5xl mx-auto px-4">
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-6 text-center">
            {[
              ...
              <div key={s.label_en}>
                <div className={`text-3xl font-black ${s.color} stat-number`}>
                  {isZh ? s.zh : s.en}
                </div>
