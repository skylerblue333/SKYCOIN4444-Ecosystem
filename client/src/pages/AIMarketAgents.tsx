...
function IcoStatsBanner() {
  const { data: stats, refetch } = trpc.aiMarket.getIcoStats.useQuery(
    undefined,
    {
  ...
  const rarity =
    RARITY_CONFIG[stats.rarityStatus as keyof typeof RARITY_CONFIG] ??
    RARITY_CONFIG.rare;
  const pct = Math.min(100, stats.percentRaised);

  return (
    <div
      className={`relative overflow-hidden rounded-2xl border ${rarity.border} ${rarity.bg} shadow-xl ${rarity.glow} p-5`}
    >
      ...
      <div className="relative flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <span
            className={`text-xs font-bold px-2.5 py-1 rounded-full border ${rarity.border} ${rarity.color}`}
          >
            ✦ {stats.rarityLabel}
          </span>
        </div>
      ...
      <div className="relative grid grid-cols-2 sm:grid-cols-4 gap-4 mb-4">
        <div>
          <div className="text-2xl font-black text-rainbow">
            ${(stats.totalRaisedUsd / 1000).toFixed(0)}K
          </div>
        ...
        <div>
          ...
          <div className="text-xs text-blue-400 mt-0.5">
            +{Math.floor(Math.random() * 12) + 3} today
          </div>
        </div>
        <div>
          <div className="text-2xl font-black text-amber-400">
            ${parseFloat(stats.tokenPriceUsd.toString()).toFixed(5)}
          </div>
          <div className="text-xs text-slate-400">SKY444 Price</div>
          <div
            className={`text-xs mt-0.5 flex items-center gap-0.5 ${stats.trendDirection === "up" ? "text-green-400" : "text-red-400"}`}
          >
        ...
        <div>
          ...
          <div className="text-xs text-purple-400 mt-0.5">
            {(stats.rewardPoolSky / 1_000_000).toFixed(0)}M SKY pool
          </div>
        </div>
      </div>
      ...
      <div className="relative">
        <div className="flex justify-between text-xs text-slate-400 mb-1">
          ...
          <span className="font-bold text-white">
            {pct.toFixed(1)}% of ${(stats.hardCapUsd / 1_000_000).toFixed(1)}M
            hard cap
          </span>
        </div>
      ...
      <div className="relative flex items-center gap-4 mt-4 pt-3 border-t border-slate-700/40">
          ...
          <div className="flex items-center gap-1">
            <div className="w-16 h-1.5 bg-slate-800 rounded-full overflow-hidden">
              <div
                ...
                style={{ width: `${stats.momentumScore}%` }}
              />
            </div>
        ...
        <div className="ml-auto">
          <Link href="/investor-portal">
            <Button
              size="sm"
              className="text-xs h-7 px-3 bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-white border-0"
            >
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-3 text-xs">
            ...
            <span className="text-slate-500">
              Signals: <span style={{ color }}>{agent.totalSignals}</span>
            </span>
          </div>
        </div>
      </div>
...
function SignalCard({ signal }: { signal: any }) {
  const cfg =
    SIGNAL_CONFIG[signal.signalType as keyof typeof SIGNAL_CONFIG] ??
    SIGNAL_CONFIG.watch;
  ...
  const timeAgo = new Date(signal.createdAt).toLocaleTimeString([], {
    hour: "2-digit",
    minute: "2-digit",
  });
    ...
    <div className="flex gap-3 p-3 rounded-xl bg-slate-900/40 border border-slate-800/60 hover:border-slate-700/60 transition-all duration-150">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 mb-1">
          <span
            className={`text-[10px] font-black px-1.5 py-0.5 rounded ${cfg.bg} ${cfg.color} flex items-center gap-1`}
          >
        ...
        <div className="flex items-center gap-2 mt-1.5">
          ...
          {Array.isArray(signal.tags) &&
            signal.tags.slice(0, 3).map((tag: string) => (
              <span
                key={tag}
...
export default function AIMarketAgents() {
  ...
  const { data: agents, refetch: refetchAgents } =
    trpc.aiMarket.getAgents.useQuery();
  const { data: signals, refetch: refetchSignals } =
    trpc.aiMarket.getSignals.useQuery(
      { limit: 30 },
      { refetchInterval: 15_000 }
    );
  const { data: activity, refetch: refetchActivity } =
    trpc.aiMarket.getActivity.useQuery(
      { limit: 20 },
      { refetchInterval: 15_000 }
    );
  ...
  const triggerCycle = trpc.aiMarket.triggerCycle.useMutation({
    onSuccess: data => {
      toast.success(`${data.agentName} cycle complete`, {
        description: `Generated ${data.signalsGenerated} signal. Stats updated.`,
      });
      refetchSignals();
      refetchActivity();
    },
          ...
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <p className="text-slate-400 text-lg max-w-2xl">
                Five specialized AI agents continuously analyze market
                conditions, generate signals, and drive the ICO momentum
                dashboard — giving investors real-time intelligence on SKY444.
              </p>
            </div>
            <div className="shrink-0 hidden sm:block">
              <div className="grid grid-cols-5 gap-1">
                {["🔮", "🐋", "💓", "🔭", "🛡️"].map((emoji, i) => (
                  <div
                    key={i}
                    className="w-10 h-10 rounded-xl bg-slate-800/60 border border-slate-700/40 flex items-center justify-center text-lg animate-hero-float"
      ...
      <div className="max-w-7xl mx-auto px-4 py-8 space-y-8">
        ...
        <div>
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-xs text-slate-500">
              Click an agent to run a market cycle
            </span>
          </div>
          ...
          <div>
            ...
            <div className="space-y-2 max-h-[600px] overflow-y-auto scrollbar-hide">
              {activity?.length ? (
                activity.map(act => {
                  const agentColor =
                    AGENT_COLORS[act.agentId] ?? "oklch(0.72 0.28 260)";
                  ...
                  return (
                    <div
                      key={act.id}
                      className="flex gap-2.5 p-2.5 rounded-lg bg-slate-900/40 border border-slate-800/50"
                    >
                      <div
                        className="w-6 h-6 rounded-md flex items-center justify-center text-xs shrink-0 border"
                        style={{
                          background: `${agentColor}20`,
                          borderColor: `${agentColor}40`,
                        }}
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1">
                          <span className="text-[9px] px-1 py-0.5 rounded bg-slate-800 text-slate-500 capitalize">
