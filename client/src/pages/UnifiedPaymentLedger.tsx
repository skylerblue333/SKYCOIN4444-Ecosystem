...
export default function UnifiedPaymentLedger() {
  ...
  const [filter, setFilter] = useState<
    "all" | "tip" | "subscription" | "marketplace" | "token" | "payout"
  >("all");
  ...
  const totalRevenue = REVENUE_DATA.reduce(
    (s, d) => s + d.tips + d.subs + d.marketplace + d.tokens,
    0
  );
  const weeklyNet = REVENUE_DATA.reduce(
    (s, d) => s + d.tips + d.subs + d.marketplace + d.tokens,
    0
  );
        ...
        {[
          {
            ...
            value: `$${(weeklyNet / 100).toFixed(0)}K`,
            icon: TrendingUp,
          ...
          <Card className="glass-card border-white/10">
            <CardHeader className="pb-2">
              <CardTitle className="text-sm flex items-center gap-2">
                <BarChart3 className="w-4 h-4 text-cyan-400" /> Revenue by
                Stream (7 days)
              </CardTitle>
            </CardHeader>
            <CardContent>
              ...
              >
                <CardContent className="p-4">
                  <div className="flex items-center gap-3">
                    <div
                      className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${typeColors[tx.type]}`}
                    >
                      <Icon
                        className={`w-4 h-4 ${typeColors[tx.type].split(" ")[0]}`}
                      />
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="font-semibold text-sm">{tx.from}</div>
                      <div className="text-xs text-white/40">
                        {tx.note || tx.type} · {tx.timestamp}
                      </div>
                    </div>
                    <div className="text-right flex-shrink-0">
                      <div
                        className={`font-bold font-mono text-sm ${isPositive ? "text-green-400" : "text-white/60"}`}
                      >
                        {tx.amount}
                      </div>
                      <div
                        className={`flex items-center gap-1 justify-end text-xs ${statusColors[tx.status]}`}
                      >
