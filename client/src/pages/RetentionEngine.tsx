...
export default function RetentionEngine() {
  const [tab, setTab] = useState<"loop" | "metrics" | "sticky" | "viral">(
    "loop"
  );
      ...
      <div className="sticky top-0 z-10 bg-background/80 backdrop-blur-sm border-b border-border/50 px-4 py-3 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Habit loop & network effects — Phase 17
          </p>
        </div>
      </div>
            ...
            <div className="card p-4 bg-primary/5 border border-primary/20">
              ...
              <div className="space-y-2">
                {[
                  ...
                  {
                    step: "4",
                    label: "User earns / saves time / gets result",
                    icon: "💰",
                  },
                  ...
                  <div key={s.step}>
                    ...
                    {i < arr.length - 1 && (
                      <div className="ml-3.5 w-px h-3 bg-border/50 mt-1" />
                    )}
                  </div>
                ))}
              </div>
            ...
            {RETENTION_METRICS.map(m => (
              <div key={m.label} className="card p-4">
                <div className="flex items-center justify-between mb-1">
                  ...
                  <span className={`text-xs ${m.color}`}>
              ...
              <div key={f.name} className="card p-4">
                <div className="flex items-start justify-between gap-2 mb-1">
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full ${f.status === "live" ? "bg-green-500/20 text-green-400" : "bg-yellow-500/20 text-yellow-400"}`}
                    >
                      {f.status}
                    </span>
                    <span
                      className={`text-xs px-2 py-0.5 rounded-full ${f.impact === "high" ? "bg-primary/20 text-primary" : "bg-secondary text-muted-foreground"}`}
                    >
                      {f.impact}
                    </span>
                  </div>
          ...
          <div className="space-y-3">
            <div className="card p-4 bg-primary/5 border border-primary/20">
              ...
              <div className="text-xs text-muted-foreground space-y-1">
                <div>
                  User chats → AI helps → Action completes → User shares result
            ...
            {[
              {
                ...
                shareText: '"I just made $20 using chat on ShadowChat"',
                channel: "X/Share2",
              },
              {
                trigger: "Task completed by AI",
                shareText: '"AI completed my logo brief in 30 seconds"',
                channel: "LinkedIn",
              },
              {
                trigger: "Match found",
