...
export default function Admin() {
  ...
  const { data: modStats } = trpc.moderation.stats.useQuery(undefined, {
    enabled: isAuthenticated && user?.role === "admin",
  });
  const { data: modLogs } = trpc.moderation.logs.useQuery(undefined, {
    enabled: isAuthenticated && user?.role === "admin",
  });
    ...
    <div className="min-h-screen py-16">
      <div className="container max-w-6xl">
        <div className="flex items-center justify-between mb-8">
          ...
          <Badge
            variant="outline"
            className="border-purple-500/30 text-purple-400 flex items-center gap-1.5"
          >
              ...
              <div className="stat-card mb-6">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div className="bg-background/50 rounded-lg p-3 border border-border/30">
                    ...
                    <div className="font-bold font-mono">
                      {modStats.accuracy
                        ? `${(modStats.accuracy * 100).toFixed(1)}%`
                        : "N/A"}
                    </div>
                  </div>
                  <div className="bg-background/50 rounded-lg p-3 border border-border/30">
              ...
              {modLogs && modLogs.length > 0 ? (
                <div className="space-y-2 max-h-80 overflow-y-auto">
                  {modLogs.slice(0, 15).map((log: any, i: number) => (
                    ...
                    >
                      {log.action === "auto_flag" ||
                      log.action === "auto_remove" ? (
                ...
                <div className="text-center py-8 text-muted-foreground">
                  ...
                  <p className="text-sm">
                    No moderation logs yet. The AI will log actions as content
                    is scanned.
                  </p>
                </div>
              )}
