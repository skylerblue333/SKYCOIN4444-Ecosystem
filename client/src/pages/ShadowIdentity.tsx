...
export default function ShadowIdentity() {
  ...
  const [activeTab, setActiveTab] = useState<
    "identity" | "reputation" | "leaderboard"
  >("identity");

  const { data: identity, refetch: refetchIdentity } =
    trpc.shadowIdentity.getMyIdentity.useQuery(undefined, { enabled: !!user });
  const { data: analysis } = trpc.shadowIdentity.getReputationAnalysis.useQuery(
    undefined,
    { enabled: !!user && activeTab === "reputation" }
  );
  const { data: leaderboard } =
    trpc.shadowIdentity.getReputationLeaderboard.useQuery(undefined, {
  ...
  const toggleRevealMutation =
    trpc.shadowIdentity.toggleVerifiedReveal.useMutation({
      onSuccess: data => {
        toast.success(
          data.verifiedReveal
      ...
      <div className="min-h-screen bg-black flex items-center justify-center">
        <Card className="bg-gray-900 border-gray-700 p-8 text-center max-w-sm">
          ...
          <p className="text-gray-400 mb-4">
            Sign in to manage your anonymous identity
          </p>
      ...
      <div className="bg-gradient-to-r from-purple-950 to-black border-b border-purple-800/50 px-4 py-6">
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center gap-3 mb-2">
            ...
            {identity && (
              <Badge
                className={`border ${TIER_COLORS[identity.reputationTier] ?? "text-gray-400 border-gray-400"} bg-transparent text-xs`}
              >
              ...
              <CardContent className="space-y-3">
                {MODES.map(mode => {
                  ...
                  return (
                    ...
                    >
                      <Icon
                        className={`w-5 h-5 ${isActive ? mode.color : "text-gray-500"}`}
                      />
                      <div className="flex-1 min-w-0">
                        <div
                          className={`font-medium text-sm ${isActive ? "text-white" : "text-gray-400"}`}
                        >
                          {mode.label}
                        </div>
            ...
            <Card className="bg-gray-900 border-gray-700">
              ...
              <CardContent className="grid grid-cols-2 gap-4">
                {[
                  {
                    label: "Behavior",
                    value: identity.scores.behavior,
                    icon: Heart,
                    color: "text-green-400",
                  },
                  {
                    label: "Contribution",
                    value: identity.scores.contribution,
                    icon: Star,
                    color: "text-yellow-400",
                  },
                  {
                    label: "Reliability",
                    value: identity.scores.reliability,
                    icon: Shield,
                    color: "text-blue-400",
                  },
                  {
                    label: "Toxicity",
                    value: identity.scores.toxicity,
                    icon: AlertTriangle,
                    color: "text-red-400",
                    inverted: true,
                  },
                  ...
                  const displayValue = score.inverted
                    ? 100 - score.value
                    : score.value;
                  return (
                    <div
            ...
            <Card className="bg-gray-900 border-gray-700">
              ...
              <CardContent className="flex items-center gap-4 p-4 bg-gray-800 rounded-lg">
                ...
                <div>
                  ...
                  <Badge
                    className={`border ${TIER_COLORS[identity.reputationTier] ?? ""} bg-transparent text-xs mt-1`}
                  >
                    {identity.reputationTier}
                  </Badge>
                </div>
            ...
            {analysis ? (
              <>
                <Card className="bg-gray-900 border-gray-700">
                  ...
                  <CardContent>
                    <div
                      className={`text-lg font-bold mb-2 ${TIER_COLORS[analysis.tier] ?? "text-gray-400"}`}
                    >
                      {analysis.tier}
                    </div>
                  ...
                  <CardContent className="space-y-3">
                    {Object.entries(analysis.scores).map(([key, val]) => (
                      <div key={key} className="flex items-center gap-3">
                        <span className="text-gray-400 text-sm w-24 capitalize">
                          {key}
                        </span>
                ...
                leaderboard.map((entry, i) => (
                  ...
                  >
                    ...
                    <div className="text-right">
                      <div
                        className={`text-sm font-bold ${TIER_COLORS[entry.reputationTier]?.split(" ")[0] ?? "text-gray-400"}`}
                      >
                        {entry.composite}
                      </div>
                      <div className="text-gray-500 text-xs">
