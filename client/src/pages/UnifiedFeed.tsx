...
function FeedCard({ item }: { item: FeedItem }) {
  ...
  return (
    <Card
      className={`glass-card border ${typeColors[item.type]} hover:border-white/20 transition-all`}
    >
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              ...
              <Badge className={`text-xs ${typeBadges[item.type].color}`}>
            ...
            {item.type === "stream" && item.viewers && (
              <div className="flex items-center gap-2 mb-3 p-2 rounded-lg bg-red-500/10 border border-red-500/20">
                ...
                <Button
                  size="sm"
                  className="ml-auto h-6 text-xs bg-red-500 hover:bg-red-600 text-white gap-1"
                >
            ...
            {item.type === "marketplace" && item.price && (
              <div className="flex items-center justify-between mb-3 p-2 rounded-lg bg-green-500/10 border border-green-500/20">
                ...
                <Button
                  size="sm"
                  className="h-6 text-xs bg-green-500 hover:bg-green-600 text-white gap-1"
                >
            ...
            {item.type === "dating" && item.match && (
              <div className="flex items-center gap-2 mb-3 p-2 rounded-lg bg-pink-500/10 border border-pink-500/20">
                ...
                <Button
                  size="sm"
                  className="ml-auto h-6 text-xs bg-pink-500 hover:bg-pink-600 text-white gap-1"
                >
                  Connect
                </Button>
              </div>
            ...
            {item.type === "nft" && item.price && (
              <div className="flex items-center justify-between mb-3 p-2 rounded-lg bg-yellow-500/10 border border-yellow-500/20">
                ...
                <Button
                  size="sm"
                  className="h-6 text-xs bg-yellow-500 hover:bg-yellow-600 text-black gap-1"
                >
                  Mint NFT
                </Button>
              </div>
            ...
            {item.type === "social" || item.type === "nft" ? (
              <div className="flex items-center gap-1">
                ...
                <Button
                  variant="ghost"
                  size="sm"
                  className="h-7 gap-1 text-xs text-white/40 hover:text-white"
                >
...
export default function UnifiedFeed() {
  const [filter, setFilter] = useState<
    "all" | "social" | "stream" | "marketplace" | "dating" | "agent" | "nft"
  >("all");

  const filtered =
    filter === "all" ? FEED_ITEMS : FEED_ITEMS.filter(i => i.type === filter);
    ...
    <div className="min-h-screen p-4 md:p-6 space-y-4">
      ...
      <div className="flex items-center gap-4 overflow-x-auto pb-1">
        {[
          ...
          <div
            key={stat.label}
            className="flex items-center gap-2 flex-shrink-0 bg-white/5 rounded-lg px-3 py-1.5"
          >
