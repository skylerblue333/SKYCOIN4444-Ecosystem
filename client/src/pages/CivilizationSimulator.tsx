    ...
    <div className="min-h-screen bg-black text-white">
        ...
        <div className="relative max-w-6xl mx-auto px-6 py-10">
          <div className="text-center mb-8">
            ...
            <p className="text-white/40 max-w-xl mx-auto">
              A living digital civilization powered by AI, governed by citizens,
              and built to last 1,000 years
            </p>
          </div>
      ...
      <div className="max-w-6xl mx-auto px-6 py-8 space-y-8">
          ...
          {ERAS.map(era => {
            ...
            return (
              ...
              >
                ...
                <div
                  className={`text-xs font-bold ${selectedEra === era.id ? c.text : "text-white/50"}`}
                >
                  {era.name.split(":")[0]}
                </div>
              </button>
        ...
        <Card className={`bg-black/60 border ${colors.border}`}>
          <CardHeader>
            <CardTitle
              className={`${colors.text} text-sm uppercase tracking-widest flex items-center gap-2`}
            >
            ...
            <div className="grid grid-cols-3 gap-4">
              {activeEra.metrics.map((metric, i) => (
                <div key={i} className="p-4 rounded-xl bg-white/5 text-center">
                  <div
                    className={`text-xl font-black font-mono ${colors.text}`}
                  >
                    {metric.value}
                  </div>
        ...
        <div className="grid md:grid-cols-3 gap-4">
          <Card className="bg-black/60 border-yellow-500/20">
            <CardHeader className="pb-3">
              <CardTitle className="text-yellow-400 text-xs uppercase tracking-widest">
                ECONOMY
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              {[
                {
                  label: "Health",
                  value: econHealth?.overallHealth ?? "LOADING",
                },
          ...
          <Card className="bg-black/60 border-purple-500/20">
            <CardHeader className="pb-3">
              <CardTitle className="text-purple-400 text-xs uppercase tracking-widest">
                GOVERNANCE
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              {[
                {
                  ...
                  value: String(govHealth?.totalProposals ?? 0),
                },
                {
                  label: "Active Votes",
                  value: String(govHealth?.activeProposals ?? 0),
                },
          ...
          <Card className="bg-black/60 border-blue-500/20">
            <CardHeader className="pb-3">
              <CardTitle className="text-blue-400 text-xs uppercase tracking-widest">
                NATION
              </CardTitle>
            </CardHeader>
            <CardContent className="space-y-2">
              {[
                {
                  label: "Gov Health",
                  value: emergentStatus?.governanceHealth ?? "GENESIS",
                },
                { label: "Citizens", value: "1,247" },
        ...
        <Card className="bg-gradient-to-r from-yellow-950/30 via-black to-purple-950/30 border border-yellow-500/20">
          <CardContent className="p-8 text-center">
            ...
            <p className="text-white/50 max-w-2xl mx-auto text-sm leading-relaxed">
              SKYCOIN4444 is not a product. It is a civilization. A living,
              breathing digital nation where AI and humans co-govern, co-create,
              and co-prosper. Every token earned is a vote. Every skill learned
              is a law. Every agent deployed is a citizen. This is the future of
              human organization.
            </p>
