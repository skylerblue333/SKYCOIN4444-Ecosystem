...
const MiningCalculator: React.FC = () => {
  const [selectedHardware, setSelectedHardware] =
    useState<string>("antminer-s21");
  const [selectedCoin, setSelectedCoin] = useState<string>("BTC");
  const [electricityCost, setElectricityCost] = useState<number>(0.1); // USD per kWh
  ...
  const calculations = useMemo(() => {
    ...
    const dailyRewardCoins =
      (coin.blockReward * blocksPerDay * totalHashrate) / 1000000; // Simplified
    const dailyRewardUSD = dailyRewardCoins * coin.price;

    // Calculate costs
    const dailyElectricityCost =
      ((power * quantity * 24) / 1000) * electricityCost; // kWh to cost
    ...
    const roi =
      totalHardwareCost > 0 ? totalHardwareCost / (dailyProfit * 365) : 0;

    // Breakeven point
    const breakevenDays =
      totalHardwareCost > 0 ? totalHardwareCost / dailyProfit : 0;
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8">
          ...
          <p className="text-slate-400">
            Calculate your potential earnings based on hardware and electricity
            costs
          </p>
        </div>
              ...
              <CardContent className="space-y-6">
                ...
                {selectedHardware === "custom" && (
                  <>
                    <div className="space-y-2">
                      <Label className="text-slate-300">
                        Hashrate (TH/s or MH/s)
                      </Label>
                      <Input
                        type="number"
                        value={customHashrate || 100}
                        onChange={e =>
                          setCustomHashrate(parseFloat(e.target.value) || 100)
                        }
                        className="bg-slate-700 border-slate-600 text-white"
                      />
                    </div>
                    <div className="space-y-2">
                      ...
                      <Input
                        ...
                        onChange={e =>
                          setCustomPower(parseFloat(e.target.value) || 300)
                        }
                        className="bg-slate-700 border-slate-600 text-white"
                      />
                    </div>
              ...
              <Card className="bg-slate-800 border-slate-700">
                <CardContent className="pt-6">
                  <div>
                    ...
                    <p
                      className={`text-3xl font-bold ${calculations.roi < 2 ? "text-green-400" : calculations.roi < 3 ? "text-yellow-400" : "text-red-400"}`}
                    >
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center py-2 border-b border-slate-700">
                    <span className="text-slate-400">
                      Gross Revenue ({coin.symbol})
                    </span>
                    <span className="text-white font-semibold">
                      ${calculations.dailyRewardUSD.toFixed(2)}
                    </span>
                  </div>
                  <div className="flex justify-between items-center py-2 border-b border-slate-700">
                    <span className="text-slate-400">Electricity Cost</span>
                    <span className="text-red-400 font-semibold">
                      -${calculations.dailyElectricityCost.toFixed(2)}
                    </span>
                  </div>
                  <div className="flex justify-between items-center py-2 border-b border-slate-700">
                    <span className="text-slate-400">
                    ...
                    <span className="text-red-400 font-semibold">
                      -${calculations.dailyPoolFee.toFixed(2)}
                    </span>
                  </div>
            ...
            <Card className="bg-slate-800 border-slate-700">
              ...
              <CardContent>
                <ResponsiveContainer width="100%" height={300}>
                  <AreaChart
                    data={calculations.profitOverTime.filter(
                      (_, i) => i % 30 === 0
                    )}
                  >
                    <defs>
