...
export default function AgentsDashboard() {
  ...
  const filtered =
    category === "all"
      ? FEATURED_AGENTS
      : FEATURED_AGENTS.filter(a => a.category === category);
            ...
            <Link key={agent.id} href={`/agents/${agent.id}`}>
              <div className="rounded-2xl border border-slate-800/60 bg-slate-900/40 p-5 hover:border-slate-700 transition-all cursor-pointer group">
                <div className="flex items-start justify-between mb-4">
                  <div
                    className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${agent.color} flex items-center justify-center`}
                  >
                    <agent.icon className="h-6 w-6 text-white" />
                  </div>
                  <Badge
                    className={`text-xs border-0 ${agent.status === "active" ? "bg-emerald-500/15 text-emerald-400" : "bg-slate-700/50 text-slate-400"}`}
                  >
                ...
                <div className="flex items-center justify-between text-xs">
                  <span className="text-slate-500">
                    {agent.tasks.toLocaleString()} tasks run
                  </span>
