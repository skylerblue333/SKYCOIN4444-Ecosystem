...
export default function SocialGraph() {
  ...
  const [tab, setTab] = useState<"followers" | "following" | "suggested">(
    "suggested"
  );
  ...
  const { data: suggested = [] } = trpc.user.suggestedFollows.useQuery(
    undefined,
    { enabled: !!user }
  );
        ...
        <div className="container max-w-3xl mx-auto px-4 relative z-10">
          ...
          <div className="flex items-center gap-6 mt-6">
            {[
              {
                label: "Followers",
                value: (followers as any[]).length,
                icon: Users,
                color: "text-purple-400",
              },
              {
                label: "Following",
                value: (following as any[]).length,
                icon: UserCheck,
                color: "text-cyan-400",
              },
              {
                label: "Suggested",
                value: (suggested as any[]).length,
                icon: Star,
      ...
      <div className="container max-w-3xl mx-auto px-4 py-8">
        {!user ? (
          <div className="text-center py-20 text-muted-foreground">
            Sign in to view your social graph
          </div>
        ) : (
          <>
            {/* Tabs */}
                ...
                {currentTab.data.map((u: any) => (
                  <UserCard
                    ...
                    isFollowing={
                      ...
                      (following as any[]).some((f: any) => f.id === u.id)
                    }
