...
export default function LegendaryStatus() {
  ...
  const photoUrl =
    "/manus-storage/Screenshot_20260618-004749_Facebook_a1b2c3d4.png";
  ...
  return (
    <div className="min-h-screen bg-[oklch(0.08_0.02_280)] text-white">
      ...
      <div className="relative overflow-hidden">
        ...
        <div
          className="absolute inset-0 bg-gradient-to-br from-yellow-500/20 via-orange-500/10 to-purple-500/20 animate-pulse"
        ...
        <div
          className="absolute top-8 left-1/4 w-2 h-2 bg-yellow-400 rounded-full opacity-60 animate-bounce"
          style={{ animationDelay: "0s" }}
        />
        <div
          className="absolute top-16 right-1/3 w-1.5 h-1.5 bg-orange-400 rounded-full opacity-50 animate-bounce"
          style={{ animationDelay: "0.5s" }}
        />
        <div
          className="absolute top-12 left-2/3 w-1 h-1 bg-yellow-300 rounded-full opacity-70 animate-bounce"
        ...
        <div className="relative max-w-5xl mx-auto px-4 pt-16 pb-12">
          <div className="flex flex-col md:flex-row items-center md:items-start gap-8">
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              <p className="text-yellow-400/80 font-semibold text-lg mb-1">
                {profile.title}
              </p>
              ...
              <div className="flex flex-wrap gap-2 justify-center md:justify-start mb-4">
                {profile.degrees.map((d, i) => (
                  <div
                    key={i}
                    className="flex items-center gap-1.5 bg-white/8 border border-white/15 rounded-lg px-3 py-1.5"
                  >
              ...
              <div className="flex flex-wrap gap-3 justify-center md:justify-start">
                ...
                <Link href="/social">
                  <Button
                    variant="outline"
                    className="border-cyan-500/30 text-cyan-400 hover:text-cyan-300 gap-2"
                  >
      ...
      <div className="max-w-5xl mx-auto px-4 py-8">
        <div className="flex items-center gap-2 mb-4">
          ...
          <h2 className="text-lg font-bold text-white">
            Platform Built by One Person
          </h2>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-6 gap-3">
          <StatCard
            value={profile.platformStats.pagesBuilt}
            label="Pages Built"
            icon={<Layers className="w-4 h-4" />}
          />
          <StatCard
            value={profile.platformStats.linesOfCode.toLocaleString()}
            label="Lines of Code"
            icon={<Code2 className="w-4 h-4" />}
          />
          <StatCard
            value={profile.platformStats.testsWritten.toLocaleString()}
            label="Tests Passing"
            icon={<CheckCircle className="w-4 h-4" />}
          />
          <StatCard
            value={profile.platformStats.dbTables}
            label="DB Tables"
            icon={<Database className="w-4 h-4" />}
          />
          <StatCard
            value={profile.platformStats.routerNamespaces}
            label="API Namespaces"
            icon={<Server className="w-4 h-4" />}
          />
          <StatCard
            value={`${profile.platformStats.yearsBuilding}yr`}
            label="Building"
          ...
          {profile.achievements.map(ach => (
            <Tooltip key={ach.id}>
              <TooltipTrigger asChild>
                <div
                  className={`relative p-4 rounded-xl bg-gradient-to-br ${ACHIEVEMENT_COLORS[ach.id] || "from-white/10 to-white/5"} bg-opacity-10 border border-white/10 cursor-default hover:scale-105 transition-transform`}
                >
                  <div className="absolute inset-0 rounded-xl bg-black/60" />
                  <div className="relative flex flex-col items-center gap-2 text-center">
                    <div
                      className={`w-10 h-10 rounded-full bg-gradient-to-br ${ACHIEVEMENT_COLORS[ach.id] || "from-white/20 to-white/10"} flex items-center justify-center text-white`}
                    >
              ...
              <TooltipContent
                side="top"
                className="bg-black/90 border-white/20 text-white text-xs max-w-[160px] text-center"
              >
                {ach.desc}
              </TooltipContent>
            </Tooltip>
      ...
      <div className="max-w-5xl mx-auto px-4 py-4 pb-16">
        <Card className="bg-white/5 border-white/10">
          ...
          <CardContent>
            <p className="text-white/60 text-sm leading-relaxed mb-4">
              Innovative Information Technology Resolutions LLC (IITR LLC) is a
              technology consulting and software development company
              specializing in custom AI systems, cybersecurity solutions, cloud
              infrastructure, and enterprise IT strategy. Founded and led by
              Skyler Blue Spillers, IITR LLC delivers cutting-edge technology
        ...
        <div className="mt-6 p-6 rounded-2xl bg-gradient-to-r from-yellow-500/10 to-orange-500/10 border border-yellow-500/20 text-center">
          <div className="text-yellow-400 text-sm font-semibold mb-1 uppercase tracking-widest">
            Reputation Score
          </div>
          ...
          <div className="mt-4 flex flex-wrap gap-2 justify-center">
            <Link href="/social">
              <Button
                size="sm"
                className="bg-gradient-to-r from-cyan-500 to-blue-500 text-white border-0 gap-1"
              >
                <Users className="w-3.5 h-3.5" /> Go Social{" "}
            ...
            <Link href="/crypto-hub">
              <Button
                size="sm"
                className="bg-gradient-to-r from-yellow-400 to-orange-500 text-black border-0 gap-1"
              >
                <Zap className="w-3.5 h-3.5" /> Mine Now{" "}
                <ChevronRight className="w-3.5 h-3.5" />
              </Button>
            </Link>
            <Link href="/ai-brain">
              <Button
                size="sm"
                className="bg-gradient-to-r from-purple-500 to-fuchsia-500 text-white border-0 gap-1"
              >
                <Brain className="w-3.5 h-3.5" /> AI Brain{" "}
                <ChevronRight className="w-3.5 h-3.5" />
              </Button>
            </Link>
            <Link href="/investor">
              ...
              >
                <TrendingUp className="w-3.5 h-3.5" /> Investor Room{" "}
