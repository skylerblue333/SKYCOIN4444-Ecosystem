      ...
      <div className="relative overflow-hidden border-b border-border/50 bg-gradient-to-br from-primary/5 via-background to-purple-500/5">
        <div className="container py-14 max-w-4xl">
          ...
          <p className="text-lg text-muted-foreground mb-6 max-w-2xl">
            Publish articles, audio, live streams, and exclusive drops. Earn
            SKY444 from your audience. Build your on-chain creator profile.
          </p>
      ...
      <div className="container py-10 max-w-4xl">
            ...
            {checklist.map((item, i) => {
              ...
              return (
                ...
                >
                  {/* Step number / check */}
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 ${item.completed ? "bg-purple-600/20 border border-purple-500/30" : `${item.bg} border ${item.border}`}`}
                  >
                    {item.completed ? (
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <p
                        className={`font-semibold text-sm ${item.completed ? "line-through text-muted-foreground" : ""}`}
                      >
                        {item.title}
                      </p>
                      {item.completed && (
                        <span className="px-2 py-0.5 rounded-full bg-purple-600/20 text-purple-400 text-xs font-medium">
                          Done
                        </span>
                      )}
                    </div>
        ...
        <div className="mb-10">
          ...
          <p className="text-muted-foreground text-sm mb-6">
            Choose a content type to get started. You can create all types from
            your Creator Studio.
          </p>
            ...
            {CONTENT_TYPES.map(ct => {
              ...
              return (
                <Link key={ct.id} href={ct.href}>
                  <div
                    className={`rounded-xl border ${ct.border} bg-gradient-to-br ${ct.color} p-5 hover:scale-[1.02] transition-all duration-200 cursor-pointer group`}
                  >
                    <div className="flex items-start gap-4">
                      <div
                        className={`w-12 h-12 rounded-xl bg-background/30 border ${ct.border} flex items-center justify-center shrink-0 group-hover:scale-110 transition-transform`}
                      >
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex flex-wrap gap-1.5 mb-3">
                          {ct.examples.map(ex => (
                            <span
                              key={ex}
                              className="px-2 py-0.5 rounded-full bg-background/40 text-xs text-muted-foreground border border-border/30"
                            >
                              {ex}
                            </span>
                          ))}
                        </div>
                        <p className={`text-xs font-medium ${ct.iconColor}`}>
                          {ct.earnings}
                        </p>
                      </div>
                    </div>
