        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-white font-semibold text-sm">
              {order.listingTitle || "Marketplace Item"}
            </p>
        ...
        <div className="flex items-center gap-3 text-xs text-slate-500">
          <span className="flex items-center gap-1">
            ...
            <span className="text-white font-bold">
              {Number(order.totalPrice || 0).toLocaleString()} SKY444
            </span>
          </span>
          {order.escrowStatus && (
...
export default function OrderHistory() {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = trpc.marketplace.myOrders.useQuery(
    { role },
  ...
  const stats = {
    ...
    completed:
      orders?.filter((o: any) => ["completed", "delivered"].includes(o.status))
        .length || 0,
    totalSpent:
      orders?.reduce((s: number, o: any) => s + Number(o.totalPrice || 0), 0) ||
      0,
  };
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          {[
            ...
            {
              label: "Total Spent",
              value: `${(stats.totalSpent / 1000).toFixed(1)}K SKY`,
        ...
        {isLoading ? (
          <div className="space-y-3">
            {[1, 2, 3].map(i => (
              <div
                key={i}
                className="bg-[#0e0a1a] border border-white/5 rounded-2xl p-4 animate-pulse h-24"
              />
            ))}
          </div>
        ) : filtered.length === 0 ? (
          <div className="text-center py-16">
            ...
            {orders?.length === 0 && (
              <Link href="/marketplace">
                <Button className="mt-3 bg-purple-500/20 text-purple-300 border border-purple-500/30">
                  Browse Marketplace
                </Button>
              </Link>
            )}
