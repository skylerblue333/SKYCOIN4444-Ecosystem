...
export default function CryptoHub() {
  ...
  const { data: balances, isLoading: balancesLoading } =
    trpc.token.allBalances.useQuery(undefined, {
  ...
  const swapMutation = trpc.token.multiSwap.useMutation({
    onSuccess: data => {
      if (data.success) {
        toast.success(
          `Swapped ${fmt(data.fromAmount ?? 0, 6)} ${data.fromToken} → ${fmt(data.toAmount ?? 0, 8)} ${data.toToken}`
        );
        utils.token.allBalances.invalidate();
  ...
  const estimatedOut =
    swapAmount && livePrices[swapTo] > 0
      ? (
          parseFloat(swapAmount) *
          (livePrices[swapFrom] / (livePrices[swapTo] || 1)) *
          (1 - swapSlippage / 100)
        ).toFixed(8)
      : "0";
  ...
  const stakeMutation = trpc.token.multiStake.useMutation({
    onSuccess: data => {
      if (data.success) {
        toast.success(
          `Staked ${fmt(parseFloat(stakeAmount), 4)} ${data.token} at ${data.apy}% APY for ${data.lockDays} days`
        );
        utils.token.allBalances.invalidate();
        setStakeAmount("");
      } else {
  ...
  const burnMutation = trpc.token.burn.useMutation({
    onSuccess: data => {
      if (data.success) {
        toast.success(
          `🔥 Burned ${fmt(data.burned ?? 0, 8)} ${data.token} permanently`
        );
        utils.token.allBalances.invalidate();
        setBurnAmount("");
  ...
  const totalUSD = TOKENS.reduce(
    (sum, t) => sum + (balanceMap[t.symbol] ?? 0) * (livePrices[t.symbol] ?? 0),
    0
  );
      ...
      <div className="min-h-screen bg-[#0a0a0f] flex items-center justify-center">
        <Card className="bg-[#111118] border-[#1e1e2e] p-8 text-center max-w-md">
          ...
          <h2 className="text-xl font-bold text-white mb-2">
            Connect to CryptoHub
          </h2>
          <p className="text-zinc-400 mb-6">
            Sign in to mine, swap, stake, and burn tokens.
          </p>
          ...
          <div className="flex items-center gap-4">
            <div
              ...
              style={{
                ...
                boxShadow: "0 0 24px oklch(0.55 0.25 260 / 0.4)",
              }}
            >
          ...
          {TOKENS.map(t => (
            <div key={t.symbol} className="flex items-center gap-2 shrink-0">
              ...
              <span className="text-xs text-zinc-300">
                {livePrices[t.symbol] > 0
                  ? `$${livePrices[t.symbol].toLocaleString()}`
                  : "—"}
              </span>
            </div>
          ))}
        ...
        <Tabs defaultValue="mine" className="space-y-6">
              ...
              <div className="lg:col-span-2 space-y-4">
                <Card className="bg-[#111118] border-[#1e1e2e]">
                  <CardHeader>
                    <CardTitle className="text-white flex items-center gap-2">
                      <Cpu className="w-5 h-5 text-indigo-400" /> CPU Mining
                      Control
                    </CardTitle>
                  </CardHeader>
                        ...
                        {MINING_TOKENS.map(t => (
                          ...
                          >
                            <div
                              ...
                              style={{
                                color:
                                  miningToken === t.symbol ? t.color : "#fff",
                              }}
                            >
                              {t.symbol}
                            </div>
                            <div className="text-xs text-zinc-500">
                              {DIFFICULTY_LABEL[t.symbol]}
                            </div>
                          </button>
                        ))}
                      </div>
                    ...
                    <div>
                      <div className="flex justify-between text-sm mb-2">
                        ...
                        <span className="text-indigo-400 font-bold">
                          {(hashPower / 1000).toFixed(2)} GH/s
                        </span>
                      </div>
                      <Slider
                        value={[hashPower]}
                        onValueChange={vals =>
                          setHashPower(vals[0] ?? hashPower)
                        }
                        min={10}
                        max={10000}
                        step={10}
                    ...
                    {isMining && (
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-2 text-xs text-zinc-500">
                          ...
                          <span>
                            Processing hashes for {miningToken} blockchain
                          </span>
                        </div>
                      </div>
                    )}
                ...
                <Card className="bg-[#0d0d14] border-[#1e1e2e]">
                  ...
                  <CardContent>
                    <div
                      ref={logRef}
                      className="font-mono text-xs space-y-1 max-h-48 overflow-y-auto"
                    >
                      {miningLog.length === 0 ? (
                      ...
                      >
                        ...
                        <div className="text-right">
                          <div className="text-sm text-white">
                            {fmt(totalMined[t.symbol] ?? 0, 8)}
                          </div>
                          <div className="text-xs text-zinc-500">
                            {usd(
                              totalMined[t.symbol] ?? 0,
                              livePrices[t.symbol] ?? 0
                            )}
                          </div>
                        </div>
                ...
                <Card className="bg-[#111118] border-[#1e1e2e]">
                  ...
                  <CardContent className="space-y-2 text-xs text-zinc-400">
                    <div className="flex justify-between">
                      <span>Algorithm</span>
                      <span className="text-white">SHA-256 (sim)</span>
                    </div>
                    <div className="flex justify-between">
                      <span>Pool Fee</span>
                    ...
                    <div className="mt-3 p-2 bg-indigo-500/10 border border-indigo-500/20 rounded text-indigo-300">
                      Mining rewards are proof-of-engagement simulations. Real
                      blockchain mining requires dedicated hardware.
                    </div>
                  </CardContent>
                </Card>
              </div>
          ...
          <TabsContent value="swap">
            <div className="max-w-lg mx-auto">
              <Card className="bg-[#111118] border-[#1e1e2e]">
                <CardHeader>
                  <CardTitle className="text-white flex items-center gap-2">
                    <ArrowLeftRight className="w-5 h-5 text-indigo-400" /> Token
                    Swap
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <Select
                        value={swapFrom}
                        onValueChange={v => setSwapFrom(v as TokenSymbol)}
                      >
                        ...
                        <SelectContent className="bg-[#111118] border-[#1e1e2e]">
                          {TOKENS.map(t => (
                            <SelectItem
                              key={t.symbol}
                              value={t.symbol}
                              className="text-white hover:bg-[#1e1e2e]"
                            >
                              {t.symbol}
                            </SelectItem>
                          ))}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="flex gap-2">
                      <Select
                        value={swapTo}
                        onValueChange={v => setSwapTo(v as TokenSymbol)}
                      >
                        <SelectTrigger className="w-36 bg-[#0d0d14] border-[#1e1e2e] text-white">
                          <SelectValue />
                        </SelectTrigger>
                        ...
                        <button
                          ...
                          className={`flex-1 py-1 rounded text-xs font-bold transition-colors ${
                            swapSlippage === s
                              ...
                              : "bg-[#0d0d14] text-zinc-400 hover:bg-[#1e1e2e]"
                          }`}
                        >
                          {s}%
                        </button>
                    ...
                    <div className="bg-[#0d0d14] rounded-lg p-3 space-y-1 text-xs">
                      <div className="flex justify-between text-zinc-400">
                        ...
                        <span className="text-white">
                          ...
                          {livePrices[swapTo] > 0
                            ? (
                                livePrices[swapFrom] / livePrices[swapTo]
                              ).toFixed(8)
                            : "—"}{" "}
                          {swapTo}
                        </span>
                  ...
                  <Button
                    ...
                    onClick={() =>
                      swapMutation.mutate({
                        fromToken: swapFrom as any,
                        toToken: swapTo as any,
              ...
              <Card className="bg-[#111118] border-[#1e1e2e]">
                ...
                <CardContent className="space-y-4">
                  <div>
                    ...
                    <Select
                      value={stakeToken}
                      onValueChange={v => setStakeToken(v as TokenSymbol)}
                    >
                  ...
                  <div>
                    ...
                    <div className="text-xs text-zinc-500 mt-1">
                      Available: {fmt(balanceMap[stakeToken] ?? 0, 6)}{" "}
                      {stakeToken}
                    </div>
                  </div>
                    ...
                    <div className="bg-[#0d0d14] rounded-lg p-3 space-y-1 text-xs">
                      <div className="flex justify-between text-zinc-400">
                        ...
                        <span className="text-green-400">
                          +
                          {fmt(
                            ((parseFloat(stakeAmount) * APY[stakeLock]) / 100) *
                              (stakeLock / 365),
                            6
                          )}{" "}
                          {stakeToken}
              ...
              <Card className="bg-[#111118] border-[#1e1e2e]">
                <CardHeader>
                  <CardTitle className="text-white flex items-center gap-2">
                    ...
                    <Badge className="bg-red-500/10 text-red-400 border-red-500/30 text-xs ml-auto">
                      Irreversible
                    </Badge>
                  </CardTitle>
                </CardHeader>
                <CardContent className="space-y-4">
                  <div className="bg-red-500/10 border border-red-500/30 rounded-lg p-3 flex items-start gap-2">
                    ...
                    <p className="text-xs text-red-300">
                      Burning permanently removes tokens from circulation. This
                      action cannot be undone. Burning reduces supply and may
                  ...
                  <div>
                    ...
                    <Select
                      value={burnToken}
                      onValueChange={v => setBurnToken(v as TokenSymbol)}
                    >
                      <SelectTrigger className="bg-[#0d0d14] border-[#1e1e2e] text-white">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent className="bg-[#111118] border-[#1e1e2e]">
                        {TOKENS.map(t => (
                          <SelectItem
                            ...
                            className="text-white hover:bg-[#1e1e2e]"
                          >
                            {t.symbol} — {fmt(balanceMap[t.symbol] ?? 0, 4)}{" "}
                            available
                          </SelectItem>
                        ))}
                      </SelectContent>
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="checkbox"
                      checked={burnConfirm}
                      onChange={e => setBurnConfirm(e.target.checked)}
                      ...
                      {(balances ?? [])
                        .reduce(
                          (s, b) =>
                            s +
                            b.stakedBalance *
                              (livePrices[b.token as TokenSymbol] ?? 0),
                          0
                        )
                        .toFixed(2)}
                    </div>
          ...
          <TabsContent value="history">
            <Card className="bg-[#111118] border-[#1e1e2e]">
              <CardHeader>
                <CardTitle className="text-white flex items-center gap-2">
                  <History className="w-5 h-5 text-indigo-400" /> Transaction
                  History
                </CardTitle>
              </CardHeader>
              <CardContent>
                  ...
                  <div className="space-y-2">
                    {txHistory.map((tx: any, i: number) => {
                      ...
                      return (
                        <div
                          key={i}
                          className="flex items-center gap-3 p-3 bg-[#0d0d14] rounded-lg hover:bg-[#111118] transition-colors"
                        >
                          <div
                            className={`w-8 h-8 rounded-full bg-[#1e1e2e] flex items-center justify-center shrink-0`}
                          >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2">
                              <span
                                className={`text-sm font-bold ${cfg.color}`}
                              >
                                {cfg.label}
                              </span>
                              <Badge className="bg-[#1e1e2e] text-zinc-400 text-xs border-0">
                                {tx.token}
                              </Badge>
                            </div>
                          ...
                          <div className="text-right shrink-0">
                            <div className="text-sm font-bold text-white">
                              {fmt(Math.abs(Number(tx.amount)), 8)}
                            </div>
                            <div className="text-xs text-zinc-500">
                              {tx.createdAt
