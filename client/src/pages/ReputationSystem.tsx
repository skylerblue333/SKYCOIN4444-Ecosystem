...
export default function ReputationSystem() {
  const [tab, setTab] = useState<"leaderboard" | "factors" | "badges">(
    "leaderboard"
  );
          ...
          <div className="space-y-2">
            {TOP_CREATORS.map(c => (
              <div key={c.rank} className="card p-4 flex items-center gap-3">
                <div
                  className={`w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold ${c.rank === 1 ? "bg-yellow-500/20 text-yellow-400" : c.rank === 2 ? "bg-gray-500/20 text-gray-400" : c.rank === 3 ? "bg-orange-500/20 text-orange-400" : "bg-secondary text-muted-foreground"}`}
                >
                  {c.rank}
                </div>
        ...
        {tab === "badges" && (
          <div className="grid grid-cols-2 gap-3">
            {[
              ...
              <div
                key={b.name}
                className={`card p-4 ${b.bg} border border-border/30`}
              >
