...
export default function GeneratedApiExplorer() {
  ...
  const [selected, setSelected] = useState<{
    group: string;
    idx: number;
  } | null>({ group: "Social", idx: 0 });
  const [response, setResponse] = useState<string>("");
  const [loading, setLoading] = useState(false);
  const [copied, setCopied] = useState(false);

  const selectedEndpoint = selected
    ? ENDPOINT_GROUPS.find(g => g.group === selected.group)?.endpoints[
        selected.idx
      ]
    : null;
        ...
        actions={
          <Link href="/api-docs">
            <Button variant="outline" className="gap-2 text-xs">
              <Globe className="w-3.5 h-3.5" /> Full Docs
            </Button>
          </Link>
        }
      />
        ...
        <div className="lg:col-span-2 space-y-2">
          {ENDPOINT_GROUPS.map(group => (
            <div key={group.group} className="card overflow-hidden">
              <button
                onClick={() =>
                  setOpenGroup(openGroup === group.group ? null : group.group)
                }
                ...
                <div className="border-t border-slate-700/40">
                  {group.endpoints.map((ep, i) => (
                    <button
                      key={i}
                      onClick={() => {
                        setSelected({ group: group.group, idx: i });
                        setResponse("");
                      }}
                      className={`w-full flex items-center gap-2 px-3.5 py-2.5 text-left hover:bg-secondary/30 transition-colors ${
                        selected?.group === group.group && selected.idx === i
                          ? "bg-primary/10 border-l-2 border-primary"
                          : ""
                      }`}
                    >
                      <span
                        className={`text-[10px] font-mono font-bold px-1.5 py-0.5 rounded border shrink-0 ${METHOD_COLOR[ep.method]}`}
                      >
                        {ep.method}
                      </span>
            ...
            <>
              <div className="card p-5">
                <div className="flex items-center gap-3 mb-3">
                  <span
                    className={`text-xs font-mono font-bold px-2 py-1 rounded border ${METHOD_COLOR[selectedEndpoint.method]}`}
                  >
                ...
                <div className="mb-4">
                  <div className="text-xs font-semibold text-muted-foreground mb-2 flex items-center gap-1">
                    <Shield className="w-3 h-3" /> Request Body
                  </div>
                  <pre className="bg-black/40 rounded-xl p-3 text-xs font-mono text-green-400 border border-slate-700/40 overflow-x-auto">
                ...
                >
                  {loading ? (
                    ...
                    <>
                      <Play className="w-3.5 h-3.5" /> Run Request
                    </>
                  )}
                </Button>
              </div>
              {response && (
                <div className="card p-5">
                  ...
                  <pre className="bg-black/40 rounded-xl p-4 text-xs font-mono text-cyan-400 overflow-x-auto border border-slate-700/40 max-h-64">
                    {response}
                  </pre>
                </div>
              )}
