...
function CardDisplay({ card }: { card: Card }) {
  ...
  return (
    <div
      className={`w-16 h-24 rounded-xl bg-white border-2 ${isRed ? "border-red-200" : "border-slate-200"} flex flex-col items-center justify-center shadow-lg`}
    >
      <span
        className={`text-lg font-black leading-none ${isRed ? "text-red-600" : "text-slate-900"}`}
      >
        {card.rank}
      </span>
      <span
        className={`text-xl leading-none ${isRed ? "text-red-600" : "text-slate-900"}`}
      >
        {card.suit}
      </span>
    </div>
...
export default function GameBlackjack() {
  ...
  const deal = () => {
    ...
    let d = [...deck];
    const [p1, d1] = drawCard(d);
    d = d1;
    const [dealer1, d2] = drawCard(d);
    d = d2;
    const [p2, d3] = drawCard(d);
    d = d3;
    const [dealer2, d4] = drawCard(d, true);
    d = d4;
    const ph = [p1, p2];
  ...
  const hit = () => {
    ...
    setDeck(d);
    const newHand =
      activeHand === "main" ? [...playerHand, card] : [...splitHand, card];
  ...
  const split = () => {
    ...
    let d = [...deck];
    const [nc1, d1] = drawCard(d);
    d = d1;
    const [nc2, d2] = drawCard(d);
    d = d2;
    ...
    <div className="min-h-screen bg-[#07050f] text-white p-4">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="bg-gradient-to-b from-green-900/40 to-green-950/60 border border-green-800/30 rounded-3xl p-6 mb-4 min-h-[400px] flex flex-col justify-between">
          {/* Dealer */}
          <div>
            <p className="text-xs text-green-400/70 mb-2 font-semibold">
              DEALER {phase !== "betting" && dealerHand.length > 0 && `(${dv})`}
            </p>
          ...
          {result && (
            <div
              className={`text-center py-3 px-4 rounded-xl font-bold text-lg ${result.includes("Win") || result.includes("Blackjack") ? "bg-green-500/20 text-green-300" : result.includes("Push") ? "bg-slate-500/20 text-slate-300" : "bg-red-500/20 text-red-300"}`}
            >
              {result}
            </div>
          )}
          ...
          <div>
            {splitHand.length > 0 && (
              <div className="mb-3">
                <p
                  className={`text-xs mb-2 font-semibold ${activeHand === "split" ? "text-yellow-400" : "text-slate-500"}`}
                >
                  SPLIT HAND ({handValue(splitHand)}){" "}
            ...
            <p
              className={`text-xs mb-2 font-semibold ${activeHand === "main" && phase === "playing" ? "text-yellow-400" : "text-slate-500"}`}
            >
              YOUR HAND{" "}
              {phase !== "betting" && playerHand.length > 0 && `(${pv})`}{" "}
              {pv > 21 && "— BUST"}{" "}
        ...
        {phase === "betting" && (
          <div className="space-y-3">
            <div>
              ...
              <div className="flex gap-2">
                <Input
                  ...
                  className="bg-white/5 border-white/10 text-white text-center font-bold"
                />
            ...
            <Button
              className="w-full h-12 text-lg font-black bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-400 hover:to-emerald-400 text-black"
              onClick={deal}
            >
              Deal Cards
            </Button>
        ...
        {phase === "playing" && (
          <div className="grid grid-cols-2 gap-2">
            <Button
              className="h-12 font-bold bg-green-500/20 hover:bg-green-500/30 text-green-300 border border-green-500/30"
              onClick={hit}
            >
              Hit
            </Button>
            <Button
              className="h-12 font-bold bg-red-500/20 hover:bg-red-500/30 text-red-300 border border-red-500/30"
              onClick={stand}
            >
              Stand
            </Button>
            {canDouble && (
              <Button
                className="h-12 font-bold bg-yellow-500/20 hover:bg-yellow-500/30 text-yellow-300 border border-yellow-500/30"
                onClick={double}
              >
                Double Down
              </Button>
            ...
            {canSplit && (
              <Button
                className="h-12 font-bold bg-purple-500/20 hover:bg-purple-500/30 text-purple-300 border border-purple-500/30"
                onClick={split}
              >
                Split
              </Button>
        ...
        {phase === "result" && (
          <Button
            className="w-full h-12 font-bold bg-gradient-to-r from-green-500 to-emerald-500 text-black"
            onClick={newGame}
          >
            New Hand
          </Button>
        ...
        <div className="mt-3 p-3 bg-yellow-500/5 border border-yellow-500/20 rounded-lg flex items-start gap-2">
          ...
          <p className="text-[10px] text-yellow-600">
            Standard Blackjack rules. House edge ~0.5%. SKY444 is a platform
            token, not real currency. Play responsibly.
          </p>
        </div>
      </div>
    </div>
