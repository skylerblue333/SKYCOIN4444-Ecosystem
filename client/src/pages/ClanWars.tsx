...
export default function ClanWars() {
  ...
  const handleCreateClan = () => {
    ...
    toast.success(
      `Clan "${newClanName}" [${newClanTag.toUpperCase()}] created! Recruiting members...`
    );
    setTab("clans");
  };
        ...
        <div className="container max-w-5xl mx-auto px-4 relative z-10">
          ...
          <div className="flex items-center gap-6 mt-6">
            {[
              {
                label: "Active Wars",
                value: ACTIVE_WARS.length,
                icon: Swords,
                color: "text-red-400",
              },
              {
                label: "Total Clans",
                value: SAMPLE_CLANS.length,
                icon: Shield,
                color: "text-blue-400",
              },
              {
                label: "Warriors",
                value: SAMPLE_CLANS.reduce((s, c) => s + c.members, 0),
                icon: Users,
          ...
          {[
            {
              id: "wars" as const,
              label: "⚔️ Active Wars",
              count: ACTIVE_WARS.length,
            },
            {
              id: "clans" as const,
              label: "🛡️ Clan Rankings",
              count: SAMPLE_CLANS.length,
            },
        ...
        {tab === "wars" && (
          <div className="space-y-4">
            {ACTIVE_WARS.map(war => {
              ...
              const leading =
                war.clan1Score > war.clan2Score ? war.clan1 : war.clan2;
              return (
                <div
                  key={war.id}
        ...
        {tab === "clans" && (
          <div className="space-y-3">
            {SAMPLE_CLANS.map(clan => (
              <div
                key={clan.id}
                className="flex items-center gap-4 p-4 rounded-xl border border-white/10 bg-white/3 hover:bg-white/5 transition-all group"
              >
                <div
                  className={`w-10 h-10 rounded-xl bg-gradient-to-br ${clan.color} flex items-center justify-center text-sm font-black text-white`}
                >
                  #{clan.rank}
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-muted-foreground bg-white/5 border border-white/10 px-1.5 py-0.5 rounded">
                      [{clan.tag}]
                    </span>
                  </div>
                ...
                <button
                  onClick={() =>
                    toast.info(`Joining ${clan.name}... Clan application sent!`)
                  }
            ...
            <div className="rounded-xl border border-white/10 bg-white/3 p-6 space-y-4">
              ...
              <div>
                ...
                <input
                  value={newClanTag}
                  onChange={e =>
                    setNewClanTag(e.target.value.toUpperCase().slice(0, 4))
                  }
              ...
              <div className="text-xs text-muted-foreground text-center">
                Creating a clan costs 500 SKY444 tokens. You'll be the Clan
                Leader.
              </div>
            </div>
          </div>
