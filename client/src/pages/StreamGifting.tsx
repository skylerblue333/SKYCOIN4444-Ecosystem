...
export default function StreamGifting() {
  ...
  const triggerAnimation = useCallback(
    (gift: (typeof GIFTS)[0], sender: string) => {
  ...
  const handleSendGift = () => {
    ...
    toast.success(
      `Sent ${quantity}x ${selectedGift.emoji} ${selectedGift.name} for ${total} SKY444!`
    );
  };
              ...
              <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-3">
                <div className="flex gap-2 overflow-hidden">
                  {MOCK_RECENT_GIFTS.slice(0, 3).map((g, i) => (
                    <div
                      key={i}
                      className="flex items-center gap-1.5 bg-black/50 backdrop-blur-sm rounded-full px-2 py-1 text-[10px] whitespace-nowrap"
                    >
                      <span>{g.emoji}</span>
            ...
            <div className="mt-4 bg-[#0e0a1a] border border-white/5 rounded-2xl p-4">
              ...
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 bg-white/5 border border-white/10 rounded-xl px-3 py-2 text-sm text-slate-400">
                  Total:{" "}
                  <span className="text-yellow-400 font-bold">
                    {selectedGift.cost * quantity} SKY444
                  </span>
                </div>
                <Button
                  onClick={handleSendGift}
                  className="bg-gradient-to-r from-pink-500 to-purple-500 text-white border-0 gap-2"
                >
              ...
              <div className="grid grid-cols-2 gap-2">
                {[100, 500, 1000, 5000].map(amt => (
                  <Button
                    ...
                    onClick={() => {
                      setBalance(b => b + amt);
                      toast.success(`+${amt} SKY444 added!`);
                    }}
                  >
                    +{amt} SKY
                  </Button>
