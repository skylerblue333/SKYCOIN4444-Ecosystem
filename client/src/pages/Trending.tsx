...
export default function Trending() {
  ...
  } = trpc.explore.getTrending.useQuery();
  const { data: postsRaw, isLoading: loadingPosts } =
    trpc.social.getFeed.useQuery({ limit: 6 });
  ...
  const posts: any[] = Array.isArray(postsRaw)
    ? postsRaw
    : ((postsRaw as any)?.posts ?? []);
  ...
  return (
    <div className="container py-8 max-w-4xl animate-page-in">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        <div className="card p-4 flex flex-col gap-1">
          ...
          <div className="text-2xl font-bold">
            {trendingTags.length > 0 ? trendingTags.length : "—"}
          </div>
        </div>
        <div className="card p-4 flex flex-col gap-1">
      ...
      {(tab === "All" || tab === "Hashtags") && (
        <div className="card p-5 mb-6">
          ...
          {loadingTrending ? (
            ...
            <div className="text-center text-sm text-muted-foreground py-8">
              No trending hashtags yet — start posting with hashtags to see them
              here!
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {trendingTags.map(t => (
                <Link key={t.tag} href={`/explore?q=${t.tag}`}>
                  <div
                    className={`flex items-center gap-3 px-4 py-3 rounded-xl border transition-all hover:scale-[1.01] active:scale-[0.99] cursor-pointer ${
                      t.hot
      ...
      {(tab === "All" || tab === "Creators") && <TrendingCreators />}
...
function TrendingCreators() {
  ...
  const { data: creatorsRaw, isLoading } =
    trpc.explore.discoverUsers.useQuery();
  ...
  return (
    <div className="card p-5 mb-6">
      ...
      {isLoading ? (
        ...
        <div className="text-center text-sm text-muted-foreground py-8">
          No creators yet — start posting to appear here!
        </div>
      ) : (
          ...
          {creators.map((c: any, i: number) => (
            <Link key={c.id} href={`/profile/${c.username ?? c.id}`}>
              <div className="flex items-center gap-3 p-3 rounded-xl bg-secondary/30 border border-slate-700/40 hover:bg-secondary/60 hover:border-slate-600/60 active:scale-[0.98] transition-all cursor-pointer">
                <div
                  className={`w-10 h-10 rounded-full bg-gradient-to-br ${COLORS[i % COLORS.length]} flex items-center justify-center text-sm font-bold text-white shrink-0`}
                >
