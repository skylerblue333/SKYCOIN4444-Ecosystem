...
function MultiplierDisplay({
  ...
  return (
    <div
      className={`text-7xl font-black tabular-nums transition-colors duration-200 ${color}`}
    >
      {value.toFixed(2)}x
    </div>
  );
...
export default function GameCrash() {
  ...
  const startGame = useCallback(() => {
    ...
    intervalRef.current = setInterval(() => {
      ...
      if (current >= crashPointRef.current) {
        ...
        setHistory(prev => [
          parseFloat(crashPointRef.current.toFixed(2)),
          ...prev.slice(0, 14),
        ]);
        setTimeout(() => {
  ...
  const handleCashout = (currentMult?: number) => {
    ...
    setCashedOutAt(mult);
    toast.success(
      `Cashed out at ${mult.toFixed(2)}x! +${winnings.toFixed(2)} SKY444`
    );
  };
      ...
      <div className="max-w-5xl mx-auto">
        ...
        <div className="flex gap-1.5 mb-4 overflow-x-auto pb-2">
          {history.map((val, i) => (
            <Badge
              key={i}
              className={`shrink-0 text-xs font-bold ${getHistoryColor(val)}`}
            >
              {val.toFixed(2)}x
            </Badge>
          ))}
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
          ...
          <div className="lg:col-span-2">
            <div
              className="relative bg-[#0e0a1a] border border-white/5 rounded-2xl overflow-hidden"
              style={{ height: "400px" }}
            >
              {/* Graph background */}
              ...
              {gameState === "running" && (
                <div
                  ...
                  style={{
                    transform: `translateY(-${Math.min(80, (multiplier - 1) * 20)}%)`,
            ...
            <div className="mt-4 bg-[#0e0a1a] border border-white/5 rounded-2xl p-4">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      className="shrink-0 border-white/10 text-slate-400"
                      onClick={() =>
                        setBetAmount(v => String(parseFloat(v) * 2))
                      }
                    >
                      2x
                    </Button>
                    <Button
                      ...
                      className="shrink-0 border-white/10 text-slate-400"
                      onClick={() =>
                        setBetAmount(v => String(Math.floor(parseFloat(v) / 2)))
                      }
                    >
                      ½
                    </Button>
              ...
              <div className="flex gap-3">
                ...
                {gameState === "running" && hasBet && !cashedOut && (
                  <Button
                    className="flex-1 bg-gradient-to-r from-green-500 to-emerald-500 hover:from-green-400 hover:to-emerald-400 text-black font-bold text-lg"
                ...
                {gameState === "running" && (!hasBet || cashedOut) && (
                  <Button
                    className="flex-1 bg-white/5 text-slate-500 border border-white/5"
                    disabled
                  >
                    {cashedOut
                      ? `Cashed out at ${cashedOutAt.toFixed(2)}x`
