...
function DHgateTabContent() {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = trpc.dhgate.getProducts.useQuery(
    { category: cat, search: search || undefined, sort, limit: 24, offset: 0 },
    { refetchOnWindowFocus: false }
  );
  const { data: hotProducts } = trpc.dhgate.getHot.useQuery(undefined, {
    retry: false,
  });
  ...
  return (
    <div className="space-y-4">
      {selected && (
        <DHgateModal product={selected} onClose={() => setSelected(null)} />
      )}
      {/* Hot deals */}
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {list.map((p: any) => (
            <DHgateCard key={p.id} product={p} onClick={() => setSelected(p)} />
          ))}
        </div>
      )}
