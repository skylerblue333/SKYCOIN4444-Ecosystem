...
export default function AIModerationQueue() {
  ...
  const {
    ...
    isLoading,
    refetch,
  } = trpc.moderation.queue.useQuery(
    { limit: 50 },
        ...
        {stats && (
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {[
              {
                label: "Total Actions",
                value: (stats as any).totalActions || 0,
                icon: Shield,
                color: "text-blue-400",
              },
              {
                label: "Auto-Moderated",
                value: (stats as any).autoModerated || 0,
                icon: Zap,
                color: "text-violet-400",
              },
              {
                label: "Manual Reviews",
                value: (stats as any).manualReviews || 0,
                icon: Eye,
                color: "text-yellow-400",
              },
              {
                label: "AI Accuracy",
                value: `${(stats as any).accuracy || 0}%`,
                icon: Brain,
                  ...
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2 flex-wrap">
                        <Badge
                          className={`text-xs ${ACTION_COLORS[log.action] || "bg-zinc-700 text-zinc-400"}`}
                        >
