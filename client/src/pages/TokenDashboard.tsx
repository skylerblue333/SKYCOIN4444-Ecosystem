...
export default function TokenDashboard() {
  const { data: metrics, isLoading: metricsLoading } =
    trpc.token.metrics.useQuery();
  const { data: burnHistory, isLoading: burnLoading } =
    trpc.token.burnHistory.useQuery();
        ...
        <div className="container mx-auto px-4 relative z-10">
          <div className="max-w-3xl">
            ...
            <p className="text-lg text-muted-foreground">
              Deflationary utility token powering the Shadowchat ecosystem.
              Governance, staking, and burn mechanisms drive long-term value.
            </p>
          </div>
        </div>
      </section>
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <div className="stat-card">
                ...
                <div className="text-2xl md:text-3xl font-bold font-mono">
                  {(metrics?.totalSupply ?? 0).toLocaleString()}
                </div>
              ...
              <div className="stat-card">
                ...
                <div className="text-2xl md:text-3xl font-bold font-mono">
                  {(metrics?.circulatingSupply ?? 0).toLocaleString()}
                </div>
                <div className="text-sm text-muted-foreground font-mono mt-1">
                  {(
                    ((metrics?.circulatingSupply ?? 0) /
                      (metrics?.totalSupply || 1)) *
                    100
                  ).toFixed(1)}
                  %
                </div>
              ...
              <div className="stat-card">
                ...
                <div className="text-2xl md:text-3xl font-bold font-mono">
                  {(metrics?.burnedTokens ?? 0).toLocaleString()}
                </div>
                <div className="text-sm text-muted-foreground font-mono mt-1">
                  {(
                    ((metrics?.burnedTokens ?? 0) /
                      (metrics?.totalSupply || 1)) *
                    100
                  ).toFixed(1)}
                  %
              ...
              <div className="stat-card">
                ...
                <div className="text-2xl md:text-3xl font-bold font-mono">
                  {(metrics?.uniqueHolders ?? 0).toLocaleString()}
                </div>
                <div className="text-sm text-muted-foreground font-mono mt-1">
                  Wallets
                </div>
                    ...
                    {burnHistory?.map((burn, i) => (
                      ...
                      >
                        <td className="py-3 px-4 font-mono">
                          {new Date(burn.date).toLocaleDateString()}
                        </td>
                        <td className="py-3 px-4 font-mono text-cyber-orange">
                          -{burn.amount.toLocaleString()} SKY444
                        </td>
