...
export default function WhaleMonitor() {
  ...
  const filtered =
    filter === "All"
      ? liveTxs
      : liveTxs.filter(t => t.type === filter.toLowerCase());
        ...
        <div className="rounded-xl border border-white/10 bg-white/3 overflow-hidden">
          <div className="divide-y divide-white/5">
            {filtered.map(tx => (
              ...
              >
                <div
                  className={`w-2 h-2 rounded-full flex-shrink-0 ${IMPACT_DOT[tx.impact]}`}
                />
                <span
                  className={`text-xs font-bold px-2 py-0.5 rounded-full uppercase border ${TYPE_STYLE[tx.type]} flex-shrink-0`}
                >
                  {tx.type}
                </span>
                ...
                <div
                  className={`text-xs font-medium hidden md:block flex-shrink-0 w-16 text-right ${
                    tx.impact === "high"
                      ? "text-red-400"
                      : tx.impact === "medium"
                        ? "text-amber-400"
