...
function VestingBar({
  ...
  schedule: (typeof TEAM_VESTING)[0];
  currentMonth: number;
}) {
  ...
  const vestingProgress =
    vesting > 0 ? Math.min(1, monthsElapsed / vesting) : 1;
      ...
      >
        {isInCliff ? (
          <div
            ...
            style={{
              width: `${(currentMonth / (cliff + vesting)) * 100}%`,
...
export default function VestingSchedule() {
  ...
  const { data: mySchedules } = trpc.token.vestingSchedules.useQuery(
    undefined,
        ...
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center gap-3 mb-2">
            <div
              ...
              style={{ background: "oklch(0.72 0.28 305 / 0.15)" }}
            >
              <Lock
                className="w-5 h-5"
          ...
          >
            Full transparency on SKY444 token unlock schedule, cliff periods,
          ...
          >
            <div className="flex items-center justify-between mb-2">
              <p className="text-xs font-medium text-white">
                Simulate: Month {currentMonth} post-launch
              </p>
              <p className="text-xs" style={{ color: "oklch(0.72 0.28 160)" }}>
                {circulatingPct.toFixed(1)}% circulating
              </p>
            </div>
            <input
              type="range"
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ...
              {
                label: "Circulating",
                value: `${(totalUnlocked / 1e6).toFixed(0)}M`,
                color: "oklch(0.72 0.28 160)",
              },
              {
                label: "Still Locked",
                value: `${((TOTAL_SUPPLY - totalUnlocked) / 1e6).toFixed(0)}M`,
                color: "oklch(0.80 0.18 70)",
              },
            ].map(stat => (
              <div
            ...
            {mySchedules && (mySchedules as any[]).length > 0 ? (
              <div className="space-y-3">
                {(mySchedules as any[]).map((s: any, i: number) => (
                  <div
                    ...
                    style={{
                      background: "oklch(0.11 0.025 270)",
                      border: "1px solid oklch(0.72 0.28 160 / 0.20)",
                    }}
                  >
                    ...
                    <Button
                      size="sm"
                      onClick={() =>
                        claimMutation.mutate({ vestingId: String(s.id) })
                      }
                      disabled={claimMutation.isPending}
                      style={{
                        background: "oklch(0.72 0.28 160 / 0.20)",
                        color: "oklch(0.72 0.28 160)",
                        border: "none",
                      }}
                    >
              ...
              >
                ...
                <p
                  ...
                  style={{ color: "oklch(0.45 0.020 275)" }}
                >
                  Participate in token sales or earn through staking to get
                  vesting positions.
                </p>
              </div>
            )}
        ...
        <section>
          ...
          <div className="space-y-2">
            {[
              ...
              {
                month: 3,
                event: "Seed Investor cliff ends — 18mo linear vesting begins",
                color: "oklch(0.72 0.28 220)",
              },
              {
                month: 6,
                event: "Advisor cliff ends — 24mo linear vesting begins",
                color: "oklch(0.80 0.18 70)",
              },
              {
                month: 12,
                event: "Team & Founder cliff ends — 48mo linear vesting begins",
                color: "oklch(0.72 0.28 305)",
              },
              {
                month: 12,
                event: "Reserve cliff ends — 36mo linear vesting begins",
                color: "oklch(0.55 0.020 275)",
              },
              {
                month: 24,
