  ...
  return (
    <div className="container py-8 max-w-6xl animate-page-in">
      <PageHeader
        ...
        subtitle="Platform-wide metrics: DAU/MAU, revenue, content performance, and growth trends"
      />
          ...
          <div className="flex gap-2">
            {(["dau", "revenue", "posts"] as const).map(m => (
              <button
                key={m}
                onClick={() => setMetric(m)}
                className={`px-3 py-1 rounded text-xs font-medium uppercase transition-colors ${metric === m ? "bg-primary text-primary-foreground" : "bg-secondary text-muted-foreground hover:text-foreground"}`}
              >
                {m}
              </button>
            ))}
            ...
            const val =
              metric === "dau"
                ...
                : metric === "revenue"
                  ...
                  : d.posts;
            const maxVal =
              metric === "dau" ? maxDau : metric === "revenue" ? 4800 : 1300;
            ...
            return (
              <div key={i} className="flex-1 flex flex-col items-center gap-1">
                ...
                >
                  ...
                  <div className="absolute -top-6 left-1/2 -translate-x-1/2 text-xs text-muted-foreground opacity-0 group-hover:opacity-100 whitespace-nowrap">
                    {val.toLocaleString()}
                  </div>
                </div>
      ...
      <div className="card overflow-hidden">
        ...
        <div className="divide-y divide-border/30">
          {TOP_CONTENT.map((c, i) => (
            ...
            >
              <div className="text-2xl font-bold text-muted-foreground/30 w-6 text-center">
                {i + 1}
              </div>
