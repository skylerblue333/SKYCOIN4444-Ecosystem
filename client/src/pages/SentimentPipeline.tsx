...
const ALERTS = [
  {
    msg: "Sentiment spike detected: Share2 +23% in 1h",
    time: "2m ago",
    type: "positive",
  },
  {
...
export default function SentimentPipeline() {
  ...
  const avgSentiment =
    FEEDS.reduce((a, f) => a + f.sentiment, 0) / FEEDS.length;
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500 mt-0.5">
                NLP · Multi-source ingestion · Discord/Slack alerts ·
                Data-as-a-Service
              </p>
            </div>
          </div>
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
                ...
                <div key={f.source} className="flex items-center gap-3">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="h-2 bg-slate-800 rounded-full overflow-hidden">
                      <div
                        className={`h-full rounded-full ${f.sentiment > 0.6 ? "bg-emerald-500" : f.sentiment > 0.4 ? "bg-amber-500" : "bg-rose-500"}`}
                        style={{ width: `${f.sentiment * 100}%` }}
                      />
                    </div>
                  </div>
          ...
          <div className="bg-slate-900/40 rounded-2xl border border-slate-800/60 p-5">
            ...
            <div className="space-y-3">
              {ALERTS.map((a, i) => (
                <div
                  key={i}
                  className={`rounded-xl p-3 border ${a.type === "positive" ? "bg-emerald-500/5 border-emerald-500/20" : "bg-rose-500/5 border-rose-500/20"}`}
                >
        ...
        <div className="bg-violet-950/20 border border-violet-800/30 rounded-2xl p-5">
          ...
          <p className="text-xs text-slate-500 mt-3">
            Last scan: {lastScan} · Next scan: in 3 minutes · Kafka throughput:
            847K msg/s
          </p>
        </div>
      </div>
