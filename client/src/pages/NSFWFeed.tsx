    ...
    <div className="min-h-screen bg-[#050308] flex items-center justify-center p-4">
      <div className="max-w-md w-full bg-slate-900 border border-red-500/30 rounded-2xl p-8 text-center">
        ...
        <p className="text-slate-400 text-sm mb-6">
          This section contains adult content intended for users 18 years of age
          or older. By continuing, you confirm you are at least 18 years old.
        </p>
        <div className="space-y-3">
          ...
          >
            <Shield className="w-4 h-4 mr-2" />I am 18 or older — Enter
          </Button>
          <Link href="/social">
            <Button
              variant="outline"
        ...
        <p className="text-xs text-slate-600 mt-4">
          This platform complies with 18 U.S.C. § 2257. All performers are 18+
...
export default function NSFWFeed() {
  ...
  const [filter, setFilter] = useState<"all" | "free" | "premium" | "vip">(
    "all"
  );
  const [blurred, setBlurred] = useState<Record<number, boolean>>({});

  if (!ageVerified)
    return <AgeGateBanner onConfirm={() => setAgeVerified(true)} />;
        ...
        <div className="flex gap-3 overflow-x-auto pb-3 mb-6 scrollbar-hide">
          {[
            ...
            >
              ...
              <span className="text-[10px] text-slate-400 group-hover:text-white transition-colors truncate w-14 text-center">
                {name}
              </span>
            </div>
          ))}
        ...
        >
          {filtered.map(item => (
            ...
            >
              {/* Thumbnail */}
              <div
                className={`relative ${viewMode === "list" ? "w-24 h-20 shrink-0 rounded-lg overflow-hidden" : "aspect-[4/3]"} bg-gradient-to-br from-slate-800 to-slate-900`}
              >
                ...
                {item.locked && (
                  <div
                    ...
                    onClick={() =>
                      setBlurred(b => ({ ...b, [item.id]: !b[item.id] }))
                    }
                  >
                ...
                <Badge
                  className={`absolute top-2 left-2 text-[9px] px-1.5 py-0.5 ${TIER_COLORS[item.tier]}`}
                >
                  {item.tier}
                </Badge>
              </div>
              ...
              <div className={viewMode === "list" ? "flex-1 min-w-0" : "p-3"}>
                ...
                <div className="flex items-center gap-3 text-xs text-slate-500">
                  ...
                  {item.locked && (
                    <Button
                      size="sm"
                      className="ml-auto h-6 px-2 text-[10px] bg-red-600 hover:bg-red-500 text-white"
                    >
                      Unlock ${item.price}
                    </Button>
                  )}
