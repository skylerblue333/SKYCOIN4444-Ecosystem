...
export default function RateLimitDashboard() {
  ...
  const { data: secDashboard, isLoading } = trpc.security.getDashboard.useQuery(
    undefined,
    {
          ...
          <Card className="bg-amber-500/10 border-amber-500/30">
            <CardContent className="p-4 flex items-center gap-3">
              ...
              <p className="text-sm text-amber-300">
                Full rate limit analytics require admin access. Showing
                platform-wide rate limit rules below.
              </p>
            </CardContent>
          </Card>
        )}
          ...
          {[
            {
              ...
              value: isAdmin
                ? ((secDashboard as any)?.totalRequests ?? 0).toLocaleString()
                : "Admin only",
              icon: Activity,
              color: "text-blue-400",
            },
            {
              ...
              value: isAdmin
                ? ((secDashboard as any)?.blockedRequests ?? 0).toLocaleString()
                : "Admin only",
              icon: Shield,
              color: "text-red-400",
            },
            {
              label: "Unique IPs",
              value: isAdmin
                ? ((secDashboard as any)?.uniqueIPs ?? 0).toLocaleString()
                : "Admin only",
              icon: Users,
            ...
            },
            {
              label: "Block Rate",
              value: isAdmin
                ? `${((((secDashboard as any)?.blockedRequests ?? 0) / Math.max((secDashboard as any)?.totalRequests ?? 1, 1)) * 100).toFixed(2)}%`
                : "Admin only",
              icon: TrendingUp,
          ...
          <Card className="bg-zinc-900 border-zinc-800">
            ...
            <CardContent className="p-0">
              {!isAdmin ? (
                ...
                ((secDashboard as any)?.topOffenders ?? []).map(
                  (o: any, i: number) => (
                    <div
                      key={i}
                      className="flex items-center justify-between p-3 border-b border-zinc-800 last:border-0"
                    >
                      <div>
          ...
          <Card className="bg-zinc-900 border-zinc-800">
            ...
            <CardContent className="p-0">
              {!isAdmin ? (
                ...
                <div className="p-6 text-center text-zinc-500 text-sm">
                  No blocks recorded recently
                </div>
              ) : (
                ((secDashboard as any)?.recentBlocks ?? []).map(
                  (b: any, i: number) => (
                    <div
                      key={i}
