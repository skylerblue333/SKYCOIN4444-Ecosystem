          ...
          {[
            ...
            return (
              <div key={s.key} className="flex items-center gap-2 flex-1">
                ...
                <span
                  className={`text-xs hidden sm:block ${isActive ? "text-white" : isDone ? "text-emerald-400" : "text-zinc-500"}`}
                >
                  {s.label}
                </span>
                {i < arr.length - 1 && (
                  <div
                    className={`h-px flex-1 ${isDone ? "bg-emerald-500/50" : "bg-zinc-800"}`}
                  />
                )}
              </div>
            );
            ...
            <CardContent className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {BACKUP_CODES.map((c, i) => (
                  <div
                    key={i}
                    className="bg-zinc-800 rounded-lg p-2 text-center font-mono text-sm text-white tracking-wider"
                  >
                    {c}
                  </div>
                ))}
