...
export default function ServerHealth() {
  ...
  const { data: healthData, refetch } = trpc.system.health.useQuery(
    { timestamp: ts },
    {
      ...
      <div className="sticky top-0 z-40 bg-[#050308]/95 backdrop-blur border-b border-slate-800 px-4 py-3">
        <div className="max-w-5xl mx-auto flex items-center justify-between gap-4">
          <div className="flex items-center gap-2">
            ...
            <Badge
              className={`text-[10px] px-2 py-0.5 border ${STATUS_BG[overallStatus]} ${STATUS_COLORS[overallStatus]}`}
            >
      ...
      <div className="max-w-5xl mx-auto px-4 py-6">
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          {[
            ...
            <div
              key={label}
              className={`border rounded-xl p-4 text-center ${bg}`}
            >
          ...
          <div className="grid md:grid-cols-2 gap-3">
            {SERVICES.map(svc => {
              ...
              return (
                <div
                  key={svc.name}
                  className={`flex items-center gap-3 p-3.5 rounded-xl border ${STATUS_BG[svc.status]}`}
                >
                  <div
                    className={`w-9 h-9 rounded-lg bg-slate-900/50 flex items-center justify-center shrink-0`}
                  >
                  ...
                  <div className="text-right shrink-0">
                    <p
                      className={`text-xs font-bold ${STATUS_COLORS[svc.status]} capitalize`}
                    >
                      {svc.status}
                    </p>
        ...
        <div className="bg-slate-900/60 border border-slate-800 rounded-xl overflow-hidden">
          ...
          <div className="divide-y divide-slate-800">
            {RECENT_INCIDENTS.map(inc => (
              ...
              >
                <Badge
                  className={`text-[9px] px-1.5 py-0.5 border ${SEVERITY_COLORS[inc.severity]} shrink-0`}
                >
                  {inc.severity}
                </Badge>
