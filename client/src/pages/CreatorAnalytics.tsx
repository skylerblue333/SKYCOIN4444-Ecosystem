    ...
    <Card className="bg-gray-900/60 border-gray-700/50 hover:border-cyan-500/30 transition-all">
      <CardContent className="p-5">
        <div className="flex items-start justify-between mb-3">
          ...
          {change !== undefined && (
            <div
              className={`flex items-center gap-1 text-xs font-medium ${isPositive ? "text-purple-400" : "text-red-400"}`}
            >
              {isPositive ? (
    ...
    <div className="flex items-center gap-4 p-4 bg-gray-800/40 rounded-xl border border-gray-700/30 hover:border-cyan-500/20 transition-all">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between mb-1">
          ...
          <span className="text-xs text-cyan-400 ml-2 shrink-0">
            {progress}%
          </span>
        </div>
      ...
      {milestone.completed && (
        <Badge className="bg-purple-600/20 text-purple-400 border-purple-500/30 shrink-0">
          Done
        </Badge>
      )}
    </div>
...
export default function CreatorAnalytics() {
  ...
  const { data: analytics, isLoading: analyticsLoading } =
    trpc.creator.analytics.useQuery(undefined, { enabled: !!user });
  const { data: earnings } = trpc.creator.earnings.useQuery(undefined, {
    enabled: !!user,
  });
  const { data: milestones } = trpc.creator.milestones.useQuery(undefined, {
    enabled: !!user,
  });
  const { data: fanScores } = trpc.creator.fanScores.useQuery(undefined, {
    enabled: !!user,
  });
  const { data: forecast } = trpc.creator.revenueForecasting.useQuery(
    undefined,
    { enabled: !!user }
  );
  const { data: subscribers } = trpc.creator.mySubscriptions.useQuery(
    undefined,
    { enabled: !!user }
  );
  const chartData = useMemo(
    () => generateChartData(period === "7d" ? 7 : period === "30d" ? 30 : 90),
    [period]
  );
    ...
    <div className="min-h-screen bg-gray-950 text-white">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          ...
          <StatCard
            label="Followers"
            value={a?.followers || a?.followerCount || 0}
            change={8}
            icon={Users}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <StatCard
            label="Subscribers"
            value={e?.subscriptions || subscribers?.length || 0}
            change={15}
            icon={Crown}
                  ...
                  {[
                    {
                      ...
                      value: a?.postsCount || a?.posts || 0,
                      icon: MessageSquare,
                      color: "text-blue-400",
                    },
                    {
                      label: "Total Likes",
                      value: a?.totalLikes || a?.likes || 0,
                      icon: Heart,
                      color: "text-pink-400",
                    },
                    {
                      label: "Total Comments",
                      value: a?.totalComments || a?.comments || 0,
                      icon: MessageSquare,
                      color: "text-purple-400",
                    },
                    {
                      label: "Total Shares",
                      value: a?.totalShares || 0,
                      icon: Share2,
                      color: "text-purple-400",
                    },
                    {
                      label: "Video Views",
                      value: a?.videoViews || 0,
                      icon: Play,
                      color: "text-orange-400",
                    },
                  ].map(item => (
                    ...
                    <>
                      <div className="text-center py-4">
                        <div className="text-3xl font-bold text-purple-400">
                          $
                          {Number(
                            (forecast as any).projected30Day ||
                              (forecast as any).monthly ||
                              0
                          ).toFixed(2)}
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        <div className="bg-gray-800/50 rounded-lg p-3 text-center">
                          <div className="text-lg font-bold text-cyan-400">
                            $
                            {Number(
                              (forecast as any).subscriptionRevenue || 0
                            ).toFixed(2)}
                          </div>
                        ...
                        <div className="bg-gray-800/50 rounded-lg p-3 text-center">
                          <div className="text-lg font-bold text-yellow-400">
                            $
                            {Number((forecast as any).tipRevenue || 0).toFixed(
                              2
                            )}
                          </div>
              ...
              <Card className="bg-gray-900/60 border-gray-700/50">
                ...
                <CardContent>
                  ...
                  <div className="text-xs text-purple-400 flex items-center gap-1">
                    <TrendingUp className="w-3 h-3" />+{a?.followerGrowth || 8}%
                    this month
                  </div>
                ...
                <CardContent>
                  ...
                  <div className="space-y-2">
                    {[
                      {
                        label: "Organic",
                        value: `${a?.organicReach || 72}%`,
                        color: "text-purple-400",
                      },
                      {
                        label: "Viral",
                        value: `${a?.viralReach || 18}%`,
                        color: "text-purple-400",
                      },
                      {
                        label: "Paid",
                        value: `${a?.paidReach || 10}%`,
                        color: "text-yellow-400",
                      },
                    ].map(item => (
                      <div
                        ...
                        >
                          ...
                          <div className="flex-1">
                            <div className="text-sm font-medium text-white">
                              {fan.username || fan.name || "Fan"}
                            </div>
                            <div className="text-xs text-gray-400">
                              Score: {fan.score || fan.engagementScore || 0}
                            </div>
                          </div>
                          <div className="text-right">
                            ...
                            <div className="text-xs text-gray-500">
                              {fan.interactions || 0} interactions
                            </div>
                          </div>
                        </div>
                      ))}
