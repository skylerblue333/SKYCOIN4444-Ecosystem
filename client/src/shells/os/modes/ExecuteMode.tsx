...
export function ExecuteMode() {
  ...
  const sendMessage = async (text?: string) => {
    ...
    try {
      const res = await aiChat.mutateAsync({userId: user?.id || "guest", 
        message: content,
      ...
      const aiContent =
        (res as any)?.content ?? (res as any)?.message ?? "Action queued.";
      const hasAction = intent.type !== ACTION_TYPES.PLAIN_TEXT;
      const payloadAmount = (intent.payload as any)?.amount as
        number | undefined;
      setMessages(prev => [
        ...prev,
        {
          ...
          actionLabel: hasAction
            ? intent.type === ACTION_TYPES.PAYMENT
              ? "Confirm Payment"
              : intent.type === ACTION_TYPES.TIP
                ? "Send Tip"
                : intent.type === ACTION_TYPES.CALL_AI_AGENT
                  ? "Launch Agent"
                  : intent.type === ACTION_TYPES.CREATE_LISTING
                    ? "Create Listing"
                    : "Execute"
            : undefined,
            ...
            <div
              className={`max-w-[85%] rounded-2xl px-4 py-3 ${
                msg.role === "user"
                  ...
                  : msg.role === "system"
                    ? "bg-secondary/30 text-muted-foreground text-sm border border-border/30"
            ...
            >
              ...
              {msg.actionLabel && (
                <button className="mt-2 w-full flex items-center justify-center gap-1.5 px-3 py-1.5 bg-primary/20 hover:bg-primary/30 rounded-lg text-xs font-medium text-primary transition-colors">
                  ...
                  {msg.actionLabel}
                  {msg.actionAmount ? ` — $${msg.actionAmount}` : ""}
                </button>
              )}
            </div>
          </div>
