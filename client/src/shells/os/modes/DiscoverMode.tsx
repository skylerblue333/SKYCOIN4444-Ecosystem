...
export function DiscoverMode() {
  ...
  const TRENDING_TOPICS: { tag: string; posts: string; hot: boolean }[] =
          ...
          <div className="grid grid-cols-3 gap-2">
            {AI_PERSONAS.map(p => (
              <button
                key={p.name}
                onClick={() =>
                  handleActionFromPost(`Talk to ${p.name} — ${p.role}`)
                }
              ...
              >
                <div
                  className={`w-10 h-10 rounded-full bg-gradient-to-br ${p.color} flex items-center justify-center text-sm font-bold text-white`}
                >
                  {p.avatar}
                </div>
        ...
        <div className="space-y-3">
          {posts.length === 0 ? (
            ...
            posts.map((post: any) => (
              <div
                key={post.id}
                className="card p-4 hover:border-slate-700/60 transition-colors"
              >
                <div className="flex items-start gap-3">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="flex items-center gap-1 mt-3">
                      ...
                      <button
                        onClick={() =>
                          handleActionFromPost(
                            `Based on this post: "${post.content.slice(0, 80)}" — what action should I take?`
                          )
                        }
