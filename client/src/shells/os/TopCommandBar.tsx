      ...
      <div className="p-4">
        ...
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2">
            <div
              className={`w-7 h-7 rounded-lg bg-gradient-to-br ${group.accent} flex items-center justify-center`}
            >
              <group.icon className="w-3.5 h-3.5 text-white" />
            </div>
            <span className="text-sm font-bold text-white">{group.label}</span>
            <span className="text-[10px] text-slate-500 ml-1">
              {group.sections.reduce((a, s) => a + s.items.length, 0)} features
            </span>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="grid grid-cols-3 gap-4">
          {group.sections.map(section => (
            <div key={section.title}>
              <p
                className={`text-[10px] font-semibold uppercase tracking-wider mb-2 ${group.accentText}`}
              >
                {section.title}
              </p>
              <div className="space-y-0.5">
                {section.items.map(item => (
                  ...
                  >
                    <div
                      className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 border ${group.accentBorder} ${group.accentBg} transition-colors`}
                    >
                      <item.icon
                        className={`w-3.5 h-3.5 ${group.accentText}`}
                      />
                    </div>
    ...
    >
      ...
      >
        ...
        <div className="py-1.5 max-h-72 overflow-y-auto">
          {filtered.map(route => (
            ...
            >
              ...
              <span className={`text-[10px] ${route.accentText} opacity-60`}>
                {route.group}
              </span>
            </button>
          ))}
        ...
        <div className="px-4 py-2 border-t border-slate-800 flex items-center gap-3 text-[10px] text-slate-600">
          <span>
            <kbd className="border border-slate-700 rounded px-1">↑↓</kbd>{" "}
            navigate
          </span>
          <span>
            <kbd className="border border-slate-700 rounded px-1">↵</kbd> go
          </span>
          <span>
            <kbd className="border border-slate-700 rounded px-1">ESC</kbd>{" "}
            close
          </span>
...
export function TopCommandBar() {
  const { mode, setMode, shell, toggleShell, unreadCount, clearUnread } =
    useAppStore();
  const { user } = useAuth();
  ...
  const toggleGroup = useCallback(
    (id: string) => setActiveGroup(prev => (prev === id ? null : id)),
    []
  );
      ...
      >
        ...
        <div className="flex items-center gap-1 px-3 h-12">
          ...
          <div className="hidden lg:flex items-center gap-0.5 flex-1 relative">
            ...
            <Link href="/dashboard">
              <button
                className="flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-xs font-medium text-slate-500 hover:text-white hover:bg-slate-800/60 transition-all duration-150"
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            <button
              onClick={toggleShell}
              title={
                shell === "os" ? "Switch to Legacy Mode" : "Switch to OS Mode"
              }
            ...
            {user && (
              <button
                onClick={() => {
                  if (shell !== "os") toggleShell();
                  useAppStore.getState().setMode("identity");
                }}
          ...
          <div className="lg:hidden border-t border-slate-800/60 bg-[#07050f]/99 backdrop-blur-2xl max-h-[80vh] overflow-y-auto">
            ...
            <div className="px-3 pt-3 pb-2">
              ...
              >
                ...
                <kbd className="text-[10px] text-slate-600 border border-slate-700 rounded px-1.5 py-0.5 font-mono">
                  ⌘K
                </kbd>
              </button>
            </div>
            ...
            {NAV_GROUPS.map(group => (
              ...
              >
                <div className="flex items-center gap-2 mb-2">
                  <div
                    className={`w-5 h-5 rounded-md bg-gradient-to-br ${group.accent} flex items-center justify-center`}
                  >
                    <group.icon className="w-3 h-3 text-white" />
                  </div>
                  <p
                    className={`text-[11px] font-bold uppercase tracking-wider ${group.accentText}`}
                  >
                    {group.label}
                  </p>
                </div>
                    ...
                    .map(item => (
                      ...
                      >
                        <div className="flex items-center gap-2.5 px-3 py-3 rounded-xl hover:bg-slate-800/70 active:bg-slate-700/80 active:scale-[0.98] transition-all duration-150 cursor-pointer border border-transparent hover:border-slate-700/50">
                          <item.icon
                            className={`w-4 h-4 ${group.accentText} shrink-0`}
                          />
